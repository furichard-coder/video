...
function signature(fileName: string): FilenameSignature {
  ...
  const date = (
    stem.match(/(?:^|[^0-9])((?:19|20)\d{2}(?:[\s_.-]?(?:0[1-9]|1[0-2])(?:[\s_.-]?(?:0[1-9]|[12]\d|3[01]))?)?)/)?.[1] ??
    ""
  ).replace(/[^0-9]/g, "");
...
function sameSequenceFamily(current: FilenameSignature, neighbor: FilenameSignature): boolean {
  if (current.numericOnly || neighbor.numericOnly)
    return current.numericOnly && neighbor.numericOnly && current.length === neighbor.length;
  if (!current.prefix && !current.date) return false;
  return current.prefix === neighbor.prefix && current.date === neighbor.date;
}

export function filenameDisplayPriority(
  fileName: string,
  neighboringFileNames: Array<string | undefined>,
): FilenameDisplayPriority {
