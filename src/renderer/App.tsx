...
export function App() {
  ...
  const [historyState, setHistoryState] = useState<ProjectHistoryState>({
    canUndo: false,
    canRedo: false,
    undoCount: 0,
    redoCount: 0,
  ...
  const [concatRenderPurpose, setConcatRenderPurpose] = useState<"CONCAT" | "INTRO">("CONCAT");
  ...
  const [materialAnalysisAsset, setMaterialAnalysisAsset] = useState<SourceAsset>();
  const [materialAnalysisScope, setMaterialAnalysisScope] = useState<SubtitleTimelineScope>("MAIN");
  ...
  const setVolumeAsset = (asset?: SourceAsset) => {
    if (!asset) {
      setVolumeAssetState(undefined);
      setMaterialEditorAsset(undefined);
      return;
    }
    if (materialEditorAsset?.id === asset.id) {
      setMaterialEditorAsset(undefined);
      setVolumeAssetState(asset);
      return;
    }
  ...
  const setMediaInsertionVideo = (asset?: SourceAsset) => {
    if (!asset) {
      setMediaInsertionVideoState(undefined);
      setMaterialEditorAsset(undefined);
      return;
    }
    if (materialEditorAsset?.id === asset.id) {
      setMaterialEditorAsset(undefined);
      setMediaInsertionVideoState(asset);
      return;
    }
    setMaterialEditorAsset(asset);
  ...
  useEffect(() => {
    void Promise.all([
      window.sourceApp.getProject(),
      window.sourceApp.getProjectFileState(),
      window.sourceApp.getAppInfo(),
      window.sourceApp.getUserPreferences(),
    ])
      .then(([restored, fileState, info, preferences]) => {
        setProject(restored);
  ...
  const closeProjectViews = () => {
    ...
    setShowConcatRender(false);
    setShowIntroStudio(false);
    setExternalAsset(undefined);
    setVolumeAssetState(undefined);
    setShowBgm(false);
    setShowSubtitles(false);
    setConfirmRemoveAsset(undefined);
    setPlacementQueue([]);
    setLastRemovedMainId(undefined);
    setMaterialAnalysisAsset(undefined);
    setMediaInsertionVideoState(undefined);
  ...
  const runHistory = useCallback(
    async (direction: "UNDO" | "REDO") => {
      ...
      try {
        const restored =
          direction === "UNDO" ? await window.sourceApp.undoProject() : await window.sourceApp.redoProject();
        closeProjectViews();
        setProject(restored);
        setNotice(
          direction === "UNDO"
  ...
  const moveAsset = async (assetId: string, toIndex: number) => {
    try {
      setProject(await window.sourceApp.moveTimelineAsset(assetId, toIndex));
      setNotice("素材順序已保存；對應字幕已依目前疊化秒數同步移動。");
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : String(reason));
    }
  };
  ...
  const finishGridDrag = async () => {
    ...
    try {
      ...
      setProject(updated);
      setNotice(`已將素材移到第 ${finalIndex + 1} 項；對應字幕已依目前疊化秒數同步移動。`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : String(reason));
      try {
        setProject(await window.sourceApp.getProject());
      } catch {
        /* Keep the last visible state if reload also fails. */
      }
    }
  };
  ...
  const decidePlacement = async (assetId: string, placement: import("../shared/domain").PlacementRequest) => {
    ...
    if (placement.action !== "PENDING") setNotice("素材已插入指定位置；後方字幕順序與時間已依疊化設定同步重算。");
  };
  ...
  const changeViewMode = async (next: ViewMode) => {
    ...
    setError(undefined);
    try {
      await window.sourceApp.updateUserPreferences({ viewMode: next });
    } catch (reason) {
  ...
  const openExternalDirect = async (asset: SourceAsset) => {
    ...
    try {
      ...
      if (result.status === "USE_INTERNAL") {
        ...
        setNotice(
          result.message ??
            `已使用 ${result.playerLabel} 開啟 ${asset.fileName} 的唯讀意圖原檔；實際來源保護仍取決於外部播放器。`,
        );
      }
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : String(reason));
    } finally {
      setExternalOpeningId(undefined);
    }
  };
  ...
  const renderInProgress = backgroundJobs.some((job) => job.kind === "CONCAT_RENDER" && job.status === "RUNNING");
    ...
    <main className="app-shell">
      <header className="topbar">
          ...
          <div className="file-menu">
            ...
            {showFileMenu && (
              <div className="file-menu-popover" role="menu">
                <div className="file-menu-project">
                  <strong>{project.name}</strong>
                  <small title={projectFilePath}>{projectFilePath ?? "尚未另存為專案檔"}</small>
                </div>
                <button
                  role="menuitem"
                  type="button"
                ...
                <button role="menuitem" type="button" onClick={() => void saveProjectFile(true)}>
                  另存專案…
                </button>
                <button role="menuitem" type="button" onClick={() => void openProjectFile()}>
                  開啟舊專案…
                </button>
              </div>
            )}
        ...
        <div className="topbar-actions">
          <div className="history-controls" aria-label="復原與重做">
            <button
              type="button"
              disabled={historyBusy || !historyState.canUndo}
              title="復原上一步（Ctrl+Z）"
            ...
            <button
              type="button"
              disabled={historyBusy || !historyState.canRedo}
              title="重做下一步（Ctrl+Y）"
          ...
          <span className="version-pill" aria-label={`軟體版本 ${appInfo?.version ?? "讀取中"}`}>
            版本 v{appInfo?.version ?? "…"}
          </span>
          <button
            className="settings-button display-settings-button"
            type="button"
            aria-label="Aa 設定"
            title="介面文字與縮放設定"
          ...
          <button className="settings-button ai-settings-button" type="button" onClick={() => setShowAiSettings(true)}>
            ✦ AI 帳號／故事
          </button>
          <button
            className="settings-button youtube-settings-button"
            type="button"
            onClick={() => setShowYoutubeSettings(true)}
          >
            平台上傳設定
          </button>
          <button
            className="settings-button output-history-button"
            type="button"
            onClick={() => setShowOutputHistory(true)}
          >
            ▣ 預覽檔案庫
          </button>
          ...
          <div className="protection-pill">
            <span>●</span>
            <div>
              <strong>來源唯讀保護已開啟</strong>
              <small>移除清單 ≠ 刪除檔案</small>
            </div>
          </div>
          <span className={`save-state save-state-${saveState}`} role="status">
            {saveState === "SAVING"
              ...
              : saveState === "ERROR"
                ? "保存失敗"
                : `已保存${savedAt ? ` ${new Date(savedAt).toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" })}` : ""}`}
          </span>
        </div>
      </header>
      ...
      <section className="hero">
        <div>
          ...
          <div className="hero-actions">
            <button className="primary-button" type="button" disabled={busy} onClick={() => void runImport("FILES")}>
              <span>＋</span> 選擇多個檔案
            </button>
            <button className="secondary-button" type="button" disabled={busy} onClick={() => void runImport("FOLDER")}>
              選擇來源資料夾
            </button>
            {busy && (
              <button className="cancel-button" type="button" onClick={() => void window.sourceApp.cancelImport()}>
                取消載入
              </button>
            )}
          </div>
        ...
        <div className="hero-summary">
          ...
          <div>
            <strong>{project.sources.filter((asset) => asset.kind === "VIDEO").length}</strong>
            <span>影片</span>
          </div>
          <div>
            <strong>{project.sources.filter((asset) => asset.kind === "IMAGE").length}</strong>
            <span>照片</span>
          </div>
        </div>
      </section>
      ...
      {busy && (
        <section className="progress-banner" aria-live="polite">
          <span className="spinner" />
          <div>
            <strong>{progress?.phase === "ADDING" ? "正在加入來源清單" : "正在找尋支援的媒體"}</strong>
            <span>{progress?.currentName ?? "請稍候…"}</span>
          </div>
          <span>
            {progress?.processed ?? 0} / {progress?.discovered ?? 0}
          </span>
        </section>
      )}
      {backgroundJobs.length > 0 && (
        <section className="background-jobs-banner" aria-live="polite" aria-label="背景工作進度">
          <div>
            <strong>背景工作</strong>
            <span>
              {renderInProgress
                ? "轉檔尚未完成：Windows 防睡眠保護已開啟，App 也暫時不可關閉。"
                : "關閉功能視窗不會中斷；完成後結果會保存到專案或預覽檔案庫。"}
            </span>
          </div>
          <div className="background-jobs-list">
            {backgroundJobs.slice(0, 4).map((job) => (
              <span className={`background-job is-${job.status.toLowerCase()}`} key={job.id}>
                <b>
                  {job.status === "RUNNING"
                    ? "●"
      ...
      {notice && (
        <div className="notice success notice-with-action" role="status">
          <span>{notice}</span>
          {lastRemovedMainId && project.excludedMainAssetIds.includes(lastRemovedMainId) && (
            <button type="button" onClick={() => void restoreMainAsset(lastRemovedMainId)}>
              撤銷，恢復正片
            </button>
          )}
        </div>
      ...
      <section className="library-section">
        <nav className="workspace-nav" aria-label="主要工作區">
          <button className={activeWorkspace === "SOURCES" ? "active" : ""} onClick={() => openWorkspace("SOURCES")}>
            01 素材與順序
          </button>
          <button className={activeWorkspace === "INTRO" ? "active" : ""} onClick={() => openWorkspace("INTRO")}>
            02 片頭
          </button>
          <button className={activeWorkspace === "BGM" ? "active" : ""} onClick={() => openWorkspace("BGM")}>
            03 配樂
          </button>
          <button
            className={activeWorkspace === "SUBTITLES" ? "active" : ""}
            onClick={() => openWorkspace("SUBTITLES")}
          >
            04 字幕
          </button>
          <button
            className={activeWorkspace === "WATERMARK" ? "active" : ""}
            onClick={() => openWorkspace("WATERMARK")}
          >
            05 浮水印
          </button>
          <button className={activeWorkspace === "OUTPUTS" ? "active" : ""} onClick={() => openWorkspace("OUTPUTS")}>
            06 預覽輸出／成品庫
          </button>
          <button className={activeWorkspace === "PUBLISH" ? "active" : ""} onClick={() => openWorkspace("PUBLISH")}>
            07 發布設定
          </button>
          <button
            className={activeWorkspace === "AI_PUBLISH" ? "active" : ""}
            onClick={() => openWorkspace("AI_PUBLISH")}
          >
            08 AI 發布素材
          </button>
        </nav>
        <div className="library-toolbar">
          <div>
            <span className="eyebrow">PROJECT SOURCE MANIFEST</span>
            <h2>素材清單</h2>
          </div>
          <div className="toolbar-actions">
            <label className="sort-control">
              <span>排序</span>
              <select value={project.sortMode} onChange={(event) => void changeSort(event.target.value as SortMode)}>
            ...
            <div className="segmented-control" aria-label="檢視方式">
              <button
                type="button"
                className={viewMode === "GRID" ? "active" : ""}
                onClick={() => void changeViewMode("GRID")}
              >
                網格
              </button>
              <button
            ...
            <button
              ...
              type="button"
              disabled={!sortedAssets.length}
              onClick={() => {
                setMaterialEditorAsset(sortedAssets[0]);
                setMaterialEditorSection("CLIP");
              }}
            >
              素材工作區
            </button>
            ...
            <button
              ...
              type="button"
              disabled={!mainClips.length}
              title={!mainClips.length ? "正片沒有可供輸出的保留影片片段" : undefined}
              onClick={() => {
                setConcatRenderPurpose("CONCAT");
                setShowConcatRender(true);
              }}
            >
              ⇄ 產出串連預覽
            </button>
            <button className="ai-intro-button" type="button" onClick={() => setShowIntroStudio(true)}>
              ✦ AI 精彩片頭
            </button>
            <button className="music-button" type="button" onClick={() => setShowBgm(true)}>
              ♫ 配樂 {project.bgmTracks.length ? `(${project.bgmTracks.length})` : ""}
            </button>
            <button className="subtitle-button" type="button" onClick={() => setShowSubtitles(true)}>
              CC 字幕 {project.subtitleCues.length ? `(${project.subtitleCues.length})` : ""}
            </button>
          </div>
        </div>
        {viewMode === "GRID" && sortedAssets.length > 1 && (
          <p className="grid-reorder-help">
            長按卡片或縮圖後拖曳，即時調整前後順序；放開後自動保存。也可使用卡片下方的上移／下移。
          </p>
        )}
        ...
        {sortedAssets.length === 0 ? (
          <div className="empty-state">
            ...
            <h2>{project.sources.length ? "正片清單目前為空" : "尚未加入素材"}</h2>
            <p>
              {project.sources.length
                ? "所有素材都已從正片移除或仍待決定。來源檔仍在原位置，可從最近移除區恢復。"
                : "選擇數個影片／照片，或直接選擇一個來源資料夾。App 只讀取檔案並建立自己的預覽快取。"}
            </p>
            {!project.sources.length && (
              <button className="primary-button" type="button" onClick={() => void runImport("FILES")}>
                開始選擇素材
              </button>
            )}
          </div>
          ...
          <div className={`asset-collection ${viewMode === "LIST" ? "list-view" : "grid-view"}`}>
            {sortedAssets.map((asset, index) => (
              <AssetCard
                ...
                asset={asset}
                index={index}
                previousFileName={sortedAssets[index - 1]?.fileName}
                nextFileName={sortedAssets[index + 1]?.fileName}
                viewMode={viewMode}
                onOpen={setSelectedAsset}
                onOpenExternalDirect={(item) => void openExternalDirect(item)}
                onOpenExternalOptions={setExternalAsset}
                externalOpening={externalOpeningId === asset.id}
                onOpenVolume={setVolumeAsset}
                onOpenMaterialAnalysis={(item) => {
                  setMaterialAnalysisScope("MAIN");
                  setMaterialAnalysisAsset(item);
                }}
                onOpenMediaInsertion={setMediaInsertionVideo}
                mediaInsertions={project.mediaInsertions
                  .filter((item) => item.anchorVideoAssetId === asset.id)
                  .map((item) => {
                    const inserted = project.sources.find((candidate) => candidate.id === item.insertedAssetId);
                    return {
                      ...item,
                      insertedFileName: inserted?.fileName ?? "找不到素材",
                      insertedKind: inserted?.kind ?? "IMAGE",
                    };
                  })}
                ...
                onMove={(delta) => void moveAsset(asset.id, index + delta)}
                dragging={draggingAssetId === asset.id}
                dragTarget={Boolean(draggingAssetId && dragTargetAssetId === asset.id && draggingAssetId !== asset.id)}
        ...
        {pendingAssets.length > 0 && (
          <section className="pending-section">
            <div>
              <span className="eyebrow">PENDING PLACEMENT</span>
              <h3>待決定區 · {pendingAssets.length} 項</h3>
              <p>此區素材尚未進入最終順序，也不會加入串連輸出。</p>
            </div>
            <div className="pending-list">
              {pendingAssets.map((asset) => (
                <div key={asset.id}>
                  <strong>{asset.fileName}</strong>
                  <span>{asset.kind === "VIDEO" ? "影片" : "照片"}</span>
                  <button
                    onClick={() =>
                      setPlacementQueue((current) => [asset.id, ...current.filter((id) => id !== asset.id)])
                    }
                  >
                    決定安插位置
                  </button>
                  <button aria-label={`從正片移除 ${asset.fileName}`} onClick={() => setConfirmRemoveAsset(asset)}>
                    從正片移除
                  </button>
                </div>
              ))}
        ...
        {project.recentMainRemovals.length > 0 && (
          <section className="recent-removals" aria-label="最近從正片移除">
            <div>
              <span className="eyebrow">RECENTLY REMOVED</span>
              <h3>最近從正片移除</h3>
              <p>只排除專案引用，磁碟來源與相關設定仍保留。</p>
            </div>
            <div>
              {[...project.recentMainRemovals].reverse().map((record) => {
                const asset = project.sources.find((item) => item.id === record.assetId);
                return asset ? (
                  <article key={record.assetId}>
                    <strong>{asset.fileName}</strong>
                    <span>
                      {record.wasPending ? "原本在待決定區" : `原順序 ${Number(record.previousTimelineIndex) + 1}`}
                    </span>
                    <button type="button" onClick={() => void restoreMainAsset(record.assetId)}>
                      恢復正片
                    </button>
                  </article>
                ) : null;
        ...
        {project.subtitleCues.length > 0 &&
          (projectScopeNeedsReview(project, "MAIN") || projectScopeNeedsReview(project, "INTRO")) && (
            <button className="subtitle-review-banner" onClick={() => setShowSubtitles(true)}>
              ⚠ 片頭或正片時間線已改變，對應字幕需要複核
            </button>
          )}
      </section>

      <footer className="app-footer">
        <span title={projectFilePath}>
          專案：{project.name}
          {projectFilePath ? " · 已連結專案檔" : " · App Data 自動恢復"}
        </span>
        <span>Preview cache：衍生檔、可重建、不可作正式輸出</span>
        <span>串連預覽：低解析交接檢查，非正式 Master</span>
      </footer>

      {selectedAsset && (
        <PreviewModal
          asset={project.sources.find((asset) => asset.id === selectedAsset.id) ?? selectedAsset}
          project={project}
          onClose={() => setSelectedAsset(undefined)}
          onAssetUpdated={updateAsset}
          onProjectUpdated={setProject}
          onOpenExternal={setExternalAsset}
          onOpenIntro={() => {
            setSelectedAsset(undefined);
      ...
      {materialEditorAsset && (
        <MaterialEditorModal
          asset={project.sources.find((asset) => asset.id === materialEditorAsset.id) ?? materialEditorAsset}
          initialSection={materialEditorSection}
          onClose={() => setMaterialEditorAsset(undefined)}
          onOpenSection={(section) => {
            setMaterialEditorSection(section);
            if (section === "VOLUME") {
              setMaterialEditorAsset(undefined);
              setVolumeAsset(materialEditorAsset);
            } else if (section === "INSERT") {
              setMaterialEditorAsset(undefined);
              setMediaInsertionVideo(materialEditorAsset);
            } else if (section === "CLIP" || section === "PREVIEW") {
              setMaterialEditorAsset(undefined);
              setSelectedAsset(materialEditorAsset);
            }
          }}
        />
      )}
      {showPlaylist && mainClips.length > 0 && (
        <PlaylistModal
      ...
      {showConcatRender && mainClips.length > 0 && (
        <ConcatRenderModal
          ...
          assets={project.sources}
          mainClips={mainClips}
          introClips={project.introSegments}
          introSegmentMaxDurationMs={project.introSegmentMaxDurationMs}
          confirmedSubtitleCount={
            project.subtitleCues.filter(
              (cue) =>
                (cue.reviewStatus ?? "CONFIRMED") === "CONFIRMED" &&
                (cue.timelineScope ?? "MAIN") === (concatRenderPurpose === "INTRO" ? "INTRO" : "MAIN"),
            ).length
          }
          subtitlesNeedReview={
            project.subtitleCues.some(
              (cue) => (cue.timelineScope ?? "MAIN") === (concatRenderPurpose === "INTRO" ? "INTRO" : "MAIN"),
      ...
      {showIntroStudio && (
        <IntroStudio
          project={project}
          videos={sortedVideos}
          onClose={() => setShowIntroStudio(false)}
          onProjectUpdated={setProject}
          onOpenMaterialAnalysis={(asset) => {
            setMaterialAnalysisScope("INTRO");
            setMaterialAnalysisAsset(asset);
          }}
        />
      )}
      ...
      {showOutputHistory && (
        <OutputHistoryModal
          onOpenYoutubeSettings={() => setShowYoutubeSettings(true)}
          onClose={() => setShowOutputHistory(false)}
        />
      )}
      {showAiPublish && (
        <AiPublishStudio project={project} onProjectUpdated={setProject} onClose={() => setShowAiPublish(false)} />
      )}
      ...
      {showAiSettings && (
        <AiSettingsModal project={project} onProjectUpdated={setProject} onClose={() => setShowAiSettings(false)} />
      )}
      {externalAsset && <ExternalOpenModal asset={externalAsset} onClose={() => setExternalAsset(undefined)} />}
      {volumeAsset && (
        <VolumeSegmentsModal
          asset={project.sources.find((asset) => asset.id === volumeAsset.id) ?? volumeAsset}
          defaultVolumePercent={project.sourceAudioVolumePercent ?? 100}
          onClose={() => setVolumeAsset(undefined)}
          onAssetUpdated={updateAsset}
        />
      )}
      {materialAnalysisAsset && (
        <MaterialSubtitleAnalysisModal
          asset={project.sources.find((asset) => asset.id === materialAnalysisAsset.id) ?? materialAnalysisAsset}
          project={project}
          initialScope={materialAnalysisScope}
          onProjectUpdated={setProject}
          onNotice={setNotice}
          onOpenSubtitleReview={(updated) => {
            setProject(updated);
            setMaterialAnalysisAsset(undefined);
            openWorkspace("SUBTITLES");
          }}
          onClose={() => setMaterialAnalysisAsset(undefined)}
        />
      )}
      {placementQueue[0] &&
        (() => {
          const asset = project.sources.find((item) => item.id === placementQueue[0]);
          return asset ? (
            <PlacementModal
              asset={asset}
              anchors={sortedAssets}
              onClose={() => setPlacementQueue((current) => current.slice(1))}
              onDecide={(placement) => decidePlacement(asset.id, placement)}
            />
          ) : null;
        })()}
      {showBgm && (
      ...
      {confirmRemoveAsset && (
        <ConfirmMainRemovalModal
          asset={confirmRemoveAsset}
          project={project}
          busy={removeBusy}
          onCancel={() => setConfirmRemoveAsset(undefined)}
          onConfirm={() => void removeAsset(confirmRemoveAsset)}
        />
      )}
      {mediaInsertionVideo && (
        <MediaInsertionModal
          project={project}
          video={project.sources.find((asset) => asset.id === mediaInsertionVideo.id) ?? mediaInsertionVideo}
          onProjectUpdated={replaceProject}
          onClose={() => setMediaInsertionVideo(undefined)}
        />
      )}
      {showCloseConfirmation && (
        <div className="modal-backdrop close-confirm-backdrop" role="presentation">
          <section className="close-confirm-modal" role="dialog" aria-modal="true" aria-label="再次確認關閉軟體">
            <div className="close-confirm-mascot" aria-hidden="true">
              🎬<span>✨</span>
            </div>
            ...
            <div>
              ...
              <button
                ...
                onClick={() => window.sourceApp.confirmAppClose()}
              >
                確定關閉
              </button>
            </div>
