...
export function AssetCard({
  ...
  const armDragAfterHold = (event: React.PointerEvent<HTMLElement>) => {
    ...
    if (
      interactive &&
      !interactive.classList.contains("thumb-button") &&
      !interactive.classList.contains("drag-sort-hint")
    )
      return;
    clearDragHold();
  ...
  useEffect(() => {
    ...
    void Promise.all([
      asset.metadataState === "READY" ? Promise.resolve(asset) : window.sourceApp.ensureMetadata(asset.id),
  ...
  const hasClipEdits = Boolean(
    asset.mainExclusionRanges?.length ||
    asset.zoomSegments?.length ||
    (asset.previewRange &&
      asset.mediaInfo?.durationMs &&
      (asset.previewRange.inMs > 0 || asset.previewRange.outMs < asset.mediaInfo.durationMs)),
  );
  ...
  return (
    ...
    >
      <div className="thumb-frame">
        {inlinePreview && asset.kind === "VIDEO" ? (
          <video
            ...
            onContextMenu={(event) => {
              event.preventDefault();
              onOpenExternalOptions(asset);
            }}
            onLoadedMetadata={(event) => {
              if (range) event.currentTarget.currentTime = range.inMs / 1000;
            }}
            onPlay={(event) => {
              if (
                range &&
                (event.currentTarget.currentTime * 1000 < range.inMs ||
                  event.currentTarget.currentTime * 1000 >= range.outMs)
              )
                event.currentTarget.currentTime = range.inMs / 1000;
            }}
            onTimeUpdate={keepPlaybackWithinRange}
          />
        ) : (
          <button
            ...
            onKeyDown={(event) => {
              if (asset.kind === "VIDEO" && (event.key === "ContextMenu" || (event.shiftKey && event.key === "F10"))) {
                event.preventDefault();
                onOpenExternalOptions(asset);
              }
            }}
            aria-label={
              asset.kind === "VIDEO" ? `從縮圖以外部播放器直接播放 ${asset.fileName}` : `預覽 ${asset.fileName}`
            }
            title={
              asset.kind === "VIDEO" ? "左鍵直接依副檔名設定，以外部播放器開啟唯讀意圖原檔" : "在 App 內等比例放大照片"
            }
          >
            {thumbnailUrl ? (
              ...
              <span className={`thumb-placeholder ${error ? "is-error" : ""}`}>
                {loading ? (
                  ...
                  <span className="fallback-icon">{asset.kind === "VIDEO" ? "▶" : "▧"}</span>
                )}
                <span>{loading ? "建立縮圖中" : "無法顯示縮圖"}</span>
              </span>
            )}
            <span className="preview-hint">
              {asset.kind === "VIDEO" ? (externalOpening ? "正在開啟外部播放器…" : "左鍵以外部播放器播放") : "點擊放大"}
            </span>
          </button>
        )}
        <span className="sequence-number">{String(index + 1).padStart(2, "0")}</span>
        {viewMode === "GRID" && (
          <span className={`drag-sort-hint ${dragArmed ? "is-armed" : ""}`} aria-hidden="true">
            {dragArmed ? "放開完成排序" : "⋮⋮ 長按拖曳排序"}
          </span>
        )}
        <span className="duration-chip">
          {asset.kind === "VIDEO"
            ? formatDuration(asset.mediaInfo?.durationMs)
            : `${(asset.imageDurationMs ?? DEFAULT_IMAGE_DURATION_MS) / 1000} 秒`}
        </span>
        {asset.kind === "VIDEO" && (
          <button
            ...
            onClick={() => void loadInlinePreview()}
          >
            {inlineLoading ? "載入中…" : inlinePreview ? "播放／暫停" : "▶ 網格播放"}
          </button>
        )}
        {inlinePreview && (
          <button className="inline-expand-button" type="button" onClick={() => onOpen(asset)}>
            放大
          </button>
        )}
      </div>
      ...
      {asset.kind === "VIDEO" && range && asset.mediaInfo?.durationMs && (
        <ClipRangeControl
          durationMs={asset.mediaInfo.durationMs}
          range={range}
          onChange={updateRange}
          onCommit={(next) => void commitRange(next)}
        />
      )}
      ...
      <div className="asset-body">
        <div className="asset-heading">
          <div>
            ...
            <h3
              className={fileNamePriority === "END" ? "prefer-filename-end" : "prefer-filename-start"}
          ...
          <button
            ...
            title="只從正片排除，不刪除來源檔"
            aria-label={`從正片移除 ${asset.fileName}`}
          >
            移除
          </button>
        </div>
        ...
        <dl className="asset-facts">
          ...
          <div>
            <dt>規格</dt>
            <dd>{formatResolution(asset.mediaInfo?.width, asset.mediaInfo?.height)}</dd>
          </div>
          <div>
            <dt>編碼</dt>
            <dd>{codec || "讀取中／未提供"}</dd>
        ...
        {viewMode === "LIST" && (
          <p className="source-path" title={asset.sourcePath}>
            {asset.sourcePath}
          </p>
        )}
        {asset.kind === "VIDEO" && asset.mainExclusionRanges?.length ? (
          <p className={`main-exclusion-status ${retainedDurationMs(asset) === 0 ? "is-fully-excluded" : ""}`}>
            {retainedDurationMs(asset) === 0
              ? "此素材有效範圍已全部排除，不參與正片輸出"
        ...
        {asset.kind === "VIDEO" && (
          <>
            <div className="card-tool-row">
              <button
                className={`clip-tool-button ${hasClipEdits ? "is-configured" : ""}`}
                type="button"
                title={hasClipEdits ? "已儲存片段修正；點擊查看或調整" : "刪除／排除部分片段、局部放大與高解析片段輸出"}
                aria-label={`刪除／排除部分片段${hasClipEdits ? "，已儲存設定" : ""}`}
                onClick={() => onOpen(asset)}
              >
                ✂ {hasClipEdits ? "已設片段" : "片段"}
                {asset.mainExclusionRanges?.length ? ` (${asset.mainExclusionRanges.length})` : ""}
              </button>
              <button
                className={`volume-tool-button ${hasVolumeEdits ? "is-configured" : ""}`}
                type="button"
                title={hasVolumeEdits ? "已儲存音量修正；點擊查看或調整" : "設定素材原音的分段音量"}
                aria-label={`音量區段${hasVolumeEdits ? "，已儲存設定" : ""}`}
                onClick={() => onOpenVolume(asset)}
              >
                ♫ {hasVolumeEdits ? "已設音量" : "音量"}
                {asset.volumeSegments?.length ? ` (${asset.volumeSegments.length})` : ""}
              </button>
              <button
                className={`insertion-tool-button ${hasInsertionEdits ? "is-configured" : ""}`}
                type="button"
                title={hasInsertionEdits ? "已儲存安插修正；點擊查看或調整" : "在這支影片的指定時間安插照片或影片"}
                aria-label={`於影片時段插入照片或影片${hasInsertionEdits ? "，已儲存設定" : ""}`}
                onClick={() => onOpenMediaInsertion(asset)}
              >
                ▧ {hasInsertionEdits ? "已設安插" : "安插"}
                {mediaInsertions.length ? ` (${mediaInsertions.length})` : ""}
              </button>
              <button
            ...
            {mediaInsertions.length > 0 && (
              <div className="photo-insertion-summary" aria-label={`${asset.fileName} 已安插素材`}>
                {[...mediaInsertions]
                  .sort((a, b) => a.atMs - b.atMs || a.sequenceIndex - b.sequenceIndex)
                  .map((item) => (
                    <button type="button" key={item.id} onClick={() => onOpenMediaInsertion(asset)}>
                      <strong>
                        {item.insertedKind === "VIDEO" ? "影片" : "照片"} · {item.insertedFileName}
                      </strong>
                      <span>
                        {formatDuration(item.atMs)} 插入 · 使用 {formatDuration(item.sourceOutMs - item.sourceInMs)}
                      </span>
                    </button>
                  ))}
              </div>
            )}
            {availableMediaCount === 0 && mediaInsertions.length === 0 && (
              <small className="external-open-hint">加入並排定其他照片或影片後，即可指定插入這支影片的時段。</small>
            )}
          </>
        )}
        {asset.kind === "IMAGE" && (
          <>
            <div className="image-duration-row">
              <label htmlFor={`image-duration-${asset.id}`}>照片顯示時間</label>
              <select
                id={`image-duration-${asset.id}`}
                aria-label={`${asset.fileName} 照片顯示時間`}
                disabled={imageDurationBusy}
                value={asset.imageDurationMs ?? DEFAULT_IMAGE_DURATION_MS}
                onChange={(event) => void changeImageDuration(Number(event.target.value))}
              >
            ...
            <label className="photo-sound-toggle">
              <input
                type="checkbox"
                checked={asset.photoSoundEnabled !== false}
                disabled={photoSoundBusy}
                onChange={(event) => void changePhotoSound(event.target.checked)}
              />
              <span>
                <strong>預設相機快門音效</strong>
                <small>沿用沙丘前案核准音效 · 輸出增益 70%</small>
              </span>
            </label>
            <label className="photo-analysis-toggle">
              <input
                type="checkbox"
                checked={photoAnalysisEnabled}
                onChange={(event) => setPhotoAnalysisEnabled(event.target.checked)}
              />
              <span>
                <strong>預設勾選：AI 分析照片</strong>
                <small>物種／物品／地點 → 簡化成可編輯字幕</small>
              </span>
            </label>
            <button
              className="material-analysis-card-button"
              type="button"
              disabled={!photoAnalysisEnabled}
              onClick={() => onOpenMaterialAnalysis(asset)}
            >
              ✦ 分析照片並產生字幕
            </button>
          </>
        ...
        {asset.kind === "VIDEO" && (
          <button
            className="material-analysis-card-button video-analysis-card-button"
            type="button"
            onClick={() => onOpenMaterialAnalysis(asset)}
          >
            ✦ 影格分析 → 字幕
