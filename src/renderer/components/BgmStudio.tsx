...
export function BgmStudio({ project, timelineDurationMs, onProjectUpdated, onClose }: Props) {
  ...
  useEffect(() => {
    setDrafts(structuredClone(project.bgmTracks));
  ...
  }, [project.bgmTracks, project.sourceAudioVolumePercent, project.musicSuggestionResult]);
  useEffect(() => {
  ...
  const patchTrack = (id: string, field: keyof BgmTrack, value: number) => setDrafts((current) => current.map((track) => track.id === id ? { ...track, [field]: Math.round(value * 1000) } : track));
  const patchPercent = (id: string, value: number) => setDrafts((current) => current.map((track) => track.id === id ? { ...track, volumePercent: value } : track));
  ...
  const save = async (track: BgmTrack) => { setBusy(true); setError(undefined); try { onProjectUpdated(await window.sourceApp.updateBgmTrack(track)); setNotice(`已保存 ${track.fileName} 的混音設定。`); } catch (reason) { report(reason); } finally { setBusy(false); } };
  const saveSourceVolume = async () => { setBusy(true); setError(undefined); try { onProjectUpdated(await window.sourceApp.setSourceAudioVolume(sourceVolume)); setNotice(`素材原音預設已設為 ${sourceVolume}%。`); } catch (reason) { report(reason); } finally { setBusy(false); } };
  const sequence = async () => { setBusy(true); setError(undefined); try { onProjectUpdated(await window.sourceApp.sequenceBgmTracks()); setNotice("已依目前清單順序，從影片開頭連續排列所有配樂。"); } catch (reason) { report(reason); } finally { setBusy(false); } };
  ...
  return <div className="modal-backdrop studio-backdrop" role="presentation"><section className="editor-modal bgm-modal" role="dialog" aria-modal="true" aria-label="配樂與混音">
    <header className="modal-header"><div><span className="eyebrow">BGM · READ-ONLY SOURCES</span><h2>配樂與混音</h2></div><button className="icon-button" onClick={onClose} aria-label="關閉">×</button></header>
    <div className="editor-modal-body">
      <div className="studio-summary bgm-summary"><div><span>目前影片時間基準</span><strong>{formatDuration(timelineDurationMs)}</strong></div><div className="source-volume-control"><label>素材原音預設<input aria-label="素材原音音量" type="number" min="0" max={MAX_MIX_VOLUME_PERCENT} value={sourceVolume} onChange={(event) => setSourceVolume(Number(event.target.value))} /><span>%</span></label><button className="secondary-button" disabled={busy} onClick={() => void saveSourceVolume()}>保存原音</button></div><p>素材原音預設 80%、新配樂預設 35%；兩者可在 0–200% 調整。混合後仍套用 0.95 peak limiter，因此 200% 是增益上限，不保證聽感兩倍。</p></div>
      ...
      {!drafts.length ? <div className="empty-mini">尚未加入配樂。影片原音預設以 80% 輸出，仍可由每個素材的音量區段覆寫。</div> : <div className="bgm-list">{drafts.map((track,index) => <article className={`bgm-track ${track.resolutionStatus === "NEEDS_LOCAL_FILE" ? "is-pending" : ""}`} key={track.id}>
        ...
        {track.resolutionStatus === "NEEDS_LOCAL_FILE" ? <div className="bgm-rights-resolution"><label><input type="checkbox" checked={rightsConfirmed[track.id] === true} onChange={(event) => setRightsConfirmed((current) => ({ ...current, [track.id]: event.target.checked }))} />我確認將指定的 MP3 是自有、已授權，或由 YouTube Audio Library 正式下載</label><button className="primary-button" disabled={busy || rightsConfirmed[track.id] !== true} onClick={() => void resolve(track)}>指定本機授權 MP3…</button></div> : <div className="bgm-fields"><label>Timeline 開始（秒）<input type="number" min="0" step="0.01" value={track.timelineInMs/1000} onChange={(e)=>patchTrack(track.id,"timelineInMs",Number(e.target.value))}/></label><label>Timeline 結束（秒）<input type="number" min="0" step="0.01" value={track.timelineOutMs/1000} onChange={(e)=>patchTrack(track.id,"timelineOutMs",Number(e.target.value))}/></label><label>Source IN（秒）<input type="number" min="0" step="0.01" value={track.sourceInMs/1000} onChange={(e)=>patchTrack(track.id,"sourceInMs",Number(e.target.value))}/></label><label>Source OUT（秒）<input type="number" min="0" max={track.durationMs/1000} step="0.01" value={track.sourceOutMs/1000} onChange={(e)=>patchTrack(track.id,"sourceOutMs",Number(e.target.value))}/></label><label>淡入（秒）<input type="number" min="0" step="0.01" value={track.fadeInMs/1000} onChange={(e)=>patchTrack(track.id,"fadeInMs",Number(e.target.value))}/></label><label>淡出（秒）<input type="number" min="0" step="0.01" value={track.fadeOutMs/1000} onChange={(e)=>patchTrack(track.id,"fadeOutMs",Number(e.target.value))}/></label><label>音量<input type="number" min="0" max={MAX_MIX_VOLUME_PERCENT} value={track.volumePercent} onChange={(e)=>patchPercent(track.id,Number(e.target.value))}/><span>%</span></label><button className="save-track-button" disabled={busy} onClick={() => void save(track)}>保存此曲</button></div>}
      </article>)}</div>}
      {notice && <div className="notice success">{notice}</div>}{error && <div className="notice error" role="alert">{error}</div>}
    </div><footer className="settings-footer"><p>本機 MP3 不會被修改；待補、離線或不存在會阻擋影片輸出並指出曲名。YouTube 連結只作來源與授權紀錄。</p><button className="primary-button" onClick={onClose}>完成</button></footer>
  </section></div>;
}
