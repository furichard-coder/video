...
function safeRetainedDuration(asset: SourceAsset, ranges: MainExclusionRange[]): number | undefined {
  try {
    return retainedDurationMs({ ...asset, mainExclusionRanges: ranges });
  } catch {
    return undefined;
  }
}
...
export function PreviewModal({
  ...
  useEffect(
    () => setIntroInsertRank((current) => Math.max(1, Math.min(current, project.introSegments.length + 1))),
    [project.introSegments.length],
  );
  ...
  const patchRange = (id: string, field: "startMs" | "endMs", valueMs: number) => {
    setRanges((current) =>
      current.map((range) => (range.id === id ? { ...range, [field]: Math.round(valueMs) } : range)),
    );
  };
  ...
  const addRange = () => {
    const startMs = Math.max(
      allowed.inMs,
      Math.min(Math.round(currentMs), Math.max(allowed.inMs, allowed.outMs - 100)),
    );
  ...
  const saveRanges = async (nextRanges = ranges) => {
    ...
    try {
      ...
      setNotice(
        result.mergedRangeCount
          ? `已保存；${result.mergedRangeCount} 個重疊或相鄰區段已自動合併。`
          : "正片排除區段已保存；來源與代理檔均未修改。",
      );
    } catch (reason) {
  ...
  const addZoomSegment = () => {
    ...
    setZoomSegments((current) => [
      ...current,
      {
        ...
        centerXPercent: 50,
        centerYPercent: 50,
        enhancementPreset: DEFAULT_ZOOM_ENHANCEMENT_PRESET,
      },
    ]);
  ...
  const validateIntroRange = (range: { inMs: number; outMs: number }, latestProject: ProjectManifest) => {
    ...
    if (usedMs + lengthMs > latestProject.introTargetDurationMs)
      throw new Error(
        "加入後會超過目前片頭目標時間；為避免改動既有片段與總時間，請先到片頭頁縮短、移除片段或手動增加目標時間。",
      );
  };
  ...
  const saveZoomsWithActions = async () => {
    ...
    try {
      ...
      if (target && renderSavedZoomToMp4)
        setClipRenderSelection(makeManualIntroSegment({ inMs: target.startMs, outMs: target.endMs }));
      if (target && addSavedZoomToIntro)
        setNotice(
          `已保存並把此時段加入片頭第 ${Math.min(introInsertRank, latestProject.introSegments.length)} 順位；既有片段 IN／OUT 與片頭目標時間保持不變，來源影片未修改。`,
        );
    } catch (reason) {
      setEditorError(reason instanceof Error ? reason.message : String(reason));
    }
  ...
  return (
    <>
      ...
      >
        <section
          className="preview-modal expanded-preview-modal"
          role="dialog"
          aria-modal="true"
          aria-label={`預覽 ${asset.fileName}`}
        >
          ...
          <div className="preview-stage">
            {!preview && !previewError && (
              <div className="large-loading">
                ...
                <p>正在建立安全預覽，不會修改來源檔…</p>
              </div>
            )}
            {previewError && (
              <div className="preview-error">
                <strong>無法預覽此檔案</strong>
                <p>{previewError}</p>
                <small>來源仍保留在清單中；可重建代理或使用外部播放器查看唯讀原檔。</small>
                <div className="preview-recovery-actions">
                  ...
                  <button type="button" onClick={() => onOpenExternal(asset)}>
                    用外部播放器查看
                  </button>
                </div>
              </div>
            ...
            <section className="main-exclusion-editor">
              <div className="precision-transport">
                <button
                  type="button"
                  onClick={() => void togglePlayback()}
                  aria-label={playing ? "暫停放大預覽" : "播放放大預覽"}
                >
                  {playing ? "❚❚ 暫停" : "▶ 播放"}
                </button>
                <strong>
                  {formatPlayerTime(currentMs)} / {formatPlayerTime(durationMs)}
                </strong>
                <input
                  aria-label="放大預覽播放頭"
                  type="range"
              ...
              <div className="exclusion-heading">
                <div>
                  <span className="eyebrow">MAIN EXCLUSION RANGES</span>
                  <h3>正片排除區段</h3>
                  <p>只從正片輸出扣除；來源、proxy、metadata 與 Intro 均不變。重疊或相鄰區段保存時會自動排序、合併。</p>
                </div>
                <button type="button" onClick={addRange}>
                  ＋ 新增排除區段
                </button>
              </div>
              <div className="exclusion-timeline" aria-label="正片排除區段時間軸">
                {ranges.map((range) => (
                  <span
                    key={range.id}
              ...
              {ranges.length ? (
                <div className="exclusion-rows">
                  {ranges.map((range, index) => (
                    <div className="exclusion-row" key={range.id}>
                      <strong>區段 {index + 1}</strong>
                      <TimeRangeFields
                        className="inline-time-range"
                        startLabel={`排除 ${index + 1} 開始`}
                        endLabel={`排除 ${index + 1} 結束`}
                        startMs={range.startMs}
                        endMs={range.endMs}
                        maxMs={durationMs}
                        onStartChange={(value) => patchRange(range.id, "startMs", value)}
                        onEndChange={(value) => patchRange(range.id, "endMs", value)}
                      />
                      <button type="button" onClick={() => patchRange(range.id, "startMs", currentMs)}>
                        目前設為開始
                      </button>
                      <button type="button" onClick={() => patchRange(range.id, "endMs", currentMs)}>
                        目前設為結束
                      </button>
                      <button
                        className="range-remove-button"
                        type="button"
                        aria-label={`刪除正片排除區段 ${index + 1}`}
                ...
                <p className="empty-exclusion-note">目前沒有排除區段，完整 IN／OUT 會進入正片。</p>
              )}
              <div className="exclusion-actions">
                <span>{ranges.length ? `編輯中 ${ranges.length} 段；保存後重算正片時間。` : "未排除任何範圍。"}</span>
                <button
                  type="button"
                ...
                <button className="primary-button" type="button" onClick={() => void saveRanges()}>
                  保存排除區段
                </button>
              </div>
              {draftRetainedDurationMs === 0 && (
                <div className="notice error" role="status">
                  這支素材的有效 IN／OUT 已全部排除，將不參與正片輸出；Intro 仍可獨立使用來源片段。
                </div>
              )}
            </section>
          )}
          ...
          {asset.kind === "VIDEO" && durationMs > 0 && (
            <section className="zoom-clip-editor">
              <div className="exclusion-heading">
                <div>
                  <span className="eyebrow">SOURCE RANGE · ZOOM · INTRO · 4K</span>
                  <h3>片段時段／局部放大／畫質優化</h3>
                  <p>
                    原本重複的「固定時間段／片頭素材」已整合至此。放大量 0%
                    代表維持原構圖；仍可把這段加入片頭或從唯讀原檔輸出 4K MP4。區段不可重疊。
                  </p>
                </div>
                <button type="button" onClick={addZoomSegment}>
                  ＋ 新增片段時段
                </button>
              </div>
              {zoomSegments.length ? (
                <div className="zoom-segment-rows">
                  {zoomSegments.map((segment, index) => (
                    <article
                      key={segment.id}
                      className={`${activeZoom?.id === segment.id ? "is-active" : ""} ${selectedZoomId === segment.id ? "is-selected" : ""}`}
                      onClick={() => setSelectedZoomId(segment.id)}
                    >
                      <strong>時段 {index + 1}</strong>
                      <TimeRangeFields
                        className="inline-time-range"
                        startLabel={`片段 ${index + 1} 開始`}
                        endLabel={`片段 ${index + 1} 結束`}
                        startMs={segment.startMs}
                        endMs={segment.endMs}
                        maxMs={durationMs}
                        onStartChange={(value) => patchZoom(segment.id, { startMs: value })}
                        onEndChange={(value) => patchZoom(segment.id, { endMs: value })}
                      />
                      <button type="button" onClick={() => setZoomBoundary(segment.id, "start", currentMs)}>
                        目前設為開始
                      </button>
                      <button type="button" onClick={() => setZoomBoundary(segment.id, "end", currentMs)}>
                        目前設為結束
                      </button>
                      <label>
                        放大量
                        <div className="percent-input">
                          <input
                            aria-label={`局部放大 ${index + 1} 倍數`}
                            type="number"
                            min={MIN_ZOOM_PERCENT - 100}
                            max={MAX_ZOOM_PERCENT - 100}
                            step="1"
                            value={segment.zoomPercent - 100}
                            onChange={(event) =>
                              patchZoom(segment.id, { zoomPercent: Number(event.target.value) + 100 })
                            }
                          />
                          <span>%</span>
                        </div>
                      ...
                      <label>
                        ...
                        <div className="percent-input">
                          <input
                            aria-label={`局部放大 ${index + 1} 中心 X`}
                            type="number"
                            min="0"
                            max="100"
                            step="1"
                            value={segment.centerXPercent}
                            onChange={(event) => patchZoom(segment.id, { centerXPercent: Number(event.target.value) })}
                          />
                          <span>%</span>
                        </div>
                      </label>
                      <label>
                        ...
                        <div className="percent-input">
                          <input
                            aria-label={`局部放大 ${index + 1} 中心 Y`}
                            type="number"
                            min="0"
                            max="100"
                            step="1"
                            value={segment.centerYPercent}
                            onChange={(event) => patchZoom(segment.id, { centerYPercent: Number(event.target.value) })}
                          />
                          <span>%</span>
                        </div>
                      </label>
                      <label className="zoom-enhancement-select">
                        銳利化／消噪
                        <select
                          aria-label={`局部放大 ${index + 1} 畫質優化`}
                          value={segment.enhancementPreset ?? DEFAULT_ZOOM_ENHANCEMENT_PRESET}
                          onChange={(event) =>
                            patchZoom(segment.id, { enhancementPreset: event.target.value as ZoomEnhancementPreset })
                          }
                        >
                          {ZOOM_ENHANCEMENT_OPTIONS.map((option) => (
                        ...
                        <small>
                          {
                            ZOOM_ENHANCEMENT_OPTIONS.find(
                              (option) => option.id === (segment.enhancementPreset ?? DEFAULT_ZOOM_ENHANCEMENT_PRESET),
                            )?.detail
                          }
                        </small>
                      </label>
                      <button
                        className="range-remove-button"
                        type="button"
                        aria-label={`刪除局部放大區段 ${index + 1}`}
                        onClick={() => {
                          setZoomSegments((current) => current.filter((item) => item.id !== segment.id));
                          setSelectedZoomId((current) => (current === segment.id ? undefined : current));
                        }}
                      >
                        刪除時段
                      </button>
                ...
                <p className="empty-exclusion-note">尚未設定片段時段；新增後預設 0% 放大並勾選加入片頭。</p>
              )}
              <div className="zoom-save-panel">
                <div className="zoom-save-options">
                  <label>
                    <input
                      type="checkbox"
                      checked={addSavedZoomToIntro}
                      disabled={!selectedZoomId}
                      onChange={(event) => setAddSavedZoomToIntro(event.target.checked)}
                    />{" "}
                    儲存後直接加入片頭（預設勾選）
                  </label>
                  {addSavedZoomToIntro && (
                    <label>
                      ...
                      <select
                        aria-label="加入片頭順位"
                        value={introInsertRank}
                        onChange={(event) => setIntroInsertRank(Number(event.target.value))}
                      >
                        {Array.from(
                          { length: Math.min(INTRO_MAX_SEGMENTS, project.introSegments.length + 1) },
                          (_, index) => (
                  ...
                  <label>
                    <input
                      type="checkbox"
                      checked={renderSavedZoomToMp4}
                      disabled={!selectedZoomId}
                      onChange={(event) => setRenderSavedZoomToMp4(event.target.checked)}
                    />{" "}
                    儲存後開啟此時段的 4K MP4 輸出
                  </label>
                </div>
                <div className="exclusion-actions">
                  <span>
                    {selectedZoomId
                      ? `已選擇片段時段 ${zoomSegments.findIndex((item) => item.id === selectedZoomId) + 1}；勾選項會在保存成功後執行。`
                      : activeZoom
                        ? `目前預覽：放大 ${activeZoom.zoomPercent - 100}% · 中心 ${activeZoom.centerXPercent}% / ${activeZoom.centerYPercent}%`
                        : "請新增或點選一個片段時段。"}
                  </span>
                  <button
                    type="button"
                    disabled={!zoomSegments.length}
                    onClick={() => {
                      setZoomSegments([]);
                  ...
                  <button className="primary-button" type="button" onClick={() => void saveZoomsWithActions()}>
                    保存片段設定
                  </button>
                </div>
              </div>
          ...
          <footer className="modal-details">
            <span>
              {asset.kind === "VIDEO"
                ? formatDuration(asset.mediaInfo?.durationMs)
                : `照片 · ${(asset.imageDurationMs ?? 5_000) / 1000} 秒`}
            </span>
            ...
            {asset.kind === "VIDEO" && (
              <button className="modal-external-button" type="button" onClick={() => onOpenExternal(asset)}>
                ↗ 外部開啟
              </button>
            )}
      ...
      {clipRenderSelection && (
        <ConcatRenderModal
          ...
          mainClips={[clipRenderSelection]}
          initialResolution="4K"
          onClose={() => setClipRenderSelection(undefined)}
        />
      )}
      {introAddedRank !== undefined && (
        <div className="modal-backdrop intro-added-backdrop" role="presentation">
          <section className="intro-added-confirmation" role="dialog" aria-modal="true" aria-label="片頭片段加入成功">
            <span className="eyebrow">INTRO SAVED</span>
            <h2>已加入片頭第 {introAddedRank} 順位</h2>
            <p>
              片段已寫入目前專案；既有片段的 IN／OUT 與片頭目標時間 {formatDuration(introAddedTargetDurationMs)}{" "}
              都保持不變。要現在前往片頭頁確認播放、順序與內容嗎？
            </p>
            <div>
              <SafeDefaultButton
                ...
                type="button"
                onClick={() => {
                  setIntroAddedRank(undefined);
                  setIntroAddedTargetDurationMs(undefined);
                }}
              >
                留在片段設定
              </SafeDefaultButton>
