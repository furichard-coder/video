...
function WatermarkPreview({ settings }: { settings: WatermarkSettings }) {
  ...
  return (
    <div className="watermark-preview" aria-label="16:9 浮水印位置預覽">
      <div className="watermark-preview-sky" />
        ...
        <div className="watermark-settings-body">
          ...
          <section className="watermark-reference-card">
            ...
            <button
              ...
              disabled={busy}
              onClick={() => {
                setDraft(structuredClone(DEFAULT_WATERMARK_SETTINGS));
                setNotice("已載入草漯沙丘預設；按保存後才會寫入專案。");
              }}
            >
              套用草漯沙丘預設
            </button>
          </section>
          <fieldset className="watermark-enable">
            <legend>使用範圍</legend>
            <label>
              <input
                type="checkbox"
            ...
            <label>
              <input
                type="checkbox"
                checked={draft.applyToMain}
                disabled={!draft.enabled}
                onChange={(event) => patch("applyToMain", event.target.checked)}
              />{" "}
              正片完成輸出（含已串接片頭）
            </label>
            <label>
          ...
          <div className="watermark-text-grid">
            {(["chinese", "english"] as const).map((key) => {
              const item = draft[key];
              const label = key === "chinese" ? "中文浮水印" : "英文浮水印";
              return (
                <fieldset key={key}>
                  <legend>{label}</legend>
                  <label>
                  ...
                  <div>
                    <label>
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          patchText(key, {
                            position: event.target.value as WatermarkCorner,
                          })
                        }
                      >
                        <option value="LOWER_LEFT">左下角</option>
                        <option value="LOWER_RIGHT">右下角</option>
                      </select>
                    </label>
                    <label>
                      排列方式
                      <select
                        aria-label={`${label}排列方式`}
                        value={item.layout}
                        onChange={(event) =>
                          patchText(key, {
                            layout: event.target.value as WatermarkTextLayout,
                          })
                        }
                      >
                        <option value="STACKED_TWO_LINES">上下兩行</option>
                        <option value="SINGLE_LINE">單行橫排</option>
                      </select>
                    </label>
                    <label>
                      文字大小（1080p）
                      <span className="number-with-unit">
                        <input
                          aria-label={`${label}文字大小`}
                          type="number"
                          min="12"
                          max="180"
                          value={item.fontSize1080p}
                          onChange={(event) =>
                            patchText(key, {
                              fontSize1080p: Number(event.target.value),
                            })
                          }
                        />
                        <em>px</em>
          ...
          <fieldset className="watermark-schedule">
            <legend>顯示排程</legend>
            <label>
              首次顯示
              <input
                aria-label="浮水印首次顯示秒數"
                type="number"
                min="0"
                max="86400"
                step="1"
          ...
          <fieldset className="watermark-appearance">
            <legend>外觀與安全邊距</legend>
            <label>
              文字不透明度
              <input
                aria-label="浮水印文字不透明度"
                type="range"
                min="0"
                max="100"
          ...
          {notice && (
            <p className="watermark-notice" role="status">
              {notice}
            </p>
          )}
        </div>
