...
function preferredMimeType(): string {
  ...
  return (
    ["audio/webm;codecs=opus", "audio/webm", "audio/ogg;codecs=opus", "audio/mp4"].find((value) =>
...
export function VoiceInputButton({ language, disabled, onTranscript, onError }: Props) {
    ...
    try {
      ...
      recorder.addEventListener(
        ...
        async () => {
          ...
          try {
            ...
            const result = await window.sourceApp.transcribeVoiceInput({
      ...
      onError(
        name === "NotAllowedError"
          ? "Windows 或 App 尚未允許使用麥克風；請在隱私權設定開啟權限後重試。"
  ...
  return (
    ...
    >
      {recognizing ? "辨識中…" : recording ? "■ 停止並轉文字" : "🎙 語音輸入"}
    </button>
  );
}
