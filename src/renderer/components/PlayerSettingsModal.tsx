...
export function PlayerSettingsModal({ onClose }: PlayerSettingsModalProps) {
  ...
  const extensions = useMemo(
    () => [...new Set([...STARTER_EXTENSIONS, ...Object.keys(overrides)])].sort(),
    [overrides],
  );
  ...
  const save = async () => {
    ...
    setError(undefined);
    try {
      const next = await window.sourceApp.updateExternalPlayerSettings({
        defaultPlayerId,
    ...
    >
      <section className="player-settings-modal" role="dialog" aria-modal="true" aria-label="外部播放器設定">
        <header className="modal-header">
          <div>
            <span className="eyebrow">SETTINGS · EXTERNAL PLAYBACK</span>
            <h2>外部播放器設定</h2>
          </div>
        ...
        {!snapshot ? (
          ...
          <div className="player-settings-body">
            <section className="settings-section">
              ...
              <select
                aria-label="全域預設播放器"
                value={defaultPlayerId}
                onChange={(event) => setDefaultPlayerId(event.target.value as ExternalPlayerId)}
              >
                {selectOptions()}
              </select>
            </section>
              ...
              <div className="extension-routes">
                {extensions.map((extension) => (
                  <div className="extension-route" key={extension}>
                    ...
                    <select
                      aria-label={`${extension} 播放器`}
                      value={overrides[extension] ?? ""}
                      onChange={(event) => {
                    ...
                    {!STARTER_EXTENSIONS.includes(extension) && (
                      <button
                        type="button"
                        onClick={() =>
                          setOverrides((current) => {
            ...
            <section className="settings-section detected-section">
              <div>
                <h3>播放器偵測</h3>
                <p>安裝路徑每次使用前都會再驗證，不把下列位置視為永久事實。</p>
              </div>
              <div className="player-detection-list">
                {playerOptions.map((player) => (
                  <div
                    className={`player-detection ${player.available ? "is-available" : "is-missing"}`}
                    key={player.id}
                  >
                    <span>{player.available ? "●" : "○"}</span>
                    <div>
                      <strong>{player.label}</strong>
                      <small title={player.executablePath}>{player.executablePath ?? player.detail}</small>
                    </div>
                    {player.kind === "CUSTOM" && (
                      <button type="button" disabled={busy} onClick={() => void removeCustom(player.id)}>
                        移除
                      </button>
                    )}
                  </div>
                ...
                <button
                  className="browse-player-button"
                  type="button"
                  disabled={busy}
        ...
        <footer className="settings-footer">
          ...
          <div>
            ...
            <button className="primary-button" type="button" disabled={!snapshot || busy} onClick={() => void save()}>
              儲存設定
            </button>
          </div>
        </footer>
