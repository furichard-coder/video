...
export function useOutputLibrary(refreshKey = 0): OutputLibraryState {
  ...
  const refresh = useCallback(async () => {
    ...
    try { setOutputs((await window.sourceApp.getPreviewOutputHistory()).outputs); }
...
export function outputPurposeMatches(output: PreviewOutputRecord, purpose?: PreviewOutputPurpose | PreviewOutputPurpose[]) {
...
export function OutputRecordActions({ output, onError, onCopySuccess, onYoutube, onPlatform, showPublish = false }: OutputRecordActionsProps) {
  ...
  const run = async (action: "PLAY" | "REVEAL" | "COPY" | BrowserUploadPlatform) => {
    ...
    try {
      ...
      else if (action === "COPY") { await window.sourceApp.copyPreviewOutputPath(output.jobId); onCopySuccess?.(); }
      else if (onPlatform) await onPlatform(action);
    } catch (reason) { onError?.(reason instanceof Error ? reason.message : String(reason)); }
    finally { setBusy(false); }
  };
  const disabled = !output.exists || busy;
  return <div className="output-record-actions">
...
interface Props { purpose?: PreviewOutputPurpose | PreviewOutputPurpose[]; title: string; refreshKey?: number; recentOnly?: boolean; emptyText?: string; }

/** Shared compact output query/actions used by Intro, Subtitle, Concat and output pages. */
export function OutputLibrary({ purpose, title, refreshKey = 0, recentOnly = true, emptyText = "尚無仍存在的預覽檔。" }: Props) {
  const { outputs, error, setError } = useOutputLibrary(refreshKey);
  const visible = outputs.filter((item) => item.exists && outputPurposeMatches(item, purpose)).sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, recentOnly ? 3 : undefined);
  return <section className="output-library-compact" aria-label={title}><header><h3>{title}</h3><small>播放、開啟位置與複製路徑使用同一成品庫。</small></header>{!visible.length ? <p className="inline-history-empty">{emptyText}</p> : <ol>{visible.map((output) => <li key={output.jobId}><div><button type="button" title={output.outputPath} onClick={() => void window.sourceApp.playConcatOutput(output.jobId).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : String(reason)))}>{output.fileName}</button><small>{new Date(output.createdAt).toLocaleString("zh-TW")} · {formatBytes(output.sizeBytes)}{output.resolution ? ` · ${output.resolution}` : ""}</small></div><OutputRecordActions output={output} onError={setError} /></li>)}</ol>}{error && <p className="inline-error" role="alert">{error}</p>}</section>;
}
