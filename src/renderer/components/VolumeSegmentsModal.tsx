...
export function VolumeSegmentsModal({ asset, defaultVolumePercent, onClose, onAssetUpdated }: Props) {
  ...
  const updateVolume = (id: string, value: number) =>
    setSegments((current) => current.map((item) => (item.id === id ? { ...item, volumePercent: value } : item)));
  const add = () => {
  ...
  const save = async () => {
    ...
    setNotice(undefined);
    try {
      const updated = await window.sourceApp.setVolumeSegments(asset.id, segments);
      onAssetUpdated(updated);
      setSegments(updated.volumeSegments ?? []);
  ...
  return (
    <div className="modal-backdrop volume-backdrop" role="presentation">
      <section
        className="editor-modal volume-modal"
        role="dialog"
        aria-modal="true"
        aria-label={`音量區段 ${asset.fileName}`}
      >
        <header className="modal-header">
          <div>
            <span className="eyebrow">CLIP AUDIO AUTOMATION</span>
            <h2>音量區段</h2>
        ...
        <div className="editor-modal-body">
          <div className="range-summary">
            <span>目前可用 IN／OUT</span>
            <strong>
              {formatDuration(range.inMs)} → {formatDuration(range.outMs)}
            </strong>
            <small>未被任何區段覆蓋的部分使用素材原音預設 {defaultVolumePercent}%。區段不可重疊。</small>
          </div>
          <div className="table-head volume-grid">
            <span>開始（分鐘:秒數）</span>
            <span>結束（分鐘:秒數）</span>
            <span>音量</span>
            <span />
          ...
          <div className="editable-rows">
            {segments.map((segment) => (
              <div className="editable-row volume-grid" key={segment.id}>
                <TimeRangeFields
                  ...
                  onStartChange={(value) =>
                    setSegments((current) =>
                      current.map((item) => (item.id === segment.id ? { ...item, startMs: value } : item)),
                    )
                  }
                  onEndChange={(value) =>
                    setSegments((current) =>
                      current.map((item) => (item.id === segment.id ? { ...item, endMs: value } : item)),
                    )
                  }
                />
                <label className="percent-input">
                  <input
                    ...
                    step="1"
                    value={segment.volumePercent}
                    onChange={(event) => updateVolume(segment.id, Number(event.target.value))}
                  />
                  <span>%</span>
                </label>
                <button
          ...
          <button className="add-row-button" type="button" onClick={add}>
            ＋ 新增音量區段
          </button>
          <p className="mix-note">
            300% 是數位增益上限，不代表聽感三倍；最終輸出另套用 peak limiter 防止超過安全峰值。
          </p>
