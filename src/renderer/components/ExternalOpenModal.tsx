...
export function ExternalOpenModal({ asset, onClose }: ExternalOpenModalProps) {
  ...
  return (
    <div
      className="modal-backdrop external-open-backdrop"
      role="presentation"
      onMouseDown={(event) => event.target === event.currentTarget && onClose()}
    >
      <section
        className="external-open-modal"
        role="dialog"
        aria-modal="true"
        aria-label={`以外部播放器開啟 ${asset.fileName}`}
      >
        <header className="modal-header">
          <div>
            <span className="eyebrow">EXTERNAL VIEW ONLY</span>
            <h2>以外部播放器開啟</h2>
          </div>
        ...
        <div className="external-open-body">
          <div className="external-file">
            ...
            <span>
              {asset.extension.toLowerCase()} → {selectedPlayer?.label ?? "正在讀取設定…"}
            </span>
          </div>
          <fieldset className="external-targets">
            ...
            <label className={target === "PROXY" ? "is-selected" : ""}>
              <input
                ...
                value="PROXY"
                checked={target === "PROXY"}
                onChange={() => setTarget("PROXY")}
              />
              <span>
                <strong>App cache proxy（建議）</strong>
                <small>衍生的低解析預覽，不把原始來源交給外部程式。</small>
              </span>
            </label>
            <label className={target === "ORIGINAL" ? "is-selected warning" : "warning"}>
              ...
              />
              <span>
                <strong>唯讀意圖開啟原檔</strong>
                <small>App 不會修改來源；但來源保護仍取決於外部播放器自身行為。</small>
              </span>
            </label>
          </fieldset>
          <p className="external-security-note">
            播放器與目標檔會在 Main process 重新驗證，並以獨立參數啟動，不執行媒體路徑中的任何命令文字。
          </p>
          {result?.message && (
            <div className={`notice ${result.status === "USE_INTERNAL" ? "error" : "success"}`} role="status">
              {result.message}
            </div>
          )}
          {result && !result.message && (
            <div className="notice success" role="status">
              已交給 {result.playerLabel} 開啟。
            </div>
          )}
          {error && (
        ...
        <footer className="settings-footer">
          ...
          <div>
            ...
            <button className="primary-button" type="button" disabled={busy || !snapshot} onClick={() => void open()}>
              {busy ? (target === "PROXY" ? "正在準備 proxy…" : "正在開啟…") : "開啟觀看"}
            </button>
          </div>
        </footer>
      </section>
