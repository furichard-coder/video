...
import type { BrowserUploadPlatform, PreviewOutputHistorySnapshot, PreviewOutputRecord } from "../../shared/domain";
...
export function OutputHistoryModal({ onClose, onOpenYoutubeSettings }: Props) {
  const [history, setHistory] = useState<PreviewOutputHistorySnapshot>();
  ...
  const refresh = async () => {
    ...
    try { setHistory(await window.sourceApp.getPreviewOutputHistory()); }
  ...
  const runFileAction = async (output: PreviewOutputRecord, action: "PLAY" | "REVEAL" | "COPY") => {
  ...
  const removeRecord = async (jobId: string) => {
    ...
    try { setHistory(await window.sourceApp.removePreviewOutputRecord(jobId)); setNotice("已從歷史清單移除紀錄；磁碟上的 MP4 沒有刪除或修改。"); }
    catch (reason) { setError(reason instanceof Error ? reason.message : String(reason)); }
  ...
  return <div className="modal-backdrop output-history-backdrop" role="presentation">
    <section className="output-history-modal" role="dialog" aria-modal="true" aria-label="預覽成品紀錄">
      ...
      <div className="output-history-body">
        {loading && !history ? <div className="loading-shell"><span className="spinner" /><p>正在讀取預覽成品紀錄…</p></div> : history?.outputs.length ? <div className="output-history-list">{history.outputs.map((output) => {
          ...
          return <article key={output.jobId} className={!output.exists ? "is-offline" : ""}>
            ...
            <div className="output-history-actions"><button type="button" disabled={!output.exists || busyId === output.jobId} onClick={() => void runFileAction(output, "PLAY")}>▶ 播放</button><button type="button" disabled={!output.exists || busyId === output.jobId} onClick={() => void runFileAction(output, "REVEAL")}>在檔案總管顯示</button><button type="button" disabled={!output.exists || busyId === output.jobId} onClick={() => void runFileAction(output, "COPY")}>複製路徑</button>{canPublish && <button className="youtube-connect-button" type="button" disabled={!output.exists || busyId === output.jobId} onClick={() => setYoutubeOutput(output)}>上傳 YouTube</button>}{canPublish && <button className="bilibili-upload-button" type="button" disabled={!output.exists || busyId === output.jobId} onClick={() => void handoff(output, "BILIBILI")}>BiliBili</button>}{canPublish && <button className="tiktok-upload-button" type="button" disabled={!output.exists || busyId === output.jobId} onClick={() => void handoff(output, "TIKTOK")}>TikTok</button>}</div>
