...
function parsedAllowedUrl(raw: string): URL | undefined {
  try {
    ...
    if (YOUTUBE_HOSTS.has(host)) {
      const valid =
        host === "youtu.be" ? url.pathname.length > 1 : url.pathname === "/watch" && Boolean(url.searchParams.get("v"));
      return valid ? url : undefined;
    }
    if (TIKTOK_HOSTS.has(host)) {
      const valid =
        host === "ads.tiktok.com"
          ? url.pathname.toLowerCase().includes("creativecenter")
          : url.pathname.toLowerCase().includes("/music/");
      return valid ? url : undefined;
    }
  } catch {
    /* invalid URL */
...
function normalizeCandidates(
  candidates: MusicSuggestionCandidate[],
  includeTikTok: boolean,
  royaltyFreeOnly: boolean,
): { suggestions: MusicSuggestion[]; discarded: number } {
  ...
  const suggestions = candidates.flatMap((candidate): MusicSuggestion[] => {
    ...
    if (
      (!includeTikTok && candidate.platform === "TIKTOK") ||
      (royaltyFreeOnly && !rightsEvidence) ||
      !isAllowedMusicSuggestionUrl(candidate.auditionUrl) ||
      !isAllowedMusicSuggestionUrl(candidate.evidenceUrl)
    ) {
      discarded += 1;
      return [];
    }
    ...
    return [
      {
        ...
        artist: candidate.artist.trim() || undefined,
        reason: candidate.reason.trim(),
        auditionUrl,
        evidenceUrl,
        observedAt: candidate.observedAt.trim() || undefined,
        trendEvidence: candidate.trendEvidence.trim() || undefined,
        rightsEvidence: rightsEvidence || undefined,
...
export class MusicSuggestionService {
  ...
  async generate(request: MusicSuggestionRequest, signal?: AbortSignal): Promise<MusicSuggestionResult> {
    if (!request || typeof request.includeTikTokTrending !== "boolean" || typeof request.royaltyFreeOnly !== "boolean")
      throw new Error("AI 配樂建議設定無效。");
    ...
    let openAiFailure = "";
    try {
      raw = await this.openAi.suggestMusic(
        context,
        request.includeTikTokTrending,
        request.royaltyFreeOnly,
        this.aiSettings.getRuntimeAccount(),
        signal,
      );
    } catch (error) {
      ...
      raw = await this.codex.suggestMusic(
        context,
        request.includeTikTokTrending,
        request.royaltyFreeOnly,
        profile.visionModel,
    ...
    if (youtubeCount < 3)
      throw new Error(
        "AI 沒有找到至少 3 個可核對的 YouTube 試聽連結；本次結果未保存，請稍後再試或補充更明確的影片主題。",
      );
    const warnings = [
      ...
      ...(request.includeTikTokTrending && !normalized.suggestions.some((item) => item.platform === "TIKTOK")
        ? ["本次沒有找到可核對的 TikTok／抖音近期熱門項目；未以舊榜單冒充即時結果。"]
        : []),
      request.royaltyFreeOnly
        ? "本次已套用『只找有授權線索』條件；這仍不等於法律上的無版權，請逐首查看原始授權條款。"
        : "所有建議只供試聽與選曲；上傳、商用與影音同步授權須由使用者逐首確認。",
    ];
    return {
      provider,
      accountName,
      ...
      royaltyFreeOnly: request.royaltyFreeOnly,
  ...
  async open(url: string): Promise<void> {
    ...
    const error = await this.openExternal(allowed.toString()).then(
      () => "",
