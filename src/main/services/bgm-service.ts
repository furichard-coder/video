...
export class BgmService {
    ...
    for (const selected of paths) {
      ...
      try {
        ...
        tracks.push({
          ...
          timelineOutMs: timelineCursor + info.durationMs,
          fadeInMs: Math.min(1000, Math.floor(info.durationMs / 4)),
          fadeOutMs: Math.min(1000, Math.floor(info.durationMs / 4)),
          volumePercent: DEFAULT_BGM_VOLUME_PERCENT,
  ...
  async addYoutubeReferences(urls: string[]): Promise<BgmImportResult> {
    ...
    const existing = new Set(
      this.store
        .getProject()
        .bgmTracks.map((track) => track.sourceUrl)
        .filter(Boolean),
    );
    for (const raw of urls) {
      ...
      try {
        ...
        if (
          ...
          throw new Error("只接受 HTTPS YouTube／youtu.be 連結。");
        const pathParts = url.pathname.split("/").filter(Boolean);
        const videoId =
          host === "youtu.be"
            ? pathParts[0]
            : (url.searchParams.get("v") ??
              (["shorts", "embed", "live"].includes(pathParts[0] ?? "") ? pathParts[1] : undefined));
        ...
        errors.push(`${candidate.slice(0, 120)}：${error instanceof Error ? error.message : String(error)}`);
      }
    }
    const project = tracks.length ? await this.store.addBgmTracks(tracks) : this.store.getProject();
    return { cancelled: false, addedCount: tracks.length, errors, project };
  ...
  async resolveReference(
    ...
    if (!current || current.resolutionStatus !== "NEEDS_LOCAL_FILE")
      throw new Error("找不到待補音檔的 YouTube 配樂參考。");
  ...
  private async inspectLocalTrack(
    selected: string,
    id: string,
    sourceUrl?: string,
    signal?: AbortSignal,
