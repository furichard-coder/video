...
export function selectPublishVisualCandidates(
  project: ReturnType<ProjectStore["getProject"]>,
): PublishVisualSelection[] {
  ...
  const add = (assetId: string, sourceTimeMs: number, origin: "INTRO" | "MAIN") => {
    ...
    if (selected.some((item) => item.assetId === assetId && Math.abs(item.sourceTimeMs - normalizedTimeMs) < 500))
      return;
  ...
  const ranges = project.introSegments.length
    ? project.introSegments.map((item) => ({
        assetId: item.assetId,
        inMs: item.inMs,
        outMs: item.outMs,
        origin: "INTRO" as const,
      }))
    : plan.clips.map((item) => ({
        assetId: item.assetId,
        inMs: item.inMs,
  ...
  for (let index = 0; selected.length > 0 && selected.length < 3; index += 1)
    selected.push({ ...selected[index % uniqueCount] });
...
export class AiPublishAssetsService {
  constructor(
    ...
    private readonly codex = new CodexCliStoryProvider(path.join(previews.cacheRoot, "codex-publish")),
    private readonly gemini = new GeminiPublishReviewProvider(),
  ) {}

  private async applyGeminiReview(
    ...
    if (!settings.enabled) {
      assets.geminiReview = {
        ...
        summary: "Gemini 輔助復核未啟用；本次由 ChatGPT 主生成。",
        warnings: [],
      };
      return;
    }
    if (!settings.apiKey) {
      assets.geminiReview = {
        ...
        status: "NOT_CONFIGURED",
        model: settings.model,
        summary: "已啟用 Gemini 輔助，但尚未設定可用的 Gemini API Key。",
        warnings: ["本次沒有執行 Gemini 復核。"],
      };
      return;
    }
    try {
      assets.geminiReview = await this.gemini.review(
        assets,
        input,
      ...
      if (assets.geminiReview.recommendedThumbnailId)
        assets.selectedThumbnailId = assets.geminiReview.recommendedThumbnailId;
    } catch (error) {
      ...
      assets.geminiReview = {
        ...
        status: "FAILED",
        model: settings.model,
        summary: "ChatGPT 主生成已保留；Gemini 第二次復核未完成。",
        warnings: [message],
      };
  ...
  private async materializeThumbnailPreviews(
    assets: AiPublishAssets,
    ...
    for (const [index, candidate] of assets.thumbnails.entries()) {
      ...
      if (!asset) {
        assets.warnings.push(`縮圖候選來源已不存在：${candidate.sourceFileName}`);
        continue;
      }
      const cacheKey = createHash("sha256")
        .update(JSON.stringify({ analyzer: PUBLISH_ANALYZER_VERSION, source: asset.previewCacheKey, candidate }))
        .digest("hex");
      ...
      try {
        ...
        assets.warnings.push(
          `縮圖候選 ${index + 1} 無法合成：${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
  ...
  async generate(
    options: AiPublishGenerationOptions,
    signal?: AbortSignal,
    onProgress: (percent: number, detail: string) => void = () => undefined,
  ): Promise<AiPublishGenerationResult> {
    const project = this.store.getProject();
    const topic = options.topic;
    if (!topic.topic.trim() && !topic.storySummary.trim() && !topic.locations.length)
      throw new Error("請先輸入主題、地點或故事摘要，再產生發布素材。");
    if (signal?.aborted) throw new DOMException("AI 發布素材分析已取消。", "AbortError");
    ...
    const titles: PublishTitleCandidate[] = rawTitles.map((text, index) => {
      ...
      return {
        ...
        reason: index === 0 ? "主題與地點優先" : "依觀眾承諾與故事摘要產生的候選",
      };
    });
    const plan = buildTimelinePlan(project);
    const assetById = new Map(project.sources.map((asset) => [asset.id, asset]));
    ...
    for (const [index, selection] of visualSelections.entries()) {
      ...
      try {
        previewUrl = (await this.previews.ensure(asset.id, "THUMBNAIL", signal)).url;
      } catch (error) {
        warnings.push(
          `縮圖候選 ${asset.fileName} 無法建立預覽：${error instanceof Error ? error.message : String(error)}`,
        );
      }
      thumbnails.push({
        ...
        reason: `${selection.origin === "INTRO" ? "已選片頭" : "正片時間線"}畫面；只使用來源內容，不猜測真實身分。`,
        layout: index % 2 ? "RIGHT_TEXT" : "LEFT_TEXT",
        colorNote: "保留來源比例，使用輕微亮暗遮罩",
        previewUrl,
        style: {
          text: subject.slice(0, 18),
          textXPercent: index % 2 ? 66 : 28,
          textYPercent: 78,
          fontSizePx: 64,
          textColor: "#FFFFFF",
          outlineWidthPx: 3,
    ...
    for (const selection of visualSelections) {
      ...
      const frameKey = createHash("sha256")
        .update(`${PUBLISH_ANALYZER_VERSION}\0${asset.previewCacheKey}\0${selection.sourceTimeMs}`)
        .digest("hex");
      const framePath = path.join(analysisFrameDirectory, `${frameKey}.jpg`);
      try {
        await renderThumbnail(
          asset,
          { ...candidate, style: { ...candidate.style, text: "", overlayOpacityPercent: 0 } },
          framePath,
          "jpg",
        );
        ...
        warnings.push(
          `AI 候選影格無法建立：${asset.fileName}；${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    const timelineDurationMs = plan.durationMs;
    const generationInput: PublishGenerationInput = {
      topic: {
      ...
      introSummary: project.introSegments.map((segment, order) => ({
        assetId: segment.assetId,
        fileName: assetById.get(segment.assetId)?.fileName ?? segment.fileName,
        sourceInMs: segment.inMs,
        sourceOutMs: segment.outMs,
        order: order + 1,
      })),
      timelineSummary: plan.clips.map((clip) => ({
        assetId: clip.assetId,
        fileName: assetById.get(clip.assetId)?.fileName ?? clip.assetId,
        startMs: clip.outputStartMs,
        endMs: clip.outputStartMs + clip.outMs - clip.inMs,
      })),
      candidates,
    };
    try {
      const model =
        snapshot.accounts.find((item) => item.id === snapshot.activeAccountId)?.visionModel ?? "gpt-5.6-terra";
      onProgress(55, "正在由 Codex／ChatGPT 分析片頭畫面並主生成標題、縮圖與說明…");
      const generated = await this.codex.generatePublishAssets(generationInput, model, signal);
      const generatedAt = new Date().toISOString();
      const generatedAssets = applyGeneratedDraft(project.aiPublishAssets, generated.draft, {
        topicSnapshot: { ...topic, capturedAt: generatedAt },
        provider: "CODEX_CHATGPT",
        model: generated.model,
        analyzerVersion: PUBLISH_ANALYZER_VERSION,
        generatedAt,
        mainTimelineRevision: project.mainTimelineRevision ?? project.timelineRevision,
        introTimelineRevision: project.introTimelineRevision,
      });
      for (const thumbnail of generatedAssets.thumbnails)
        thumbnail.previewUrl = thumbnails.find(
          (item) => item.assetId === thumbnail.assetId && item.sourceTimeMs === thumbnail.sourceTimeMs,
        )?.previewUrl;
      await this.materializeThumbnailPreviews(generatedAssets, signal, onProgress);
      onProgress(97, "正在執行可選的 Gemini 第二次復核…");
      await this.applyGeminiReview(generatedAssets, generationInput, signal);
      const latest = this.store.getProject();
      if (
        latest.id !== project.id ||
        (latest.mainTimelineRevision ?? latest.timelineRevision) !==
          (project.mainTimelineRevision ?? project.timelineRevision)
      )
        throw new Error("專案或正片時間線在 AI 生成期間已變更，結果未套用，請重新產生。");
      const updated = await this.store.setAiPublishAssets(generatedAssets);
      onProgress(100, "ChatGPT 主生成發布素材已保存");
      return { project: updated, assets: generatedAssets, provider: "CODEX_CHATGPT" as const, model: generated.model };
    } catch (codexError) {
      if (signal?.aborted) throw new DOMException("AI 發布素材分析已取消。", "AbortError");
      warnings.push(
        `Codex／ChatGPT 主生成失敗：${codexError instanceof Error ? codexError.message : String(codexError)}`,
      );
      try {
        const account = this.aiSettings.getRuntimeAccount();
        onProgress(65, "ChatGPT 登入模式不可用，改用 OpenAI API 主生成…");
        const generated = await this.openAi.generatePublishAssets(generationInput, account, signal);
        const generatedAt = new Date().toISOString();
        const generatedAssets = applyGeneratedDraft(project.aiPublishAssets, generated.draft, {
          topicSnapshot: { ...topic, capturedAt: generatedAt },
          provider: "OPENAI_API",
        ...
        if (
          ...
          throw new Error("專案或正片時間線在 AI 生成期間已變更，結果未套用，請重新產生。");
        const updated = await this.store.setAiPublishAssets(generatedAssets);
        onProgress(100, "OpenAI API 備援發布素材已保存");
        return { project: updated, assets: generatedAssets, provider: "OPENAI_API" as const, model: generated.model };
      } catch (openAiError) {
        if (signal?.aborted) throw new DOMException("AI 發布素材分析已取消。", "AbortError");
        warnings.push(
          `OpenAI API 備援生成失敗：${openAiError instanceof Error ? openAiError.message : String(openAiError)}`,
        );
      }
    }
    onProgress(80, "正在依 transition-aware timeline 建立離線 fallback 草稿…");
    const draftChapters: PublishChapterCue[] = plan.clips
      .map((clip, index) => ({
        id: randomUUID(),
        startMs: clip.outputStartMs,
        title: clipLabel(assetById.get(clip.assetId)?.fileName ?? clip.assetId, index),
        description: `來源：${assetById.get(clip.assetId)?.fileName ?? clip.assetId}`,
        sourceAssetId: clip.assetId,
      }))
      .filter((chapter, index, all) => index === 0 || chapter.startMs - all[index - 1].startMs >= 10_000);
    const chapters = isValidYoutubeChapterSet(draftChapters, timelineDurationMs) ? draftChapters : [];
    if (!chapters.length)
      warnings.push("目前正片太短或章節數不足 3 段；章節草稿暫不啟用，請調整時間後再重跑或手動新增。");
    const generatedAt = new Date().toISOString();
    const assets: AiPublishAssets = {
      ...
      titles,
      description: `${topic.storySummary.trim() || `${subject}的旅程紀錄。`}\n\n${topic.audiencePromise.trim() || "用畫面認識這段旅程的特色與故事。"}`,
      englishSummary: `A visual journey through ${subject}.`,
      hashtags: ["#旅遊", `#${subject.replace(/\s+/g, "")}`].slice(0, 5),
      thumbnails,
      chapters,
