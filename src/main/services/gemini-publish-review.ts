...
export class GeminiPublishReviewProvider {
  ...
  async review(
    ...
    const imageParts = await Promise.all(input.candidates.slice(0, 3).map(async (candidate) => ({ inlineData: { mimeType: "image/jpeg", data: (await readFile(candidate.framePath)).toString("base64") } })));
    ...
    try { parsed = JSON.parse(text) as Record<string, unknown>; } catch { throw new Error("Gemini 輔助復核格式無法解析。"); }
    const recommendedTitleId = titleIds.includes(String(parsed.recommendedTitleId)) ? String(parsed.recommendedTitleId) : undefined;
    const recommendedThumbnailId = thumbnailIds.includes(String(parsed.recommendedThumbnailId)) ? String(parsed.recommendedThumbnailId) : undefined;
    return {
      enabled: true,
      status: "REVIEWED",
