...
interface CodexBatchItem extends StoryFrameAnalysis {
  index: number;
}
...
function normalizeMusic(value: unknown): MusicSuggestionCandidate[] {
  ...
  return value.flatMap((entry): MusicSuggestionCandidate[] => {
    ...
    if (
      ["title", "reason", "auditionUrl", "evidenceUrl"].some(
...
export class CodexCliStoryProvider {
  ...
  isInstalled(): boolean {
    return Boolean(this.configuredExecutable);
  }
  ...
  async testConnection(signal?: AbortSignal): Promise<CodexConnectionDiagnostic> {
    ...
    let result;
    try {
      result = await this.processRunner(executable, ["login", "status"], combinedSignal(signal));
    } catch (error) {
      throw new Error(`Codex 登入狀態檢查失敗：${error instanceof Error ? error.message : String(error)}`);
    }
    const status = `${result.stdout}\n${result.stderr}`;
    if (!/logged in using chatgpt/i.test(status))
      throw new Error("Codex 尚未使用 ChatGPT 登入；請先在 Codex 完成登入後重試。");
  ...
  async generatePublishAssets(
    ...
    model: string,
    signal?: AbortSignal,
  ): Promise<{ draft: PublishGeneratedDraft; model: string }> {
    const executable = this.configuredExecutable;
    ...
    const prompt = [
      "你是旅遊 YouTube 發布素材編輯。先理解主題、故事與正片內容，再優先判讀已選片頭的實際候選畫面，讓標題與縮圖呈現影片開場承諾。不要執行工具、不要讀寫本機檔案；只依照文字與附加低解析影格回答。不得猜測人物身分或未提供的地點。",
      JSON.stringify({
        topic: input.topic,
      ...
      "標題須延伸影片內容與片頭畫面；嚴格依 output schema 回傳完整 titles、description、englishSummary、hashtags、3 個可追溯縮圖概念與章節。縮圖概念應優先選用 origin=INTRO 的候選，除非該畫面明顯不適合。",
    ...
    try {
      ...
      const args = [
        ...
        this.cacheRoot,
        "--image",
        ...input.candidates.map((candidate) => candidate.framePath),
        "--output-schema",
        schemaPath,
        "--json",
        "-",
      ...
      let output;
      try {
        output = await this.processRunner(executable, args, combinedSignal(signal), {
          cwd: this.cacheRoot,
          input: prompt,
        });
      } catch (error) {
        ...
        const detail =
          error instanceof ProcessFailure
            ...
            : error instanceof Error
              ? error.message
              : String(error);
        throw new Error(`Codex／ChatGPT 發布素材失敗：${detail}`);
      }
      let parsed: unknown;
      try {
        parsed = JSON.parse(finalAgentMessage(output.stdout));
      } catch (error) {
        throw new Error(`Codex 發布素材 JSON 無法解析：${error instanceof Error ? error.message : String(error)}`);
      }
  ...
  async analyzeStoryFramesBatch(
    requests: CodexStoryRequest[],
    model: string,
    signal?: AbortSignal,
  ): Promise<Map<string, StoryFrameAnalysis>> {
    ...
    for (let offset = 0; offset < requests.length; offset += CODEX_BATCH_SIZE) {
      ...
      try {
        ...
        try {
          parsed = JSON.parse(finalAgentMessage(output.stdout)) as { results?: unknown };
        } catch (error) {
          throw new Error(`Codex 字幕結果格式無法解析：${error instanceof Error ? error.message : String(error)}`);
        }
        if (!Array.isArray(parsed.results) || parsed.results.length !== batch.length)
          throw new Error(
            `Codex 應回傳 ${batch.length} 筆字幕，但收到 ${Array.isArray(parsed.results) ? parsed.results.length : 0} 筆。`,
          );
        const normalized = parsed.results.map(normalizeResult);
        const indexes = new Set(normalized.map((item) => item.index));
        if (indexes.size !== batch.length || [...indexes].some((index) => index >= batch.length))
          throw new Error("Codex 字幕結果序號重複或超出範圍。");
  ...
  async suggestMusic(
    ...
    try {
      ...
      try {
        parsed = JSON.parse(finalAgentMessage(output.stdout)) as { suggestions?: unknown; warnings?: unknown };
      } catch (error) {
        throw new Error(`Codex 配樂研究結果格式無法解析：${error instanceof Error ? error.message : String(error)}`);
      }
