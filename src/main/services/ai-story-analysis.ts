...
function contextReady(project: ProjectManifest, mode: "SUBTITLE" | "INTRO" = "SUBTITLE"): boolean {
  ...
  return Boolean(
    ...
    (mode === "INTRO" && context?.introPrompt?.trim()),
  );
}

function overlaps(
  cue: Pick<SubtitleCue, "startMs" | "endMs">,
  occupied: Array<Pick<SubtitleCue, "startMs" | "endMs">>,
): boolean {
  return occupied.some((item) => Math.max(cue.startMs, item.startMs) < Math.min(cue.endMs, item.endMs));
}

function clipDuration(clip: RenderClipSelection): number {
...
export class AiStoryAnalysisService {
  ...
  async testAccount(accountId: string, signal?: AbortSignal): Promise<OpenAiConnectionDiagnostic> {
    try {
      return await this.provider.testConnection(this.settings.getRuntimeAccount(accountId), signal);
    } catch (openAiError) {
      try {
        const codex = await this.codexProvider.testConnection(signal);
        return {
          checks: [...codex.checks, `OpenAI API 不可用，已確認可改用 ${codex.providerLabel}`],
          requestIds: [],
          providerLabel: "CODEX_CHATGPT",
        };
      } catch (codexError) {
        throw new Error(
          `OpenAI API 測試未通過：${openAiError instanceof Error ? openAiError.message : String(openAiError)}；Codex／ChatGPT 備援也不可用：${codexError instanceof Error ? codexError.message : String(codexError)}`,
        );
      }
    }
  }
  ...
  async transcribeVoiceInput(request: VoiceInputRequest, signal?: AbortSignal): Promise<VoiceInputResult> {
    ...
    const bytes =
      unknownBytes instanceof Uint8Array
        ? unknownBytes
    ...
    const result = await this.provider.transcribeBytes(
      bytes,
      `voice-input.${extension}`,
      mimeType,
      account,
      request.language === "zh-TW" ? "zh" : "en",
      signal,
    );
  ...
  async generateSubtitles(
    ...
    const requestedCueCount =
      options.targetCueCount === undefined ? DEFAULT_GENERATED_CUES : Math.round(Number(options.targetCueCount));
    if (!Number.isFinite(requestedCueCount) || requestedCueCount < 1 || requestedCueCount > MAX_GENERATED_CUES)
      throw new Error(`AI 字幕目標數量必須介於 1 到 ${MAX_GENERATED_CUES}。`);
    if (!contextReady(project, "SUBTITLE"))
      throw new Error("請先在 AI 帳號設定填寫專案主題、故事、人物或地點，再產生字幕草稿。");
    ...
    onProgress({
      ...
      processed: 0,
      total: 1,
      currentName: `正在確認 ${profile.name}；API 不可用時會安全改用 Codex／ChatGPT 登入…`,
    });
    const readinessKey = stableKey({
      accountId: profile.id,
      accountUpdatedAt: profile.updatedAt,
    ...
    if (this.codexFallbackReadinessKey === readinessKey && Date.now() < this.codexFallbackReadinessExpiresAt) {
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            `AI 字幕尚未開始分析。OpenAI API 不可用：${openAiFailure}；Codex／ChatGPT 登入備援也不可用：${codexError instanceof Error ? codexError.message : String(codexError)}`,
          );
        }
      }
    }
    if (storyBackend === "CODEX_CHATGPT" && options.includeSpeechTranscription) {
      throw new Error(
        `目前已改用 Codex／ChatGPT 登入產生知識型字幕，但它不接收本機音訊。請取消勾選「同時分析素材語音」後重試；若需要語音辨識，OpenAI API 仍需可用。原 API 原因：${openAiFailure}`,
      );
    }
    ...
    if (!totalClips)
      throw new Error(
        scopes.length === 1 && scopes[0] === "INTRO"
          ? "片頭沒有可分析的片段。"
          : "所選片頭／正片沒有可分析的保留片段。",
      );
    ...
    for (const plan of plans) {
      ...
      for (let clipIndex = 0; clipIndex < plan.clips.length; clipIndex += 1) {
        ...
        if (durationMs >= 800) {
          ...
          for (
            let sampleIndex = 0;
            sampleIndex < sampleCount && candidates.length < requestedCueCount;
            sampleIndex += 1
          ) {
            ...
            const localStart = Math.max(
              0,
              Math.min(durationMs - visualDuration, Math.round(slotStart + (slotDuration - visualDuration) / 2)),
            );
            ...
            if (
              ...
              !overlaps({ startMs: candidate.timelineStartMs, endMs: candidate.timelineEndMs }, confirmedInScope)
            )
              candidates.push(candidate);
          }
        }
        if (
          candidates.length < requestedCueCount &&
          asset.kind === "VIDEO" &&
          options.includeSpeechTranscription &&
          ready.mediaInfo?.audioCodec
        ) {
          for (let chunkOffsetMs = 0; chunkOffsetMs < durationMs; chunkOffsetMs += MAX_AUDIO_CHUNK_MS) {
            ...
            try {
              ...
              const segments = await this.provider.transcribe(
                audioPath,
                account!,
                project.aiStoryContext.subtitleLanguage,
                signal,
              );
              for (const segment of segments) {
                ...
                if (
                  candidate &&
                  candidate.timelineEndMs - candidate.timelineStartMs >= 100 &&
                  candidates.length < requestedCueCount &&
                  !overlaps({ startMs: candidate.timelineStartMs, endMs: candidate.timelineEndMs }, confirmedInScope)
                )
                  candidates.push(candidate);
    ...
    const limited = [...candidates]
      .sort(
        (left, right) =>
          ...
          left.timelineScope.localeCompare(right.timelineScope) ||
    ...
    for (let index = 0; index < limited.length; index += 1) {
      ...
      onProgress({
        phase: "SAMPLING_FRAMES",
        processed: index,
        total: limited.length,
        currentName: candidate.sourceAsset.fileName,
      });
      onProgress({
        phase: "MATCHING_STORY",
        processed: index,
        total: limited.length,
        currentName: candidate.sourceAsset.fileName,
      });
      const analysis =
        codexAnalyses?.get(index) ?? (await this.analyzeCandidate(candidate, project, account!, "SUBTITLE", signal));
      ...
      const occupied = [...confirmed, ...drafts].filter(
        (item) => (item.timelineScope ?? "MAIN") === candidate.timelineScope,
      );
    ...
    return {
      ...
      skippedConfirmedCount:
        ...
        candidates.filter((candidate) =>
          overlaps({ startMs: candidate.timelineStartMs, endMs: candidate.timelineEndMs }, confirmed),
        ).length,
  ...
  async analyzeMaterialForSubtitles(
    ...
    if (
      ...
      (request.timelineScope !== "MAIN" && request.timelineScope !== "INTRO")
    )
      throw new Error("素材字幕分析要求格式無效。");
    if (request.provider !== "CHATGPT" && request.provider !== "GEMINI") throw new Error("素材分析方式無效。");
    const project = this.store.getProject();
    if (!contextReady(project, "SUBTITLE"))
      throw new Error("請先在 AI 帳號／故事設定填寫目前簡略主題、故事、人物或地點。");
    ...
    const ready = await this.sources.ensureMetadata(asset.id, signal);
    const durationMs =
      asset.kind === "IMAGE" ? Math.max(1, ready.imageDurationMs ?? 5_000) : (ready.mediaInfo?.durationMs ?? 0);
    ...
    const selectedTimes =
      asset.kind === "IMAGE"
        ...
        : [
            ...new Set(
              (request.frameTimesMs ?? [])
                .map((value) => Math.round(Number(value)))
                .filter((value) => Number.isFinite(value)),
            ),
          ];
    if (asset.kind === "VIDEO" && !selectedTimes.length) throw new Error("影片至少要輸入一個影格時間點。");
    if (selectedTimes.some((value) => value < 0 || value >= durationMs))
      throw new Error(`影格時間必須介於 00:00 與 ${Math.ceil(durationMs / 1000)} 秒之間。`);
    const retainedRange = asset.previewRange ?? { inMs: 0, outMs: durationMs };
    if (
      asset.kind === "VIDEO" &&
      selectedTimes.some((value) => value < retainedRange.inMs || value >= retainedRange.outMs)
    )
      throw new Error("影格時間必須落在目前素材保留的 IN／OUT 範圍內；若要分析被排除段，請先調整範圍。");
    ...
    if (!scoped.length)
      throw new Error(request.timelineScope === "INTRO" ? "目前片頭沒有可分析的片段。" : "目前正片沒有可分析的片段。");
    const candidates: Candidate[] = [];
    const warnings: string[] = [];
    for (const sourceTimeMs of selectedTimes) {
      const clipIndex = scoped.findIndex(
        (item) => item.assetId === asset.id && sourceTimeMs >= item.inMs && sourceTimeMs < item.outMs,
      );
      const clip = clipIndex >= 0 ? scoped[clipIndex] : undefined;
      if (!clip) {
        warnings.push(
          `${asset.fileName} 的 ${Math.round(sourceTimeMs / 1000)} 秒不在目前${request.timelineScope === "INTRO" ? "片頭" : "正片"}輸出範圍，已略過。`,
        );
        continue;
      }
      const localTimelineStart = Math.max(0, Math.round(clip.outputStartMs - scopeOrigin + (sourceTimeMs - clip.inMs)));
      const clipTimelineEnd = Math.round(
        Math.min(clip.outputEndMs, scoped[clipIndex + 1]?.outputStartMs ?? clip.outputEndMs) - scopeOrigin,
      );
      const timelineEnd = Math.min(
        localTimelineStart + (asset.kind === "IMAGE" ? Math.min(durationMs, 4_000) : 3_000),
        clipTimelineEnd,
      );
      if (timelineEnd - localTimelineStart < 100) {
        warnings.push(
          `${asset.fileName} 的 ${Math.round(sourceTimeMs / 1000)} 秒距離片段尾端太近，無法建立可讀字幕時段，已略過。`,
        );
        continue;
      }
      candidates.push({
    ...
    if (request.provider === "GEMINI") {
      ...
      if (!gemini.apiKey)
        throw new Error("尚未設定 Gemini API Key。請到 AI 帳號／故事設定保存 Gemini 金鑰，再重新分析照片。");
      geminiAnalyses = await this.analyzeCandidatesWithGemini(
        candidates,
        project,
        { model: gemini.model, apiKey: gemini.apiKey },
        signal,
        onProgress,
      );
    } else {
      ...
      if (this.codexFallbackReadinessKey === readinessKey && Date.now() < this.codexFallbackReadinessExpiresAt) {
        ...
        try {
          ...
          try {
            ...
            throw new Error(
              `素材影像分析尚未開始。OpenAI API 不可用：${openAiError instanceof Error ? openAiError.message : String(openAiError)}；Codex／ChatGPT 備援也不可用：${codexError instanceof Error ? codexError.message : String(codexError)}`,
            );
          }
        }
      }
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      const analysis =
        geminiAnalyses?.get(index) ??
        codexAnalyses?.get(index) ??
        (await this.analyzeCandidate(candidate, project, account!, "SUBTITLE", signal));
      const text = (analysis.suggestedSubtitle || analysis.visualSummary).trim();
      if (!text) {
        warnings.push(`${asset.fileName} 的第 ${index + 1} 個影格沒有產生可用字幕。`);
        continue;
      }
      const cue: SubtitleCue = {
        id: randomUUID(),
        ...
        aiConfidence: analysis.confidence,
        aiWarnings: analysis.warnings,
        aiAnalysisVersion: MATERIAL_SUBTITLE_ANALYZER_VERSION,
      };
      const occupied = [...project.subtitleCues, ...drafts].filter(
        (item) => (item.timelineScope ?? "MAIN") === candidate.timelineScope && item.reviewStatus !== "REJECTED",
      );
      const existingCues = occupied
        .filter((item) => overlaps(cue, [item]))
        .map((item) => ({
          id: item.id,
      ...
      if (existingCues.length) {
        ...
        warnings.push(
          `${asset.fileName} 的第 ${index + 1} 個影格與「${existingCues
            .map((item) => item.text)
            .join("／")
            .slice(0, 120)}」重疊；建議加入字幕頁調整新舊時間。`,
        );
      }
      drafts.push(cue);
    }
  ...
  async enrichIntroSuggestions(
    suggestions: IntroSuggestion[],
    signal?: AbortSignal,
  ): Promise<{ suggestions: IntroSuggestion[]; analyzedCount: number; fallbackReason?: string }> {
    if (!suggestions.length) return { suggestions, analyzedCount: 0 };
    const project = this.store.getProject();
    if (!contextReady(project, "INTRO"))
      return {
        suggestions,
        analyzedCount: 0,
        fallbackReason: "尚未設定片頭文字指示或故事主題／人物／地點，使用本機畫面統計。",
      };
    let account;
    try {
      account = this.settings.getRuntimeAccount();
    } catch (error) {
      return { suggestions, analyzedCount: 0, fallbackReason: error instanceof Error ? error.message : String(error) };
    }
    ...
    for (const suggestion of suggestions) {
      ...
      if (terminalFailure) {
        enriched.push({
          ...
          aiWarnings: [`雲端語意分析未執行：${terminalFailure}`],
        });
        continue;
      }
      ...
      try {
        ...
        if (asset.mediaInfo?.audioCodec) {
          ...
          try {
            spokenSummary = (
              await this.provider.transcribe(audioPath, account, project.aiStoryContext.subtitleLanguage, signal)
            )
              .map((segment) => segment.text)
              .join(" ")
              .trim();
        ...
        const candidate: Candidate = {
          ...
          timelineScope: "INTRO",
          timelineStartMs: 0,
          timelineEndMs: suggestion.outMs - suggestion.inMs,
          sourceAsset: asset,
          sourceStartMs: suggestion.inMs,
          sourceEndMs: suggestion.outMs,
          transcript: spokenSummary,
        };
        const analysis = await this.analyzeCandidate(candidate, project, account, "INTRO", signal);
        const storyMatchScore = Math.min(
          100,
          analysis.topicRelevanceScore +
            (analysis.eventSummary ? 8 : 0) +
            (analysis.peopleSummary.length ? 8 : 0) +
            (analysis.locationSummary.length ? 4 : 0),
        );
        const combinedScore = Math.round(
          suggestion.score * 0.2 +
            storyMatchScore * 0.55 +
            analysis.transcriptVisualMatchScore * 0.15 +
        ...
        enriched.push({
          ...
          aiWarnings: analysis.warnings,
          reasons: [
            analysis.eventSummary || analysis.visualSummary,
            analysis.peopleSummary.length ? `人物 ${analysis.peopleSummary.join("、")}` : "",
            `人物／事件／故事 ${storyMatchScore}`,
            `聲畫符合 ${analysis.transcriptVisualMatchScore}`,
          ].filter(Boolean),
        });
      } catch (error) {
        ...
        if (/(credit_balance_exhausted|沒有可用 API 額度|API Key 無效|無法解密|尚未設定 OpenAI API Key)/i.test(message))
          terminalFailure = message;
        enriched.push({
          ...suggestion,
          analysisMode: "LOCAL_SIGNAL_ONLY",
          aiWarnings: [`雲端語意分析失敗：${message}`],
        });
      }
    }
  ...
  private speechCandidate(
    asset: SourceAsset,
    clip: RenderClipSelection,
  ...
  private async extractAudio(
    ...
    const output = assertWithinRoot(
      path.join(this.cacheRoot, "audio"),
      path.join(
        this.cacheRoot,
        "audio",
        `${asset.id}-${Math.round(startMs)}-${Math.round(durationMs)}-${randomUUID()}.mp3`,
      ),
    );
    try {
      await runProcess(
  ...
  private async analyzeCandidatesWithGemini(
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      const candidate = candidates[index];
      const key = stableKey({
        version: MATERIAL_SUBTITLE_ANALYZER_VERSION,
        backend: "GEMINI",
        source: candidate.sourceAsset.previewCacheKey,
        start: candidate.sourceStartMs,
        end: candidate.sourceEndMs,
        frameTimes: candidate.analysisFrameTimesMs,
        context: project.aiStoryContext,
        model: settings.model,
      });
      const resultPath = assertWithinRoot(
        path.join(this.cacheRoot, "results"),
        path.join(this.cacheRoot, "results", `${key}.json`),
      );
      try {
        const cached = JSON.parse(await readFile(resultPath, "utf8")) as VisualCache;
        if (cached.analyzerVersion === MATERIAL_SUBTITLE_ANALYZER_VERSION && cached.result) {
          resultByIndex.set(index, cached.result);
          continue;
        }
      } catch {
        /* cache miss */
      ...
      onProgress({
        ...
        processed: index,
        total: candidates.length,
        currentName: `Gemini 正在分析 ${candidate.sourceAsset.fileName}（${index + 1}/${candidates.length}）`,
      });
      const framePath = await this.extractStoryboard(
        candidate.sourceAsset,
        candidate.sourceStartMs,
        candidate.sourceEndMs,
        key,
        signal,
        candidate.analysisFrameTimesMs,
      );
      const result = await this.geminiProvider.analyze(
        framePath,
        {
          sourceFileName: candidate.sourceAsset.fileName,
          context: project.aiStoryContext,
  ...
  private async analyzeCandidatesWithCodex(
    ...
    for (let index = 0; index < candidates.length; index += 1) {
      ...
      const candidate = candidates[index];
      const key = stableKey({
        version: AI_STORY_ANALYZER_VERSION,
      ...
      try {
        const cached = JSON.parse(await readFile(resultPath, "utf8")) as VisualCache;
        if (cached.analyzerVersion === AI_STORY_ANALYZER_VERSION && cached.result) {
          resultByIndex.set(index, cached.result);
          continue;
        }
      } catch {
    ...
    if (missing.length) {
      onProgress({
        phase: "MATCHING_STORY",
        processed: resultByIndex.size,
        total: candidates.length,
        currentName: `Codex／ChatGPT 正在批次判斷 ${missing.length} 個畫面時段…`,
      });
      const analyzed = await this.codexProvider.analyzeStoryFramesBatch(
        missing.map((item) => item.request),
        model,
        signal,
      );
  ...
  private async writeAnalysisCache(
    resultPath: string,
    result: StoryFrameAnalysis,
    analyzerVersion = AI_STORY_ANALYZER_VERSION,
  ): Promise<void> {
  ...
  private async analyzeCandidate(
    candidate: Candidate,
    project: ProjectManifest,
    account: ReturnType<AiSettingsStore["getRuntimeAccount"]>,
    mode: "SUBTITLE" | "INTRO",
    signal?: AbortSignal,
    ...
    const result = await this.provider.analyzeStoryFrames(
      {
        mode,
        transcript: candidate.transcript,
        frames: [frame],
  ...
  private async extractStoryboard(
    asset: SourceAsset,
    startMs: number,
    endMs: number,
    key: string,
    signal?: AbortSignal,
    requestedFrameTimesMs?: number[],
  ): Promise<string> {
    const output = assertWithinRoot(
      path.join(this.cacheRoot, "frames"),
      path.join(this.cacheRoot, "frames", `${key}.jpg`),
    );
    try {
    ...
    const times = (
      requestedFrameTimesMs?.length
        ? requestedFrameTimesMs
        : [startMs + duration * 0.1, startMs + duration * 0.5, startMs + duration * 0.9]
    ).map((value) => Math.max(0, value / 1000));
    ...
    try {
      if (asset.kind === "IMAGE") {
        await runProcess(
          ...
          [
            ...
            asset.sourcePath,
            "-vf",
            "scale=1080:720:force_original_aspect_ratio=decrease,pad=1080:720:(ow-iw)/2:(oh-ih)/2:black",
            "-frames:v",
            "1",
            "-q:v",
            "4",
      ...
      const labels = times
        .map(
          (_time, index) =>
            `[${index}:v]scale=360:240:force_original_aspect_ratio=decrease,pad=360:240:(ow-iw)/2:(oh-ih)/2:black[f${index}]`,
        )
        .join(";");
      const stack =
        times.length === 1
          ? "[f0]null[out]"
          : `${times.map((_time, index) => `[f${index}]`).join("")}hstack=inputs=${times.length}[out]`;
      await runProcess(
        this.ffmpegExecutable,
        [
          "-hide_banner",
