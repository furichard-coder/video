...
function looksLikeCredentials(value: unknown): value is YoutubeCredentialFile {
  ...
  return item.schemaVersion === 1 && (item.encryptedClientSecret === undefined || typeof item.encryptedClientSecret === "string") && (item.encryptedRefreshToken === undefined || typeof item.encryptedRefreshToken === "string");
}
...
function browserCandidates(browser: YoutubeBrowser): string[] {
  ...
  const relative = browser === "CHROME" ? path.join("Google", "Chrome", "Application", "chrome.exe") : path.join("Microsoft", "Edge", "Application", "msedge.exe");
  return [...new Set([programFiles, programFilesX86, localAppData].filter((root): root is string => Boolean(root)).map((root) => path.join(root, relative)))];
}

async function firstExisting(paths: string[]): Promise<string | undefined> {
  for (const candidate of paths) {
    try { await access(candidate); return candidate; } catch { /* try the next standard install location */ }
  }
  return undefined;
}
...
export class WindowsYoutubeBrowserLauncher implements BrowserLauncher {
  ...
  async discover(): Promise<YoutubeBrowserOption[]> {
    const [chrome, edge] = await Promise.all([firstExisting(browserCandidates("CHROME")), firstExisting(browserCandidates("EDGE"))]);
    return [
  ...
  async open(preferredBrowser: YoutubeBrowser, url: string): Promise<{ browserLabel: string; usedFallback: boolean }> {
    ...
    const ordered = [options.find((item) => item.id === preferredBrowser), ...options.filter((item) => item.id !== preferredBrowser)].filter((item): item is YoutubeBrowserOption => Boolean(item));
    for (const option of ordered) {
      if (!option.executablePath) continue;
      try {
        await new Promise<void>((resolve, reject) => {
          const child = spawn(option.executablePath!, [url], { shell: false, detached: true, stdio: "ignore", windowsHide: false });
          child.once("error", reject);
          child.once("spawn", () => { child.unref(); resolve(); });
        });
        return { browserLabel: option.label, usedFallback: option.id !== preferredBrowser };
      } catch { /* try the second browser */ }
    }
...
export class YoutubeSettingsStore {
  ...
  async getSnapshot(): Promise<YoutubeSettingsSnapshot> {
    ...
    return {
      ...
      clientIdHint: this.settings.clientId ? `${this.settings.clientId.slice(0, 8)}…${this.settings.clientId.slice(-12)}` : undefined,
  ...
  async update(update: YoutubeSettingsUpdate): Promise<YoutubeSettingsSnapshot> {
    ...
    await this.mutate(() => { this.settings.preferredBrowser = update.preferredBrowser; this.settings.targetChannelName = targetChannelName; });
    return this.getSnapshot();
  }

  async importOAuthClient(filePath: string): Promise<YoutubeSettingsSnapshot> {
    if (!this.protector.isAvailable()) throw new Error("Windows 安全儲存目前不可用；為避免明文憑證落盤，無法匯入 Google OAuth client。");
    let parsed: unknown;
    try { parsed = JSON.parse(await readFile(filePath, "utf8")); }
    catch (error) { throw new Error(`無法讀取 OAuth client JSON：${error instanceof Error ? error.message : String(error)}`); }
    const installed = parsed && typeof parsed === "object" ? (parsed as { installed?: Record<string, unknown> }).installed : undefined;
    ...
    if (!clientId.endsWith(".apps.googleusercontent.com") || clientId.length > 300 || clientSecret.length < 6 || clientSecret.length > 500) {
      throw new Error("請選擇 Google Cloud 建立的「Desktop app」OAuth client JSON；Web／Chrome app 憑證不適用。");
    }
    await this.mutate(() => {
      this.settings.clientId = clientId;
      delete this.settings.channelId; delete this.settings.channelTitle; delete this.settings.connectedAt;
  ...
  getRuntimeClient(): { clientId: string; clientSecret: string; refreshToken?: string; settings: YoutubeSettingsFile } {
    if (!this.settings.clientId || !this.credentials.encryptedClientSecret) throw new Error("尚未匯入 Google Desktop app OAuth client JSON。");
    ...
    try {
      return {
        ...
        refreshToken: this.credentials.encryptedRefreshToken ? this.protector.unprotect(this.credentials.encryptedRefreshToken) : undefined,
        settings: structuredClone(this.settings),
      };
    } catch { throw new Error("YouTube 憑證無法解密，請重新匯入 OAuth client 並連結頻道。"); }
  }

  async saveConnection(refreshToken: string, channelId: string, channelTitle: string): Promise<YoutubeSettingsSnapshot> {
  ...
  private async mutate(mutator: () => void): Promise<void> {
    const operation = this.writeChain.then(async () => { mutator(); await this.writeJson(this.settingsPath, this.settings); await this.writeJson(this.credentialsPath, this.credentials); });
...
export class YoutubeUploadService {
  ...
  async connect(signal?: AbortSignal): Promise<YoutubeSettingsSnapshot> {
    ...
    try {
      ...
      const token = await this.exchangeAuthorizationCode(runtime.clientId, runtime.clientSecret, redirectUri, authorizationCode, verifier, signal);
      ...
      if (token.scope && (!scopes.has(YOUTUBE_UPLOAD_SCOPE) || !scopes.has(YOUTUBE_READ_SCOPE))) throw new Error("Google 未授予完整的 YouTube 上傳與頻道確認權限。");
  ...
  async disconnect(signal?: AbortSignal): Promise<YoutubeSettingsSnapshot> {
    ...
    try { refreshToken = this.settingsStore.getRuntimeClient().refreshToken; } catch { /* local cleanup remains available */ }
    if (refreshToken) {
      try {
        await this.fetcher(REVOKE_ENDPOINT, { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ token: refreshToken }), signal });
      } catch { /* revoke is best effort; encrypted local token is always removed */ }
    }
  ...
  async upload(outputPath: string, request: YoutubeUploadRequest, signal?: AbortSignal, onProgress: (progress: YoutubeUploadProgress) => void = () => undefined): Promise<YoutubeUploadResult> {
    ...
    const runtime = this.settingsStore.getRuntimeClient();
    if (!runtime.refreshToken || !runtime.settings.channelId || !runtime.settings.channelTitle) throw new Error("尚未連結 YouTube 頻道，請先完成 Google 授權。");
    if (runtime.settings.channelTitle.trim().toLocaleLowerCase() !== runtime.settings.targetChannelName.trim().toLocaleLowerCase()) {
      throw new Error(`目前連結頻道是「${runtime.settings.channelTitle}」，不是目標「${runtime.settings.targetChannelName}」；已阻擋上傳。`);
    }
    const file = await stat(outputPath);
    if (!file.isFile() || path.extname(outputPath).toLowerCase() !== ".mp4") throw new Error("只允許上傳本次 App 完成的 MP4 預覽檔。");
      ...
      while (offset < totalBytes) {
        ...
        for (let attempt = 0; attempt < 5; attempt += 1) {
          try {
            ...
            if (signal?.aborted || (error instanceof Error && error.name === "AbortError")) throw abortError("已取消 YouTube 上傳。");
            lastError = error;
          }
          const retryAfter = Number(response?.headers.get("retry-after"));
          await this.delay(Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1_000 : Math.min(8_000, 500 * 2 ** attempt), signal);
        }
        if (!response) throw new Error(`YouTube 上傳連線失敗：${lastError instanceof Error ? lastError.message : String(lastError)}`);
        if (response.status === 308) {
          const range = response.headers.get("range")?.match(/bytes=0-(\d+)/i);
          offset = range ? Number(range[1]) + 1 : offset + length;
          onProgress({ phase: "UPLOADING", bytesUploaded: offset, totalBytes, percent: Math.min(99, offset / totalBytes * 100) });
          continue;
        }
  ...
  private async exchangeAuthorizationCode(clientId: string, clientSecret: string, redirectUri: string, code: string, verifier: string, signal?: AbortSignal): Promise<TokenResponse> {
    ...
    if (!response.ok || !token.access_token) throw new Error(`Google 授權交換失敗：${token.error_description ?? token.error ?? response.status}`);
    return token;
  }

  private async refreshAccessToken(clientId: string, clientSecret: string, refreshToken: string, signal?: AbortSignal): Promise<string> {
    const response = await this.fetcher(TOKEN_ENDPOINT, {
      method: "POST",
      headers: { "content-type": "application/x-www-form-urlencoded" },
    ...
    const token = await response.json() as TokenResponse;
    if (!response.ok || !token.access_token) throw new Error(`YouTube 授權已失效：${token.error_description ?? token.error ?? response.status}。請重新連結頻道。`);
    return token.access_token;
  }

  private async fetchChannel(accessToken: string, signal?: AbortSignal, preferredTitle?: string): Promise<{ id: string; title: string }> {
    const response = await this.fetcher(CHANNEL_ENDPOINT, { headers: { authorization: `Bearer ${accessToken}` }, signal });
    ...
    const channels = (body.items ?? []).filter((item): item is { id: string; snippet: { title: string } } => Boolean(item.id && item.snippet?.title));
    const selected = channels.find((item) => item.snippet.title.trim().toLocaleLowerCase() === preferredTitle?.trim().toLocaleLowerCase()) ?? channels[0];
  ...
  private async createAuthorizationReceiver(expectedState: string, signal?: AbortSignal): Promise<{ server: Server; redirectUri: string; code: Promise<string>; dispose(): void }> {
    assertNotAborted(signal);
    ...
    const finish = (value: string | Error) => {
      ...
      if (value instanceof Error) rejectCode(value); else resolveCode(value);
    };
    const server = createServer((request, response) => {
      ...
      if (requestUrl.pathname !== "/oauth2callback" || requestUrl.searchParams.get("state") !== expectedState) {
        ...
        response.end("<!doctype html><meta charset=utf-8><title>授權失敗</title><h1>授權回應無效</h1><p>請關閉此頁並回到 SceneryWalker。</p>");
      ...
      if (oauthError || !authorizationCode) {
        response.end("<!doctype html><meta charset=utf-8><title>已取消</title><h1>未完成授權</h1><p>可以關閉此頁並回到 SceneryWalker。</p>");
        finish(new Error(oauthError === "access_denied" ? "使用者取消 Google 授權。" : `Google 授權失敗：${oauthError ?? "沒有授權碼"}`));
        return;
      }
      response.end("<!doctype html><meta charset=utf-8><title>授權完成</title><h1>已連結 SceneryWalker</h1><p>可以關閉此頁並回到 App。</p>");
      finish(authorizationCode);
    });
    await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(0, "127.0.0.1", () => resolve()); });
    const address = server.address();
    if (!address || typeof address === "string") { server.close(); throw new Error("無法建立 Google OAuth 本機回呼。"); }
    ...
    return { server, redirectUri: `http://127.0.0.1:${address.port}/oauth2callback`, code, dispose: () => finish(abortError("YouTube 頻道連結已結束。")) };
  }
}
