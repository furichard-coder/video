 ...
 */
export function safeActionCenter(contentBounds: UiRectangle, rendererSize: { width: number; height: number }, actionRect: UiRectangle): UiPoint | undefined {
  const values = [contentBounds.x, contentBounds.y, contentBounds.width, contentBounds.height, rendererSize.width, rendererSize.height, actionRect.x, actionRect.y, actionRect.width, actionRect.height];
  if (values.some((value) => !Number.isFinite(value))) return undefined;
  if (rendererSize.width <= 0 || rendererSize.height <= 0 || actionRect.width <= 1 || actionRect.height <= 1) return undefined;
  if (actionRect.x < 0 || actionRect.y < 0 || actionRect.x + actionRect.width > rendererSize.width + 1 || actionRect.y + actionRect.height > rendererSize.height + 1) return undefined;
  return {
...
export function moveWindowsCursor(point: UiPoint): void {
  ...
  const child = spawn(executable, ["-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden", "-Command", WINDOWS_CURSOR_SCRIPT], {
