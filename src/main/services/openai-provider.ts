...
import {
  ...
  type PublishGenerationInput,
  type PublishGeneratedDraft,
...
async function responseError(response: Response): Promise<Error> {
  ...
  const reference = [code || type ? `代碼 ${code || type}` : "", requestId ? `Request ID ${requestId}` : ""]
    .filter(Boolean)
    .join("；");
  const suffix = reference ? `（${reference}）` : "";
  if (response.status === 401)
    return new Error(`OpenAI API Key 無效、已撤銷，或不屬於可用的 API 專案，請到 AI 帳號設定重新輸入。${suffix}`);
  if (response.status === 403)
    return new Error(`這把 OpenAI API Key 沒有執行所選模型／端點的權限，請檢查 API project 權限。${suffix}`);
  if (response.status === 404)
    return new Error(`OpenAI API 找不到所選模型，或這個 API project 無法使用它。請檢查模型名稱與專案權限。${suffix}`);
  if (response.status === 429) {
    ...
    if (quotaFailure) {
      return new Error(
        `這把 Key 所屬的 OpenAI API project 沒有可用 API 額度，或已達用量／支出上限。Codex／ChatGPT 顯示的 token 與 API 帳務是分開的，不會自動供這個 App 使用。請到 OpenAI API 平台檢查 Billing、Limits 與 Key 所屬 project。${suffix}`,
      );
    }
    const retry = [
      retryAfter ? `建議 ${retryAfter} 秒後重試` : "",
      requestReset ? `requests 重設 ${requestReset}` : "",
      tokenReset ? `tokens 重設 ${tokenReset}` : "",
    ]
      .filter(Boolean)
      .join("；");
    return new Error(
      `OpenAI API 暫時超過每分鐘的請求或 token 速率限制，請稍後重試。${retry ? ` ${retry}。` : ""}${suffix}`,
    );
  }
...
export class OpenAiProvider {
  ...
  async generatePublishAssets(
    input: PublishGenerationInput,
    account: AiAccountProfile & { apiKey: string },
    signal?: AbortSignal,
  ): Promise<{ draft: PublishGeneratedDraft; model: string; requestId?: string }> {
    const content: Array<Record<string, unknown>> = [
      {
        type: "input_text",
        text: [
          "為旅遊 YouTube 影片產生發布素材。先理解主題、故事與正片內容，再優先判讀已選片頭的實際候選畫面，讓標題與縮圖呈現影片開場承諾。只能依提供的資料與低解析影格；不得猜測人物身分或未提供的地點。",
          JSON.stringify({
            topic: input.topic,
            durationMs: input.durationMs,
            intro: input.introSummary,
            timeline: input.timelineSummary,
            candidates: input.candidates.map(({ framePath: _framePath, ...candidate }) => candidate),
          }),
          "標題須延伸影片內容與片頭畫面，嚴格依 JSON schema 回傳 3–5 個標題、說明、hashtags、3 個可追溯縮圖概念與合法章節。縮圖概念應優先選用 origin=INTRO 的候選，除非該畫面明顯不適合。",
          "品質要求：標題彼此角度要有明顯差異、避免制式空話，必須具體呼應可見主體或片頭情境，總長不超過 100 字；可自然加入一個英文關鍵片語或 hashtag，但不可堆砌。縮圖顯示文字應精簡、通常不超過 12 個中文字，與標題互補而非重複，並依候選畫面的主體位置選擇左右版面、避免遮住重要景物。說明與章節只能使用有來源依據的資訊。",
        ].join("\n\n"),
      },
    ];
    ...
    const response = await this.fetcher(`${OPENAI_BASE_URL}/responses`, {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: { type: "json_schema", name: "youtube_publish_assets", strict: true, schema: PUBLISH_JSON_SCHEMA },
        },
      }),
    ...
    const resultText = outputText(body);
    let parsed: unknown;
    try {
      parsed = JSON.parse(resultText);
    } catch {
      throw new Error("OpenAI 發布素材回傳不是有效 JSON。");
    }
    return {
      draft: normalizePublishResponse(parsed, input),
      model: account.visionModel,
      requestId: response.headers.get("x-request-id") ?? undefined,
    };
  }

  async testStoryConnection(
    account: AiAccountProfile & { apiKey: string },
    signal?: AbortSignal,
  ): Promise<OpenAiConnectionDiagnostic> {
    ...
    const modelIds = new Set(
      (modelsBody.data ?? []).map((item) => (typeof item.id === "string" ? item.id : "")).filter(Boolean),
    );
    ...
    if (modelIds.size && !modelIds.has(account.visionModel))
      throw new Error(`目前 API project 的模型清單中沒有「${account.visionModel}」，請改選可用的畫面／故事模型。`);
  ...
  async translateTexts(
    ...
    try {
      parsed = JSON.parse(resultText) as { translations?: unknown };
    } catch {
      throw new Error("OpenAI 字幕翻譯回傳格式無法解析。");
    }
    if (
  ...
  async testConnection(
    ...
    let transcription;
    try {
      transcription = await this.transcribeBytes(
        diagnosticWav(),
        "scenerywalker-api-test.wav",
        "audio/wav",
        account,
        "zh",
        signal,
      ...
      throw new Error(`語音模型實際測試失敗：${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  async transcribe(
    ...
    return (
      await this.transcribeBytes(bytes, path.basename(audioPath), audioMimeType(audioPath), account, language, signal)
    ).segments;
  }
  ...
  async transcribeBytes(
    ...
    form.append(
      "file",
      new Blob([Uint8Array.from(bytes).buffer], { type: mimeType || audioMimeType(fileName) }),
    ...
    const body = (await response.json()) as {
      text?: unknown;
      duration?: unknown;
      segments?: Array<{ start?: unknown; end?: unknown; text?: unknown; speaker?: unknown }>;
    };
    ...
    if (!segments.length && typeof body.text === "string" && body.text.trim()) {
      const durationMs = Number.isFinite(Number(body.duration))
        ? Math.max(100, Math.round(Number(body.duration) * 1000))
        : 1_000;
  ...
  async analyzeStoryFrames(
    ...
    const contextText = [
      ...
      ...(request.mode === "INTRO"
        ? [`本次片頭文字指示：${request.context.introPrompt || "未提供（依人物／事件／故事優先原則）"}`]
        : []),
    ...
    try {
      ...
      throw new Error("OpenAI 視覺分析回傳格式無法解析。");
    }
    return {
  ...
  async suggestMusic(
    ...
    const topicSummary = [context.topic, context.locations.join("、"), context.storySummary, context.audiencePromise]
      .map((item) => item.trim())
      .filter(Boolean)
      .join("｜");
    ...
    try {
      parsed = JSON.parse(resultText) as { suggestions?: unknown; warnings?: unknown };
    } catch {
      throw new Error("OpenAI 配樂建議回傳格式無法解析。");
    }
