...
export class GoogleCloudTranslationProvider {
  ...
  async translateTexts(texts: string[], targetLanguage: TargetLanguage, apiKey: string, signal?: AbortSignal): Promise<string[]> {
    ...
    if (!response.ok) {
      let detail = "";
      try { detail = String(((await response.json()) as { error?: { message?: unknown } }).error?.message ?? "").replace(/[\r\n]+/g, " ").slice(0, 300); }
    ...
    const translations = (body.data?.translations ?? []).map((item) => typeof item.translatedText === "string" ? decodeEntities(item.translatedText).trim() : "");
    if (translations.length !== texts.length || translations.some((item) => !item)) throw new Error("Google Cloud Translation 沒有逐項傳回完整結果。");
    return translations;
  }
}
...
export class SubtitleTranslationService {
  ...
  async translate(cues: SubtitleCue[], languages: SubtitleRenderLanguage[], signal?: AbortSignal): Promise<SubtitleTranslationResult> {
    ...
    for (const language of languages) {
      ...
      try {
        ...
        try { await this.writeCache(texts, language, translations, "OPENAI"); } catch { /* derived cache failure must not discard a valid translation */ }
      } catch (error) {
      ...
      if (!translations) {
        try {
          translations = await this.google.translateTexts(texts, language, this.translationSettings.getGoogleCloudApiKey(), signal);
          providers.add("GOOGLE_CLOUD");
          try { await this.writeCache(texts, language, translations, "GOOGLE_CLOUD"); } catch { /* derived cache is optional */ }
        } catch (googleError) {
  ...
  private cachePath(texts: string[], language: TargetLanguage): string {
    const key = createHash("sha256").update(JSON.stringify({ version: this.version, language, texts })).digest("hex");
    return path.join(this.cacheRoot, `${key}.json`);
  }

  private async readCache(texts: string[], language: TargetLanguage): Promise<{ translations: string[]; provider: "OPENAI" | "GOOGLE_CLOUD" } | undefined> {
    try {
      const parsed = JSON.parse(await readFile(this.cachePath(texts, language), "utf8")) as { version?: unknown; language?: unknown; translations?: unknown; provider?: unknown };
      if (parsed.version !== this.version || parsed.language !== language || !Array.isArray(parsed.translations) || parsed.translations.length !== texts.length || !["OPENAI", "GOOGLE_CLOUD"].includes(String(parsed.provider))) return undefined;
  ...
  private async writeCache(texts: string[], language: TargetLanguage, translations: string[], provider: "OPENAI" | "GOOGLE_CLOUD"): Promise<void> {
    const outputPath = this.cachePath(texts, language);
    const partial = `${outputPath}.${process.pid}.${randomUUID()}.partial`;
    await mkdir(path.dirname(outputPath), { recursive: true });
    try {
      await writeFile(partial, JSON.stringify({ version: this.version, language, provider, translations, createdAt: new Date().toISOString() }), { encoding: "utf8", flag: "wx" });
