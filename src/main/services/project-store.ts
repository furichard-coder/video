...
import {
  ...
  type PreviewRange,
  type PreviewRangeUpdateResult,
  type ProjectChangeSection,
  type ProjectChangedEvent,
  type ProjectColorSettings,
  type ProjectFileResult,
  type ProjectFileState,
  type ProjectHistoryState,
  type ProjectManifest,
  type RemovedIntroSegment,
  type RemovedMainAsset,
  type SortMode,
  type SourceAsset,
  type SubtitleCue,
  type SubtitleTimelineScope,
...
import {
  clipMainExclusionRanges,
  clipVolumeSegments,
  clipZoomSegments,
  imageDurationMs,
  normalizeMainExclusionRanges,
  validateBgmTrack,
  validateImageDurationMs,
  validateMediaInsertion,
  validatePercent,
  validateSubtitleCues,
  validateVolumeSegments,
  validateZoomSegments,
} from "../../shared/editing-rules";
...
function createProject(): ProjectManifest {
  ...
  return {
    ...
    recentMainRemovals: [],
    introSegments: [],
    introTargetDurationMs: DEFAULT_INTRO_TARGET_DURATION_MS,
    introSegmentMaxDurationMs: DEFAULT_INTRO_SEGMENT_MAX_DURATION_MS,
    colorSettings: DEFAULT_PROJECT_COLOR_SETTINGS,
    watermarkSettings: structuredClone(DEFAULT_WATERMARK_SETTINGS),
    ...
    aiStoryContext: defaultAiStoryContext(),
    subtitleCues: [],
    timelineRevision: 0,
    subtitleTimelineRevision: 0,
    mainTimelineRevision: 0,
    introTimelineRevision: 0,
    mainSubtitleReviewRevision: 0,
    introSubtitleReviewRevision: 0,
...
function normalizeZoomEnhancementPreset(value: unknown): ZoomEnhancementPreset {
  return value === "OFF" || value === "DETAIL" || value === "DENOISE" || value === "BALANCED"
    ? value
    : DEFAULT_ZOOM_ENHANCEMENT_PRESET;
}

function migrateProject(raw: Record<string, unknown>): ProjectManifest {
  const aiPublishAssets =
    raw.aiPublishAssets && typeof raw.aiPublishAssets === "object"
  ...
  const sources = (raw.sources as SourceAsset[]).map((asset) => ({
    ...
    volumeSegments: Array.isArray(asset.volumeSegments)
      ? asset.volumeSegments.map((segment) => ({
          ...segment,
          volumePercent: Math.max(
            0,
            Math.min(
              MAX_MIX_VOLUME_PERCENT,
              Number.isFinite(segment.volumePercent) ? segment.volumePercent : DEFAULT_SOURCE_AUDIO_VOLUME_PERCENT,
            ),
          ),
        }))
      : [],
    mainExclusionRanges: Array.isArray(asset.mainExclusionRanges) ? asset.mainExclusionRanges : [],
    zoomSegments: Array.isArray(asset.zoomSegments)
      ? asset.zoomSegments.map((segment) => ({
          ...segment,
          enhancementPreset: normalizeZoomEnhancementPreset(segment.enhancementPreset),
        }))
      : [],
  }));
  ...
  const mediaInsertions = rawInsertions.flatMap((item, index): MediaInsertion[] => {
    ...
    if (inserted.kind === "IMAGE") sourceOutMs = imageDurationMs(inserted);
      ...
      sourceInMs = Number.isFinite(item.sourceInMs)
        ? Math.max(0, Math.round(item.sourceInMs!))
        : (inserted.previewRange?.inMs ?? 0);
      sourceOutMs = Number.isFinite(item.sourceOutMs)
        ? Math.min(durationMs, Math.round(item.sourceOutMs!))
  ...
  const rawOrder = Array.isArray(raw.timelineOrder)
    ? raw.timelineOrder.filter(
        (id): id is string =>
          typeof id === "string" && sourceIds.has(id) && !excludedMainIds.has(id) && !insertedMediaIds.has(id),
      )
    : [];
  const order = [...new Set(rawOrder)];
  const rawPending = Array.isArray(raw.pendingAssetIds)
    ? raw.pendingAssetIds.filter(
        (id): id is string =>
          typeof id === "string" &&
          sourceIds.has(id) &&
          !excludedMainIds.has(id) &&
          !insertedMediaIds.has(id) &&
          !order.includes(id),
      )
    : [];
  ...
  if (order.length === 0 && pending.length === 0 && activeSources.length > 0)
    order.push(
      ...sortAssets(activeSources, (raw.sortMode as SortMode | undefined) ?? "SMART_SEQUENCE").map((asset) => asset.id),
    );
  else
    for (const asset of activeSources)
      if (!order.includes(asset.id) && !pending.includes(asset.id)) pending.push(asset.id);
  const recentMainRemovals = Array.isArray(raw.recentMainRemovals)
    ? (raw.recentMainRemovals as RemovedMainAsset[]).filter(
        (item) => item && excludedMainIds.has(item.assetId) && sourceIds.has(item.assetId),
      )
    : [];
  ...
  const recentIntroRemovals = Array.isArray(raw.recentIntroRemovals)
    ? (raw.recentIntroRemovals as RemovedIntroSegment[]).filter(
        (item) =>
          item?.segment && sourceIds.has(item.segment.assetId) && introExcludedSegmentIds.includes(item.segment.id),
      )
    : [];
  const longestExistingIntroMs = introSegments.reduce(
    (maximum, segment) => Math.max(maximum, segment.outMs - segment.inMs),
    0,
  );
  const introSegmentMaxDurationMs = (() => {
    try {
      return normalizeIntroSegmentMaxDuration(
        raw.introSegmentMaxDurationMs ?? Math.max(DEFAULT_INTRO_SEGMENT_MAX_DURATION_MS, longestExistingIntroMs),
      );
    } catch {
      return Math.max(DEFAULT_INTRO_SEGMENT_MAX_DURATION_MS, Math.min(INTRO_MAX_SEGMENT_MS, longestExistingIntroMs));
    }
  })();
  const bgmTracks = Array.isArray(raw.bgmTracks)
    ? (raw.bgmTracks as BgmTrack[]).map((track) => ({
        ...track,
        sourceKind: track.sourceKind ?? ("LOCAL_FILE" as const),
        resolutionStatus:
          track.resolutionStatus ?? ((track.sourcePath ? "READY" : "NEEDS_LOCAL_FILE") as "READY" | "NEEDS_LOCAL_FILE"),
        volumePercent: Math.max(
          0,
          Math.min(
            MAX_MIX_VOLUME_PERCENT,
            Number.isFinite(track.volumePercent) ? track.volumePercent : DEFAULT_BGM_VOLUME_PERCENT,
          ),
        ),
      }))
    : [];
  const sourceAudioVolumePercent = Number.isFinite(raw.sourceAudioVolumePercent)
    ...
    : DEFAULT_SOURCE_AUDIO_VOLUME_PERCENT;
  return {
    ...(raw as unknown as ProjectManifest),
    schemaVersion: MANIFEST_SCHEMA_VERSION,
    previewerVersion: PREVIEWER_VERSION,
    sources,
    timelineOrder: order,
    timelineTransitionSeconds:
      raw.timelineTransitionSeconds === 0.5 || raw.timelineTransitionSeconds === 0.7
        ? raw.timelineTransitionSeconds
        : 0.3,
    ...
    introTargetDurationMs: (() => {
      try {
        return normalizeIntroTargetDuration(raw.introTargetDurationMs ?? DEFAULT_INTRO_TARGET_DURATION_MS);
      } catch {
        return DEFAULT_INTRO_TARGET_DURATION_MS;
      }
    })(),
    introSegmentMaxDurationMs,
    colorSettings: normalizeProjectColorSettings(raw.colorSettings),
    watermarkSettings: normalizeWatermarkSettings(raw.watermarkSettings),
    introExcludedSegmentIds,
    recentIntroRemovals,
    placementDecisions: Array.isArray(raw.placementDecisions)
    ...
    subtitleTimelineRevision: Number.isInteger(raw.subtitleTimelineRevision)
      ? (raw.subtitleTimelineRevision as number)
      : 0,
    mainTimelineRevision: Number.isInteger(raw.mainTimelineRevision)
      ? (raw.mainTimelineRevision as number)
      : Number.isInteger(raw.timelineRevision)
        ? (raw.timelineRevision as number)
        : 0,
    introTimelineRevision: Number.isInteger(raw.introTimelineRevision)
      ? (raw.introTimelineRevision as number)
      : Number.isInteger(raw.timelineRevision)
        ? (raw.timelineRevision as number)
        : 0,
    mainSubtitleReviewRevision: Number.isInteger(raw.mainSubtitleReviewRevision)
      ? (raw.mainSubtitleReviewRevision as number)
      : Number.isInteger(raw.subtitleTimelineRevision)
        ? (raw.subtitleTimelineRevision as number)
        : 0,
    introSubtitleReviewRevision: Number.isInteger(raw.introSubtitleReviewRevision)
      ? (raw.introSubtitleReviewRevision as number)
      : Number.isInteger(raw.subtitleTimelineRevision)
        ? (raw.subtitleTimelineRevision as number)
        : 0,
    audioMixPolicy: "ORIGINAL_PLUS_BGM_LIMITED_0_95",
...
export class ProjectStore {
  ...
  async initialize(): Promise<ProjectManifest> {
    ...
    try {
      ...
      const migrated =
        ...
        parsed.previewerVersion !== PREVIEWER_VERSION ||
        !Array.isArray(parsed.excludedMainAssetIds) ||
        !Array.isArray(parsed.recentMainRemovals) ||
        !Array.isArray(parsed.introSegments) ||
        !Number.isFinite(parsed.introTargetDurationMs) ||
        !Number.isFinite(parsed.introSegmentMaxDurationMs) ||
        !parsed.colorSettings ||
        ...
        (parsed.timelineTransitionSeconds !== 0.3 &&
          parsed.timelineTransitionSeconds !== 0.5 &&
          parsed.timelineTransitionSeconds !== 0.7) ||
        (parsed.sources as Partial<SourceAsset>[]).some(
          (asset) =>
            !Array.isArray(asset.zoomSegments) || asset.zoomSegments.some((segment) => !segment.enhancementPreset),
        );
  ...
  subscribe(listener: (event: ProjectChangedEvent) => void): () => void {
  ...
  async openProject(filePath: string): Promise<ProjectFileResult> {
    ...
    const project = await this.queueProjectFileOperation(async () => {
      ...
      this.activeProjectPath = resolved;
      this.clearHistory();
      this.emitChanged(previous, opened, [
        "SOURCES",
        "MAIN_TIMELINE",
        "INTRO_TIMELINE",
        "BGM",
  ...
  async addAssets(assets: SourceAsset[]): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      for (const input of assets) {
        ...
        project.sources.push(asset);
        existing.add(asset.id);
        if (isInitialBatch) project.timelineOrder.push(asset.id);
        else project.pendingAssetIds.push(asset.id);
      }
  ...
  async restoreMainAsset(assetId: string): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      if (
        !record ||
        !project.excludedMainAssetIds.includes(assetId) ||
        !project.sources.some((asset) => asset.id === assetId)
      )
        throw new Error("找不到可恢復的正片素材。");
      ...
      if (record.wasPending) project.pendingAssetIds.push(assetId);
        ...
        const index = Math.max(
          0,
          Math.min(project.timelineOrder.length, record.previousTimelineIndex ?? project.timelineOrder.length),
        );
  ...
  async setIntroSegments(segments: IntroSuggestion[]): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      const normalized = this.validateIntroSegments(project, segments).filter(
        (item) => !project.introExcludedSegmentIds.includes(item.id),
      );
  ...
  async setIntroSegmentMaxDuration(durationMs: number): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      const balanced = balanceIntroSegments(
        ...
        project.sources,
        project.introTargetDurationMs,
        maximumMs,
      );
      const rangesChanged = balanced.some(
        (segment, index) =>
          segment.inMs !== project.introSegments[index]?.inMs || segment.outMs !== project.introSegments[index]?.outMs,
      );
  ...
  async removeIntroSegment(segmentId: string): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      const record: RemovedIntroSegment = {
        segment: clone(project.introSegments[index]),
        previousIndex: index,
        removedAt: new Date().toISOString(),
      };
      ...
      project.recentIntroRemovals = [
        ...project.recentIntroRemovals.filter((item) => item.segment.id !== segmentId),
        record,
      ];
      this.bumpTimeline(project, "INTRO");
      this.syncTimelineSubtitles(previous, project, ["INTRO"]);
  ...
  async restoreIntroSegment(segmentId: string): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      if (!record || !project.sources.some((asset) => asset.id === record.segment.assetId))
        throw new Error("找不到可恢復的片頭區段。");
      ...
      if (!project.introSegments.some((item) => item.id === segmentId)) {
        project.introSegments.splice(
          Math.max(0, Math.min(project.introSegments.length, record.previousIndex)),
          0,
          segment,
        );
      }
  ...
  async setPreviewRange(assetId: string, inMs: number, outMs: number): Promise<PreviewRangeUpdateResult> {
    ...
    const project = await this.mutate((project) => {
      ...
      const normalizedIn = Math.round(inMs);
      const normalizedOut = Math.round(outMs);
      if (
        !Number.isFinite(inMs) ||
        !Number.isFinite(outMs) ||
        normalizedIn < 0 ||
        normalizedOut > durationMs ||
        normalizedOut - normalizedIn < 100
      )
        throw new Error("裁切範圍必須位於影片內，且至少保留 0.1 秒。");
      ...
      const updated = {
        ...asset,
        previewRange,
        volumeSegments: clipped.segments,
        mainExclusionRanges: clippedExclusions.ranges,
        zoomSegments: clippedZooms.segments,
      };
      ...
      result = {
        asset: clone(updated),
        adjustedVolumeSegmentCount: clipped.adjustedCount,
        adjustedMainExclusionRangeCount: clippedExclusions.adjustedCount,
        adjustedZoomSegmentCount: clippedZooms.adjustedCount,
      };
  ...
  async addMediaInsertion(
    ...
    return this.mutate((project) => {
      ...
      const normalized = validateMediaInsertion(
        {
          anchorVideoAssetId,
          insertedAssetId,
          atMs,
          sourceInMs: sourceRange?.inMs ?? defaultRange.inMs,
          sourceOutMs: sourceRange?.outMs ?? defaultRange.outMs,
        },
        project.sources,
  ...
  async updateMediaInsertion(insertionId: string, atMs: number, sourceRange: PreviewRange): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      const normalized = validateMediaInsertion(
        { ...current, atMs, sourceInMs: sourceRange.inMs, sourceOutMs: sourceRange.outMs },
        project.sources,
        project.mediaInsertions,
        current.id,
      );
      ...
      if (
        ...
        if (insertion.previousPlacement === "PENDING") {
          const restoreIndex = Math.max(
            0,
            Math.min(project.pendingAssetIds.length, insertion.previousPendingIndex ?? project.pendingAssetIds.length),
          );
          project.pendingAssetIds.splice(restoreIndex, 0, insertion.insertedAssetId);
        } else {
          const stillHiddenBefore = project.mediaInsertions.filter(
            (item) =>
              item.previousPlacement !== "PENDING" && item.previousTimelineIndex < insertion.previousTimelineIndex,
          ).length;
          const restoreIndex = insertion.previousTimelineIndex - stillHiddenBefore;
          project.timelineOrder.splice(
            Math.max(0, Math.min(project.timelineOrder.length, restoreIndex)),
            0,
            insertion.insertedAssetId,
          );
        }
      }
  ...
  async moveMediaInsertion(insertionId: string, toIndex: number): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      const siblings = project.mediaInsertions
        .filter((item) => item.anchorVideoAssetId === insertion.anchorVideoAssetId)
        .sort(
          (left, right) =>
      ...
      if (!Number.isInteger(toIndex) || toIndex < 0 || toIndex >= siblings.length)
        throw new Error("安插素材順序超出範圍。");
      const [moved] = siblings.splice(fromIndex, 1);
      siblings.splice(toIndex, 0, moved);
      siblings.forEach((item, index) => {
        const projectIndex = project.mediaInsertions.findIndex((candidate) => candidate.id === item.id);
        project.mediaInsertions[projectIndex] = { ...item, sequenceIndex: index };
      });
      this.bumpTimeline(project);
      this.syncTimelineSubtitles(previous, project, ["MAIN"]);
    });
  ...
  async setVolumeSegments(assetId: string, segments: VolumeSegment[]): Promise<SourceAsset> {
    ...
    await this.mutate((project) => {
      ...
      updated = {
        ...asset,
        volumeSegments: validateVolumeSegments(segments, asset.previewRange ?? { inMs: 0, outMs: durationMs }),
      };
      project.sources[index] = updated;
    });
  ...
  async setZoomSegments(assetId: string, segments: ZoomSegment[]): Promise<ZoomSegmentUpdateResult> {
    ...
    const project = await this.mutate((project) => {
      ...
      updated = {
        ...asset,
        zoomSegments: validateZoomSegments(segments, asset.previewRange ?? { inMs: 0, outMs: durationMs }),
      };
      project.sources[index] = updated;
      this.bumpTimeline(project);
    });
  ...
  async placeAsset(assetId: string, placement: PlacementRequest): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      if (!(["FRONT", "END", "BEFORE", "AFTER", "PENDING"] as const).includes(placement.action))
        throw new Error("安插位置選項無效。");
      project.timelineOrder = project.timelineOrder.filter((id) => id !== assetId);
      project.pendingAssetIds = project.pendingAssetIds.filter((id) => id !== assetId);
      ...
      project.placementDecisions.push({
        assetId,
        action: placement.action,
        anchorAssetId: placement.anchorAssetId,
  ...
  async moveTimelineAsset(assetId: string, toIndex: number): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      project.timelineOrder.splice(fromIndex, 1);
      project.timelineOrder.splice(normalized, 0, assetId);
      project.sortMode = "MANUAL_ORDER";
      this.bumpTimeline(project);
      this.syncTimelineSubtitles(previous, project, ["MAIN"]);
    });
  ...
  async setSortMode(sortMode: SortMode): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      if (sortMode !== "MANUAL_ORDER") {
        ...
        project.timelineOrder = sortAssets(placed, sortMode).map((asset) => asset.id);
        this.bumpTimeline(project);
        this.syncTimelineSubtitles(previous, project, ["MAIN"]);
      }
    });
  ...
  async addBgmTracks(tracks: BgmTrack[]): Promise<ProjectManifest> {
    return this.mutate((project) => {
      for (const track of tracks) project.bgmTracks.push(validateBgmTrack(track));
    });
  }
  async resolveBgmTrack(trackId: string, resolved: BgmTrack): Promise<ProjectManifest> {
    return this.mutate((project) => {
      const index = project.bgmTracks.findIndex((track) => track.id === trackId);
      if (index < 0) throw new Error("找不到待補音檔的 YouTube 配樂參考。");
      if (project.bgmTracks[index].resolutionStatus !== "NEEDS_LOCAL_FILE")
        throw new Error("這個配樂項目已經有可用的本機音檔。");
      project.bgmTracks[index] = validateBgmTrack({
        ...resolved,
        id: trackId,
        sourceUrl: project.bgmTracks[index].sourceUrl,
        rightsConfirmed: true,
      });
      let cursor = 0;
      for (const track of project.bgmTracks) {
        if (track.resolutionStatus === "NEEDS_LOCAL_FILE") {
          track.timelineInMs = cursor;
          track.timelineOutMs = cursor;
          continue;
        }
        const span = track.sourceOutMs - track.sourceInMs;
        track.timelineInMs = cursor;
        track.timelineOutMs = cursor + span;
        cursor += span;
      }
    });
  }
  async updateBgmTrack(track: BgmTrack): Promise<ProjectManifest> {
    return this.mutate((project) => {
      const index = project.bgmTracks.findIndex((item) => item.id === track.id);
      if (index < 0) throw new Error("找不到配樂項目。");
      const existing = project.bgmTracks[index];
      project.bgmTracks[index] = validateBgmTrack({
        ...existing,
        sourceInMs: track.sourceInMs,
        sourceOutMs: track.sourceOutMs,
        timelineInMs: track.timelineInMs,
        timelineOutMs: track.timelineOutMs,
  ...
  async removeBgmTrack(trackId: string): Promise<ProjectManifest> {
    return this.mutate((project) => {
      project.bgmTracks = project.bgmTracks.filter((track) => track.id !== trackId);
    });
  }
  async moveBgmTrack(trackId: string, toIndex: number): Promise<ProjectManifest> {
    return this.mutate((project) => {
      const fromIndex = project.bgmTracks.findIndex((track) => track.id === trackId);
      if (fromIndex < 0) throw new Error("找不到配樂項目。");
      const [track] = project.bgmTracks.splice(fromIndex, 1);
      project.bgmTracks.splice(Math.max(0, Math.min(project.bgmTracks.length, Math.round(toIndex))), 0, track);
      let cursor = 0;
      for (const item of project.bgmTracks) {
        item.timelineInMs = cursor;
        if (item.resolutionStatus === "NEEDS_LOCAL_FILE") {
          item.timelineOutMs = cursor;
          continue;
        }
        item.timelineOutMs = cursor + item.sourceOutMs - item.sourceInMs;
        cursor = item.timelineOutMs;
      }
    });
  }
  async sequenceBgmTracks(): Promise<ProjectManifest> {
    return this.mutate((project) => {
      if (project.bgmTracks.some((track) => track.resolutionStatus === "NEEDS_LOCAL_FILE"))
        throw new Error("仍有 YouTube 參考尚未指定自有或已授權的本機 MP3，無法連續排列。");
      let cursor = 0;
      for (const track of project.bgmTracks) {
        const span = track.sourceOutMs - track.sourceInMs;
        track.timelineInMs = cursor;
  ...
  async setSourceAudioVolume(volumePercent: number): Promise<ProjectManifest> {
    return this.mutate((project) => {
      project.sourceAudioVolumePercent = validatePercent(volumePercent, "素材原音音量");
    });
  }
  async setSubtitleCues(cues: SubtitleCue[]): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      if (
        normalized.some((cue) =>
          cue.timelineScope === "INTRO" ? cue.endMs > introDurationMs : cue.endMs > mainDurationMs,
        )
      )
        throw new Error("字幕時間超出目前影片或所選片頭總時間，請調整 cue 或先完成影片順序。");
      const previousByScope = (scope: SubtitleTimelineScope) =>
        project.subtitleCues.filter((cue) => (cue.timelineScope ?? "MAIN") === scope);
      const nextByScope = (scope: SubtitleTimelineScope) =>
        normalized.filter((cue) => (cue.timelineScope ?? "MAIN") === scope);
  ...
  async forceSubtitleReReview(scopes: SubtitleTimelineScope[]): Promise<ProjectManifest> {
    return this.mutate((project) => {
      if (
        ...
        scopes.length < 1 ||
        scopes.some((scope) => scope !== "INTRO" && scope !== "MAIN")
      ) {
        throw new Error("強制重新校對至少需要選擇片頭或正片。");
      }
      const selected = new Set(scopes);
      const affected = project.subtitleCues.filter(
        (cue) => selected.has(cue.timelineScope ?? "MAIN") && cue.reviewStatus !== "REJECTED",
      );
      ...
      project.subtitleCues = project.subtitleCues.map((cue) =>
        selected.has(cue.timelineScope ?? "MAIN") && cue.reviewStatus !== "REJECTED"
          ? { ...cue, reviewStatus: "DRAFT", aiWarnings: [...new Set([...(cue.aiWarnings ?? []), warning])] }
          : cue,
      );
      if (selected.has("MAIN"))
        project.mainSubtitleReviewRevision = (project.mainTimelineRevision ?? project.timelineRevision) - 1;
      if (selected.has("INTRO"))
        project.introSubtitleReviewRevision = (project.introTimelineRevision ?? project.timelineRevision) - 1;
      project.subtitleTimelineRevision = Math.min(
        project.mainSubtitleReviewRevision ?? project.timelineRevision,
        project.introSubtitleReviewRevision ?? project.timelineRevision,
      );
    });
  }

  async replaceAiSubtitleDrafts(
    cues: SubtitleCue[],
    scopes: SubtitleTimelineScope[] = ["MAIN"],
    mode: "FILL_BLANKS" | "REPLACE_AI_SCOPE" | "PRESERVE_USER_EDITED" = "FILL_BLANKS",
  ): Promise<ProjectManifest> {
    return this.mutate((project) => {
      if (
        !Array.isArray(cues) ||
        cues.some(
          (cue) =>
            (cue.reviewStatus !== "DRAFT" && cue.reviewStatus !== "CONFIRMED") ||
            (cue.origin !== "AI_SPEECH" && cue.origin !== "AI_VISUAL"),
        )
      )
        throw new Error("AI 字幕格式無效。");
      const selectedScopes = new Set(scopes);
      if (!["FILL_BLANKS", "REPLACE_AI_SCOPE", "PRESERVE_USER_EDITED"].includes(mode))
        throw new Error("AI 字幕重生模式無效。");
      ...
      if (
        ...
        throw new Error("AI 字幕時間超出所選片頭或正片總時間。");
      project.subtitleCues = normalized;
      project.subtitleTimelineRevision = project.timelineRevision;
    });
  }
  ...
  async setAiStoryContext(context: AiStoryContext): Promise<ProjectManifest> {
    return this.mutate((project) => {
      ...
      if (
        normalized.introBaseAssetId &&
        !project.sources.some((asset) => asset.id === normalized.introBaseAssetId && asset.kind === "VIDEO")
      )
        throw new Error("指定的片頭基底影片不存在或不是影片。");
  ...
  async setAiPublishAssets(assets: AiPublishAssets): Promise<ProjectManifest> {
    return this.mutate((project) => {
      if (
        ...
        !Array.isArray(assets.titles) ||
        !Array.isArray(assets.thumbnails) ||
        !Array.isArray(assets.chapters)
      )
        throw new Error("AI 發布素材格式無效。");
      if (
        assets.titles.length > 5 ||
        assets.titles.some(
          (title) =>
            !title ||
            typeof title.text !== "string" ||
            !title.text.trim() ||
            youtubeTextLength(title.text.trim()) > 100,
        )
      )
        throw new Error("YouTube 標題候選必須為 1–5 筆，且每筆不可超過 100 字元。");
      if (
        assets.thumbnails.length > 3 ||
        assets.thumbnails.some(
          (thumbnail) =>
            !project.sources.some((source) => source.id === thumbnail.assetId) ||
            !Number.isFinite(thumbnail.sourceTimeMs) ||
            thumbnail.sourceTimeMs < 0,
        )
      )
        throw new Error("縮圖候選必須綁定現有來源素材與有效時間碼。");
      const durationMs = buildTimelinePlan(project).durationMs;
      if (assets.chapters.length && !isValidYoutubeChapterSet(assets.chapters, durationMs))
        throw new Error("章節不符合 YouTube 規則：第一段 00:00、至少三段、時間遞增且每段至少 10 秒。");
      if (
        typeof assets.description !== "string" ||
        assets.description.length > 5_000 ||
        !assets.topicSnapshot ||
        typeof assets.topicSnapshot.topic !== "string"
      )
        throw new Error("AI 發布說明或主題快照格式無效。");
  ...
  private virtualTimelineOrder(project: ProjectManifest): string[] {
    ...
    for (const insertion of project.mediaInsertions
      .filter((item) => item.previousPlacement !== "PENDING")
      .sort(
        (left, right) =>
          left.previousTimelineIndex - right.previousTimelineIndex || left.sequenceIndex - right.sequenceIndex,
      )) {
  ...
  private syncTimelineSubtitles(
    ...
    if (scopes.includes("MAIN") && result.fullyMapped.MAIN)
      project.mainSubtitleReviewRevision = project.mainTimelineRevision ?? project.timelineRevision;
    if (scopes.includes("INTRO") && result.fullyMapped.INTRO)
      project.introSubtitleReviewRevision = project.introTimelineRevision ?? project.timelineRevision;
    if (
      (project.mainSubtitleReviewRevision ?? -1) === (project.mainTimelineRevision ?? project.timelineRevision) &&
  ...
  private emitChanged(previous: ProjectManifest, next: ProjectManifest, forcedSections?: ProjectChangeSection[]): void {
    ...
    const sections =
      ...
      sectionKeys
        .filter(([, keys]) =>
          keys.some(
            (key) =>
              JSON.stringify((previous as unknown as Record<string, unknown>)[key]) !==
              JSON.stringify((next as unknown as Record<string, unknown>)[key]),
          ),
        )
        .map(([section]) => section);
    ...
    const event: ProjectChangedEvent = {
      projectId: next.id,
      revision: next.timelineRevision,
      updatedAt: next.updatedAt,
      changedSections: sections,
      project: clone(next),
      filePath: this.activeProjectPath,
    };
  ...
  private validateIntroSegments(project: ProjectManifest, segments: IntroSuggestion[]): IntroSuggestion[] {
    if (!Array.isArray(segments) || segments.length > INTRO_MAX_SEGMENTS)
      throw new Error(`片頭最多只能保留 ${INTRO_MAX_SEGMENTS} 段。`);
    const normalized = segments.map((segment) => {
      const asset = project.sources.find((item) => item.id === segment?.assetId);
      if (!segment || typeof segment.id !== "string" || !segment.id || !asset)
      ...
      return {
        ...
        inMs,
        outMs,
        origin: segment.origin === "MANUAL" ? ("MANUAL" as const) : ("AI" as const),
