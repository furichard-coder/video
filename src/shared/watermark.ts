...
function normalizeText(raw: unknown, fallback: WatermarkTextSettings, field: string): WatermarkTextSettings {
  ...
  if (!text || text.length > 80 || text.split("\n").length > 2 || text.includes("\0")) throw new Error(`${field}須為 1–80 個字，最多兩行。`);
  const position = input.position === "LOWER_RIGHT" ? "LOWER_RIGHT" : input.position === "LOWER_LEFT" ? "LOWER_LEFT" : fallback.position;
  const layout = input.layout === "SINGLE_LINE" ? "SINGLE_LINE" : input.layout === "STACKED_TWO_LINES" ? "STACKED_TWO_LINES" : fallback.layout;
...
export function normalizeWatermarkSettings(raw: unknown): WatermarkSettings {
  ...
  const settings: WatermarkSettings = {
    ...
    visibleDurationSeconds: finiteNumber(input.visibleDurationSeconds, DEFAULT_WATERMARK_SETTINGS.visibleDurationSeconds),
    ...
    textOpacityPercent: Math.round(finiteNumber(input.textOpacityPercent, DEFAULT_WATERMARK_SETTINGS.textOpacityPercent)),
  ...
  if (settings.intervalSeconds < 5 || settings.intervalSeconds > 86_400) throw new Error("每次顯示週期須介於 5–86,400 秒。");
  if (settings.visibleDurationSeconds < 1 || settings.visibleDurationSeconds > settings.intervalSeconds) throw new Error("顯示時間須至少 1 秒，且不可超過顯示週期。");
  if (settings.fadeInSeconds < 0 || settings.fadeOutSeconds < 0 || settings.fadeInSeconds + settings.fadeOutSeconds > settings.visibleDurationSeconds) throw new Error("淡入加淡出時間不可超過浮水印顯示時間。");
  if (settings.textOpacityPercent < 0 || settings.textOpacityPercent > 100 || settings.boxOpacityPercent < 0 || settings.boxOpacityPercent > 100) throw new Error("文字與底框透明度須介於 0–100%。");
  if (settings.safeMargin1080p < 12 || settings.safeMargin1080p > 300) throw new Error("安全邊距須介於 12–300 px（1080p 基準）。");
  if (settings.chinese.position === settings.english.position) throw new Error("中文與英文浮水印需分置左下與右下，避免互相重疊。");
  return settings;
}
