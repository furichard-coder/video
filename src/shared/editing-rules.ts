import { DEFAULT_BGM_VOLUME_PERCENT, DEFAULT_IMAGE_DURATION_MS, DEFAULT_ZOOM_ENHANCEMENT_PRESET, MAX_IMAGE_DURATION_MS, MAX_MIX_VOLUME_PERCENT, MAX_ZOOM_PERCENT, MIN_IMAGE_DURATION_MS, MIN_ZOOM_PERCENT, PHOTO_INSERTION_EDGE_MARGIN_MS, type BgmTrack, type MainExclusionRange, type MediaInsertion, type PreviewRange, type ProjectManifest, type RenderClipSelection, type SourceAsset, type SubtitleCue, type VolumeSegment, type ZoomEnhancementPreset, type ZoomSegment } from "./domain";
...
export function imageDurationMs(asset: SourceAsset): number {
  ...
  try { return validateImageDurationMs(asset.imageDurationMs ?? DEFAULT_IMAGE_DURATION_MS); }
  catch { return DEFAULT_IMAGE_DURATION_MS; }
}

export function validatePercent(value: number, label = "音量"): number {
  if (!Number.isFinite(value) || value < 0 || value > MAX_MIX_VOLUME_PERCENT) throw new Error(`${label}必須介於 0% 到 ${MAX_MIX_VOLUME_PERCENT}%。`);
...
export function validateVolumeSegments(segments: VolumeSegment[], range: PreviewRange): VolumeSegment[] {
  ...
  const normalized = segments.map((segment) => {
    ...
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || startMs < range.inMs || endMs > range.outMs || endMs - startMs < 10) {
  ...
  for (let index = 1; index < normalized.length; index += 1) {
    if (normalized[index].startMs < normalized[index - 1].endMs) throw new Error("音量區段不可重疊；請調整開始或結束時間。");
  }
  return normalized;
}

export function clipVolumeSegments(segments: VolumeSegment[], range: PreviewRange): { segments: VolumeSegment[]; adjustedCount: number } {
  let adjustedCount = 0;
...
export function normalizeMainExclusionRanges(ranges: MainExclusionRange[], allowed: PreviewRange): MainExclusionRange[] {
  ...
  const normalized = ranges.map((range) => {
    ...
    if (!Number.isFinite(range.startMs) || !Number.isFinite(range.endMs) || startMs < allowed.inMs || endMs > allowed.outMs || endMs <= startMs) {
...
export function clipMainExclusionRanges(ranges: MainExclusionRange[], allowed: PreviewRange): { ranges: MainExclusionRange[]; adjustedCount: number } {
...
export function validateZoomSegments(segments: ZoomSegment[], allowed: PreviewRange): ZoomSegment[] {
  ...
  const normalized = segments.map((segment) => {
    ...
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || startMs < allowed.inMs || endMs > allowed.outMs || endMs - startMs < 100) {
    ...
    if (!Number.isFinite(centerXPercent) || centerXPercent < 0 || centerXPercent > 100 || !Number.isFinite(centerYPercent) || centerYPercent < 0 || centerYPercent > 100) {
  ...
  for (let index = 1; index < normalized.length; index += 1) {
    if (normalized[index].startMs < normalized[index - 1].endMs) throw new Error("局部放大區段不可重疊；請調整開始或結束時間。");
  }
  return normalized;
}

export function clipZoomSegments(segments: ZoomSegment[], allowed: PreviewRange): { segments: ZoomSegment[]; adjustedCount: number } {
...
export function validateMediaInsertion(
  ...
  if (!video || video.kind !== "VIDEO" || !video.mediaInfo?.durationMs) throw new Error("素材安插的主影片尚未取得有效時長。");
  ...
  const retainedRange = retainedRangesForAsset(video).find((range) =>
    atMs - range.inMs >= PHOTO_INSERTION_EDGE_MARGIN_MS && range.outMs - atMs >= PHOTO_INSERTION_EDGE_MARGIN_MS,
  );
  if (!retainedRange) throw new Error("素材安插點必須位於主影片保留範圍內，並距離該段起點與終點至少 0.75 秒，以保留 0.7 秒疊化空間。");
  ...
  if (existing.some((item) => item.id !== ignoreInsertionId && item.insertedAssetId === inserted.id)) throw new Error("這個素材已安插到其他影片時段中。");
  if (existing.some((item) => item.id !== ignoreInsertionId && item.anchorVideoAssetId === inserted.id)) throw new Error("此影片本身已有安插內容，為避免巢狀時間線，請先取消其安插內容。");
  ...
  if (!Number.isFinite(insertion.sourceInMs) || !Number.isFinite(insertion.sourceOutMs) || sourceInMs < 0 || sourceOutMs > durationMs || sourceOutMs - sourceInMs < PHOTO_INSERTION_EDGE_MARGIN_MS) {
  ...
  return project.timelineOrder.flatMap((assetId) => {
    ...
    for (const retained of retainedRangesForAsset(asset)) {
      ...
      for (const insertion of within) {
        ...
        if (inserted) clips.push({ assetId: inserted.id, inMs: insertion.sourceInMs, outMs: insertion.sourceOutMs, mediaInsertionId: insertion.id });
...
export function validateBgmTrack(track: BgmTrack): BgmTrack {
  if ((track.resolutionStatus === "NEEDS_LOCAL_FILE" || track.sourceKind === "YOUTUBE_REFERENCE") && !track.sourcePath) {
    let sourceUrl: URL;
    try { sourceUrl = new URL(track.sourceUrl ?? ""); } catch { throw new Error("YouTube 配樂參考連結無效。"); }
    const host = sourceUrl.hostname.toLowerCase().replace(/^www\./, "");
    if (sourceUrl.protocol !== "https:" || !["youtube.com", "m.youtube.com", "music.youtube.com", "youtu.be"].includes(host)) throw new Error("只接受 HTTPS YouTube／youtu.be 配樂參考連結。");
    return {
      ...track,
  ...
  const fields = [track.durationMs, track.sourceInMs, track.sourceOutMs, track.timelineInMs, track.timelineOutMs, track.fadeInMs, track.fadeOutMs];
  if (fields.some((value) => !Number.isFinite(value))) throw new Error("配樂時間格式無效。");
  if (track.sourceInMs < 0 || track.sourceOutMs > track.durationMs || track.sourceOutMs <= track.sourceInMs) throw new Error("配樂 source IN／OUT 超出音樂長度。");
  ...
  if (Math.abs(sourceSpan - timelineSpan) > 50) throw new Error("配樂 timeline 長度必須與 source IN／OUT 長度相同；本版不做音樂變速。");
  if (track.fadeInMs < 0 || track.fadeOutMs < 0 || track.fadeInMs + track.fadeOutMs > timelineSpan) throw new Error("淡入與淡出總長不可超過配樂區段。");
  return { ...track, sourceKind: track.sourceKind ?? "LOCAL_FILE", resolutionStatus: "READY", volumePercent: validatePercent(track.volumePercent, "配樂音量") };
}
...
export function validateSubtitleCues(cues: SubtitleCue[]): SubtitleCue[] {
  ...
  const normalized = cues.map((cue) => {
    ...
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || startMs < 0 || endMs <= startMs) throw new Error("字幕結束時間必須晚於開始時間。");
    ...
    const list = (value: string[] | undefined) => value === undefined ? undefined : value.filter((item): item is string => typeof item === "string").map((item) => item.trim()).filter(Boolean).slice(0, 20);
    return {
      ...cue,
      startMs,
      endMs,
