...
- 單筆、批量及強制逐筆校對共用即時保存契約；寫入失敗不會跳到下一筆，也不會假顯示已保存。
- 上下移以交換同 scope 相鄰時間／來源 slot 定義，保持時間排序為事實來源；兩筆一律回到 `DRAFT`，避免改變畫面對位後沿用舊的人工確認。
- Native textarea 与编号选择控制维持分离，Delete 快捷键仍在文字输入获得焦点时停用。
