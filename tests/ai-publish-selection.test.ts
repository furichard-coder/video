...
function source(id: string, fileName: string): SourceAsset {
  return {
    id: id.repeat(64),
    sourcePath: `C:\\Media\\${fileName}`,
    sourceIdentity: `${id}i`.repeat(64).slice(0, 64),
    fileName,
    extension: ".mp4",
    kind: "VIDEO",
    sizeBytes: 100,
    fileCreatedAt: "2026-09-01T00:00:00.000Z",
    fileModifiedAt: "2026-09-01T00:00:00.000Z",
    addedAt: "2026-09-01T00:00:00.000Z",
    addedOrder: 0,
    sourcePolicy: "READ_ONLY",
    previewPolicy: "DERIVED_CACHE_ONLY_NOT_MASTER",
    previewCacheKey: `${id}c`.repeat(64).slice(0, 64),
    metadataState: "READY",
    mediaInfo: { durationMs: 30_000, width: 1920, height: 1080 },
  };
}

describe("AI publish visual candidate selection", () => {
  it("prioritizes the selected Intro time ranges before Main timeline frames", () => {
    const introA = source("a", "片頭人物.mp4");
    const introB = source("b", "片頭景觀.mp4");
    const main = source("c", "正片內容.mp4");
    const project = {
      schemaVersion: 16,
      ...
      sources: [introA, introB, main],
      timelineOrder: [main.id],
      pendingAssetIds: [],
