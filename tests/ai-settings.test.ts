...
async function root(): Promise<string> {
  const value = await mkdtemp(path.join(os.tmpdir(), "source-app-ai-settings-"));
  roots.push(value);
  return value;
}
afterEach(async () => {
...
describe("AI settings", () => {
  it("encrypts API keys separately, restores profiles, and never writes plaintext", async () => {
    ...
    const restored = new AiSettingsStore(dataRoot, protector);
  ...
  it("supports account switching and refuses plaintext storage when encryption is unavailable", async () => {
    ...
    const unsafe = new AiSettingsStore(await root(), {
      isAvailable: () => false,
      protect: () => "",
      unprotect: () => "",
    });
