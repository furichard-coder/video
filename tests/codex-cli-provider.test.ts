...
async function root(): Promise<string> {
  const value = await mkdtemp(path.join(os.tmpdir(), "source-app-codex-"));
  roots.push(value);
  return value;
}
afterEach(async () => {
...
describe("Codex ChatGPT login subtitle adapter", () => {
  it("checks that Codex is specifically logged in with ChatGPT", async () => {
    ...
    const result = await new CodexCliStoryProvider(
      await root(),
    ...
    expect(result).toMatchObject({
      ...
      checks: expect.arrayContaining(["ChatGPT 登入可用"]),
    });
  ...
  it("passes Chinese and spaced storyboard paths as argument-array items, uses stdin, and removes the temporary schema", async () => {
    ...
    const framePath = path.join(cacheRoot, "大雪山 片段 01.jpg");
    await writeFile(framePath, "frame");
    const agentText = JSON.stringify({
      results: [
        {
          index: 0,
          suggestedSubtitle: "林下植被有助涵養水分",
          visualSummary: "森林步道",
          eventSummary: "步行觀察",
          peopleSummary: [],
          locationSummary: ["大雪山"],
          animalSpecies: ["疑似山羌"],
          speciesExplanation: "森林底層常是山羌活動環境。",
    ...
    const runner = vi.fn(async (_executable, args: string[], _signal, options) => {
      ...
      return {
        stdout: `${JSON.stringify({ type: "item.completed", item: { type: "agent_message", text: agentText } })}\n`,
        stderr: "",
      };
    });
    const provider = new CodexCliStoryProvider(cacheRoot, "C:\\Codex\\codex.exe", runner);
    const result = await provider.analyzeStoryFramesBatch(
      [
        {
          id: "candidate-1",
          framePath,
          request: { mode: "SUBTITLE", transcript: "", context, sourceFileName: "森林 01.MOV" },
        },
      ],
      "gpt-5.6-sol",
    );
    expect(result.get("candidate-1")).toMatchObject({
      suggestedSubtitle: "林下植被有助涵養水分",
      animalSpecies: ["疑似山羌"],
      confidence: 84,
    });
  ...
  it("rejects missing, duplicate or incomplete structured results instead of silently saving them", async () => {
    const cacheRoot = await root();
    const framePath = path.join(cacheRoot, "frame.jpg");
    await writeFile(framePath, "frame");
    const runner = vi.fn(async () => ({
      stdout: `${JSON.stringify({ type: "item.completed", item: { type: "agent_message", text: JSON.stringify({ results: [] }) } })}\n`,
      stderr: "",
    }));
    const provider = new CodexCliStoryProvider(cacheRoot, "C:\\Codex\\codex.exe", runner);
    await expect(
      provider.analyzeStoryFramesBatch(
        [
          {
            id: "candidate-1",
            framePath,
            request: { mode: "SUBTITLE", transcript: "", context, sourceFileName: "forest.mov" },
          },
        ],
        "gpt-5.6-sol",
      ),
    ).rejects.toThrow(/應回傳 1 筆/);
  });
      ...
      suggestions: [
        {
          ...
          artist: "A",
          reason: "城市步行",
          auditionUrl: "https://www.youtube.com/watch?v=abc123XY1",
          evidenceUrl: "https://www.youtube.com/watch?v=abc123XY1",
          rightsEvidence: "YouTube 音樂頁，需確認授權",
        ...
        },
        {
          ...
          artist: "B",
          reason: "人物故事",
          auditionUrl: "https://www.youtube.com/watch?v=abc123XY2",
          evidenceUrl: "https://www.youtube.com/watch?v=abc123XY2",
          rightsEvidence: "Creative Commons 線索，需確認條款",
          observedAt: "2026-09-08",
          trendEvidence: "",
        },
        {
          ...
          artist: "C",
          reason: "旅程節奏",
          auditionUrl: "https://www.youtube.com/watch?v=abc123XY3",
          evidenceUrl: "https://www.youtube.com/watch?v=abc123XY3",
          rightsEvidence: "作者授權說明，需確認條款",
          observedAt: "2026-09-08",
          trendEvidence: "",
        },
      ],
    ...
    const runner = vi.fn(async (_executable, args: string[], _signal, options) => {
      expect(args).toEqual(
        expect.arrayContaining([
          ...
          "read-only",
          "--output-schema",
          expect.stringContaining("codex-music-schema"),
          "--json",
          "-",
        ]),
      );
