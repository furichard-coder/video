...
function photo(sourcePath: string, idChar = "p"): SourceAsset {
  return {
    ...asset(sourcePath, idChar),
    fileName: path.basename(sourcePath),
    extension: ".jpg",
    kind: "IMAGE",
    mediaInfo: { width: 1080, height: 1920, displayWidth: 1080, displayHeight: 1920, isPortrait: true },
    metadataState: "READY",
  };
}
...
describe("ProjectStore", () => {
  it("undoes and redoes project-only changes while never deleting source files", async () => {
    ...
    const sourcePath = path.join(root, "復原來源.mp4");
    await writeFile(sourcePath, "irreplaceable-source");
  ...
  it("persists the last AI music suggestions with the project and restores them after reopen", async () => {
    ...
    const result = {
      ...
      suggestions: [1, 2, 3].map((index) => ({
        ...
        artist: "測試作者",
        reason: "適合城市步行",
        auditionUrl: `https://www.youtube.com/watch?v=music${index}`,
        evidenceUrl: `https://www.youtube.com/watch?v=music${index}`,
  ...
  it("removes only the main reference, preserves settings, and never deletes the source", async () => {
    ...
    const source = {
      ...
      volumeSegments: [{ id: "gain", startMs: 100, endMs: 500, volumePercent: 120 }],
    };
    ...
    expect(persistedRemoval.timelineOrder).toEqual([]);
    expect(persistedRemoval.recentMainRemovals[0].assetId).toBe(source.id);
  ...
  it("defaults photos to five seconds, persists three-to-seven-second changes, and never changes source bytes", async () => {
    const root = await tempRoot();
    const sourcePath = path.join(root, "直式照片.jpg");
  ...
  it("saves a named complete project file, keeps it current, and restores the active project on reopen", async () => {
    ...
    const projectPath = path.join(root, "我的 河內專案.swproj");
    const store = new ProjectStore(dataRoot);
    await store.initialize();
    ...
    const reopened = new ProjectStore(dataRoot);
  ...
  it("opens an explicitly selected old project and leaves the current project unchanged when the file is invalid", async () => {
    ...
    const oldProject = {
      ...
      id: "old-project",
      name: "舊專案",
      sources: [photo(path.join(root, "old.jpg"), "o")],
      timelineOrder: ["o".repeat(64)],
      pendingAssetIds: [],
    };
    ...
    const invalidPath = path.join(root, "損壞.swproj");
    const invalidBytes = '{"schemaVersion":999,"important":true}';
  ...
  it("migrates a v0.4 schema-1 manifest without losing sources", async () => {
    ...
    const legacy = {
      ...
      sourcePolicy: "READ_ONLY",
      previewPolicy: "DERIVED_CACHE_ONLY_NOT_MASTER",
      previewerVersion: "preview-v1",
      sortMode: "ADDED_ORDER",
  ...
  it("migrates schema 2 to schema 10 and preserves every existing edit field", async () => {
    ...
    await mkdir(path.dirname(manifestPath), { recursive: true });
    const source = {
      ...asset(path.join(root, "schema2.mp4")),
      volumeSegments: [{ id: "gain", startMs: 0, endMs: 500, volumePercent: 120 }],
    };
    const schema2 = {
      schemaVersion: 2,
      id: "schema2",
      ...
      pendingAssetIds: [],
      placementDecisions: [{ assetId: source.id, action: "END", decidedAt: "2026-01-01T00:00:00.000Z" }],
      bgmTracks: [],
      subtitleCues: [],
      timelineRevision: 4,
      subtitleTimelineRevision: 4,
    ...
    expect(migrated).toMatchObject({
      ...
      subtitleTimelineRevision: 4,
      sourceAudioVolumePercent: 100,
      mainTimelineRevision: 4,
      introTimelineRevision: 4,
      mainSubtitleReviewRevision: 4,
      introSubtitleReviewRevision: 4,
    ...
    expect(migrated.sources[0].mainExclusionRanges).toEqual([]);
    expect(migrated.introSegments).toEqual([]);
    expect(migrated.recentMainRemovals).toEqual([]);
  });
  ...
  it("migrates a schema-3 photo to the current schema with the five-second and shutter defaults", async () => {
    ...
    const manifestPath = path.join(root, "projects", "default", "project.source-manifest.json");
    await mkdir(path.dirname(manifestPath), { recursive: true });
    const legacyPhoto = photo(path.join(root, "legacy-photo.jpg"));
    delete legacyPhoto.imageDurationMs;
    const schema3 = {
      schemaVersion: 3,
      id: "schema3-photo",
      name: "舊照片專案",
  ...
  it("migrates schema 5 to current schema without losing legacy subtitles", async () => {
    ...
    await mkdir(path.dirname(manifestPath), { recursive: true });
    const source = {
      ...asset(path.join(root, "legacy-ai.mp4")),
      metadataState: "READY" as const,
    ...
    const schema5 = {
      ...
      photoInsertions: [],
      bgmTracks: [],
      subtitleCues: [{ id: "legacy-cue", startMs: 0, endMs: 1_000, text: "既有字幕" }],
      timelineRevision: 2,
  ...
  it("migrates schema 10 while preserving an existing Intro segment longer than the new 15-second default", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "legacy-intro.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 60_000 },
    };
    await store.addAssets([source]);
  ...
  it("validates volume automation and clips it explicitly when IN/OUT shrinks", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "audio.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 10_000, audioCodec: "aac" },
    };
    await store.addAssets([source]);
    ...
    await expect(
      store.setVolumeSegments(source.id, [{ id: "bad", startMs: 0, endMs: 1000, volumePercent: 301 }]),
    ).rejects.toThrow(/0% 到 300%/);
    await expect(
      store.setVolumeSegments(source.id, [
        { id: "one", startMs: 0, endMs: 2000, volumePercent: 100 },
        { id: "two", startMs: 1900, endMs: 3000, volumePercent: 100 },
      ]),
    ).rejects.toThrow(/不可重疊/);
  ...
  it("persists local zoom settings and safely clips them when IN/OUT shrinks", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "zoom.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 10_000, videoCodec: "h264" },
    };
    await store.addAssets([source]);
    ...
    await expect(
      store.setZoomSegments(source.id, [
        { id: "bad", startMs: 0, endMs: 1_000, zoomPercent: 401, centerXPercent: 50, centerYPercent: 50 },
      ]),
    ).rejects.toThrow(/100% 到 400%/);
  ...
  it("places supplemental assets one by one and persists manual reordering", async () => {
    ...
    const store = new ProjectStore(root);
    await store.initialize();
    const a = asset(path.join(root, "a.mp4"), "a");
    const b = asset(path.join(root, "b.mp4"), "b");
    const c = asset(path.join(root, "c.mp4"), "c");
    await store.addAssets([a]);
    let project = await store.addAssets([b, c]);
    expect(project.timelineOrder).toEqual([a.id]);
    expect(project.pendingAssetIds).toEqual([b.id, c.id]);
    ...
    expect(project.timelineOrder).toEqual([b.id, a.id, c.id]);
    expect(project.sortMode).toBe("MANUAL_ORDER");
    const restored = new ProjectStore(root);
    expect((await restored.initialize()).timelineOrder).toEqual([b.id, a.id, c.id]);
  });
  ...
  it("moves and inserts photos while keeping their subtitles attached with transition overlap", async () => {
    ...
    expect(updated.subtitleCues[0]).toMatchObject({
      id: "b-caption",
      startMs: 9_900,
      endMs: 10_900,
      reviewStatus: "CONFIRMED",
  ...
  it("marks saved subtitles for review after timeline or trim changes", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "source.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 5_000 },
    };
    await store.addAssets([source]);
    ...
    expect(project.subtitleTimelineRevision).toBe(project.timelineRevision);
    expect(project.subtitleCues[0]).toMatchObject({
      startMs: 0,
      endMs: 1_000,
      sourceAssetId: source.id,
      sourceInMs: 0,
    });
    await expect(store.setSubtitleCues([{ id: "late", startMs: 3500, endMs: 4500, text: "超出" }])).rejects.toThrow(
      /超出目前影片/,
    );
  });
  ...
  it("keeps Intro and Main subtitle timebases independent", async () => {
    const root = await tempRoot();
    const sourcePath = path.join(root, "subtitle-scopes.mp4");
    await writeFile(sourcePath, "safe");
    const store = new ProjectStore(path.join(root, "app-data"));
    await store.initialize();
    ...
    await expect(
      store.setSubtitleCues([
        { id: "late-intro", startMs: 2_000, endMs: 3_500, text: "超出片頭", timelineScope: "INTRO" },
      ]),
    ).rejects.toThrow(/所選片頭/);
  });

  it("forces the selected subtitle scope back to review without changing text or time", async () => {
    const root = await tempRoot();
    const sourcePath = path.join(root, "force-review.mp4");
    await writeFile(sourcePath, "safe");
    const store = new ProjectStore(path.join(root, "app-data"));
    await store.initialize();
    const source = { ...asset(sourcePath), metadataState: "READY" as const, mediaInfo: { durationMs: 5_000 } };
    ...
    expect(forced.subtitleCues.find((cue) => cue.id === "main-confirmed")).toMatchObject({
      startMs: 500,
      endMs: 1_500,
      text: "正片文字",
      reviewStatus: "DRAFT",
    });
    expect(forced.subtitleCues.find((cue) => cue.id === "main-confirmed")?.aiWarnings).toContain(
      "已由使用者強制重新校對；請逐筆播放確認畫面、字幕文字與頭尾時間。",
    );
  ...
  it("sorts and merges main exclusions, clips them after IN/OUT changes, and persists them", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "exclude.mp4")),
      metadataState: "READY" as const,
    ...
    expect(retainedRangesForAsset(saved.asset)).toEqual([
      { inMs: 0, outMs: 2_000 },
      { inMs: 6_000, outMs: 10_000 },
    ]);
    ...
    await expect(
      store.setMainExclusionRanges(source.id, [{ id: "bad", startMs: 2_000, endMs: 4_000 }]),
    ).rejects.toThrow(/目前 IN／OUT/);
  });
  ...
  it("keeps Main and Intro references independent and only restores an explicitly removed segment", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "intro.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 8_000 },
    };
    await store.addAssets([source]);
    const first = {
      id: "intro-1",
  ...
  it("keeps over-limit Intro review ranges while enforcing source, minimum, and count safety", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "long-intro.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 60_000 },
    };
    await store.addAssets([source]);
    ...
    await expect(
      store.setIntroSegments(Array.from({ length: 51 }, (_, index) => segment(`count-${index}`, 0, 3_000))),
    ).rejects.toThrow(/最多只能保留 50 段/);
    await store.setIntroSegmentMaxDuration(22_000);
    await store.setIntroTargetDuration(240_000);
    const overThreeMinutes = await store.setIntroSegments(
      Array.from({ length: 9 }, (_, index) => segment(`total-${index}`, 0, 22_000)),
    );
    ...
    const accepted = await store.setIntroSegments(
      Array.from({ length: 50 }, (_, index) => segment(`ok-${index}`, 0, 3_000)),
    );
  ...
  it("applies one Intro segment maximum and distributes the target duration evenly", async () => {
    ...
    await store.initialize();
    const source = {
      ...asset(path.join(root, "balanced-intro.mp4")),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 90_000 },
    };
    await store.addAssets([source]);
  ...
  it("inserts a photo inside a retained video range, persists it, and restores the former order without touching sources", async () => {
    const root = await tempRoot();
    const dataRoot = path.join(root, "app-data");
    const videoPath = path.join(root, "影片.mp4");
    const photoPath = path.join(root, "照片.jpg");
    await writeFile(videoPath, "video-source");
    await writeFile(photoPath, "photo-source");
    const video = {
      ...asset(videoPath, "v"),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 10_000, videoCodec: "h264", audioCodec: "aac" },
    };
    ...
    expect(project.mediaInsertions).toEqual([
      expect.objectContaining({
        anchorVideoAssetId: video.id,
        insertedAssetId: still.id,
        atMs: 4_000,
        sourceInMs: 0,
        sourceOutMs: 5_000,
        previousTimelineIndex: priorPhotoIndex,
      }),
    ]);
  ...
  it("inserts a newly imported pending-folder asset and returns it to pending when cancelled", async () => {
    const root = await tempRoot();
    const dataRoot = path.join(root, "app-data");
    const hostPath = path.join(root, "主資料夾", "host.mp4");
    const supplementPath = path.join(root, "另一個資料夾", "新照片.png");
    await mkdir(path.dirname(hostPath), { recursive: true });
    await mkdir(path.dirname(supplementPath), { recursive: true });
    await writeFile(hostPath, "host-source");
    await writeFile(supplementPath, "new-photo-source");
    ...
    expect(project.mediaInsertions[0]).toMatchObject({
      insertedAssetId: supplement.id,
      previousPlacement: "PENDING",
      previousPendingIndex: 0,
    });
    expect(mainRenderSelections(project).map((clip) => clip.assetId)).toEqual([host.id, supplement.id, host.id]);
    const reopenedStore = new ProjectStore(dataRoot);
    const reopened = await reopenedStore.initialize();
  ...
  it("rejects media insertion near a cut, inside an excluded region, or with an already inserted asset while allowing consecutive items", async () => {
    ...
    await store.initialize();
    const video = {
      ...asset(path.join(root, "clip.mp4"), "v"),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 8_000 },
    };
    const first = photo(path.join(root, "one.jpg"), "p");
    const second = photo(path.join(root, "two.jpg"), "q");
  ...
  it("inserts a trimmed video and a photo at the same point, reorders the chain, and restores it on reopen", async () => {
    const root = await tempRoot();
    const dataRoot = path.join(root, "app-data");
    const hostPath = path.join(root, "主影片.mp4");
    const insertedPath = path.join(root, "補充影片.MOV");
    const photoPath = path.join(root, "接續照片.jpg");
    await writeFile(hostPath, "host-source");
    await writeFile(insertedPath, "inserted-video-source");
    await writeFile(photoPath, "photo-source");
    const host = {
      ...asset(hostPath, "h"),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 12_000, videoCodec: "h264" },
    };
    const insertedVideo = {
      ...asset(insertedPath, "i"),
      extension: ".mov",
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 9_000, videoCodec: "hevc" },
    };
    const still = photo(photoPath, "s");
    const store = new ProjectStore(dataRoot);
    await store.initialize();
    await store.addAssets([host, insertedVideo, still]);
    ...
    expect(
      mainRenderSelections(project)
        .filter((clip) => clip.mediaInsertionId)
        .map((clip) => clip.assetId),
    ).toEqual([still.id, insertedVideo.id]);
    project = await store.updateMediaInsertion(
      project.mediaInsertions.find((item) => item.insertedAssetId === insertedVideo.id)!.id,
      5_000,
      { inMs: 2_000, outMs: 4_000 },
    );
    expect(mainRenderSelections(project).find((clip) => clip.assetId === insertedVideo.id)).toMatchObject({
      inMs: 2_000,
      outMs: 4_000,
    });
    await expect(
      store.updateMediaInsertion(
        project.mediaInsertions.find((item) => item.insertedAssetId === insertedVideo.id)!.id,
        5_000,
        { inMs: 2_000, outMs: 2_700 },
      ),
    ).rejects.toThrow(/至少保留 0.75 秒/);
    await expect(
      store.updateMediaInsertion(
        project.mediaInsertions.find((item) => item.insertedAssetId === insertedVideo.id)!.id,
        5_000,
    ...
    const restoredStore = new ProjectStore(dataRoot);
    await restoredStore.initialize();
    let restoredProject = await restoredStore.removeMediaInsertion(
      restored.mediaInsertions.find((item) => item.insertedAssetId === insertedVideo.id)!.id,
    );
  ...
  it("migrates legacy photo insertions to schema 10 media insertions without losing their order", async () => {
    ...
    await mkdir(path.dirname(manifestPath), { recursive: true });
    const host = {
      ...asset(path.join(root, "legacy-host.mp4"), "h"),
      metadataState: "READY" as const,
      mediaInfo: { durationMs: 10_000 },
    };
    ...
    const legacy = {
      ...
      photoInsertions: [
        {
          id: "legacy-insertion",
          anchorVideoAssetId: host.id,
          photoAssetId: still.id,
          atMs: 4_000,
