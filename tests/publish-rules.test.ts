...
describe("AI publishing rules", () => {
  ...
  it("validates chapter ordering and minimum spacing", () => {
    const valid = [0, 10_000, 20_000].map((startMs, index) => ({
      id: String(index),
      startMs,
      title: `章節 ${index}`,
      description: "",
  ...
  it("preserves user edits when AI candidates are regenerated", () => {
    const old = [
      { id: "a", text: "人工標題", charCount: 4, reason: "人工", userEdited: true },
      { id: "b", text: "舊草稿", charCount: 3, reason: "AI" },
    ];
    const next = [
      { id: "a", text: "新 AI", charCount: 4, reason: "AI" },
      { id: "c", text: "新候選", charCount: 4, reason: "AI" },
    ];
    expect(mergePublishTitles(old, next, "PRESERVE_USER_EDITED").map((item) => item.text)).toEqual([
      "人工標題",
      "新候選",
    ]);
  ...
  it("builds the exact upload description with English, chapters and hashtags", () => {
    const chapters = [0, 10_000, 20_000].map((startMs, index) => ({
      id: String(index),
      startMs,
      title: `章節 ${index}`,
      description: "",
    }));
    const draft = buildYoutubeDescription({
      description: "森林旅程",
      englishSummary: "Forest journey",
      hashtags: ["#大雪山", "#Taiwan"],
      chapters,
    });
