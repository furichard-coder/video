...
describe("PlatformUploadHandoffService", () => {
  it.each([
    ...
    "opens the official %s login/upload portal without reading credentials or touching files",
  ...
  it.each([
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), "平台 投稿-"));
    roots.push(root);
    const outputPath = path.join(root, "河內 預覽 片.mp4");
    await writeFile(outputPath, "completed-mp4-bytes");
  ...
  it("rejects missing, empty, non-MP4, and unsupported targets before opening anything", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "platform-gate-"));
    roots.push(root);
    const adapter = { openExternal: vi.fn(async () => undefined), showItemInFolder: vi.fn(), copyText: vi.fn() };
    const service = new PlatformUploadHandoffService(adapter);
    const empty = path.join(root, "empty.mp4");
    const wrong = path.join(root, "preview.mov");
    await writeFile(empty, "");
    await writeFile(wrong, "data");
  ...
  it("prepares the completed MP4 for the default Chrome YouTube drag-and-drop handoff without changing it", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "youtube-drag-"));
    roots.push(root);
    const outputPath = path.join(root, "最新 正片.mp4");
    await writeFile(outputPath, "completed-youtube-mp4");
    const before = await sha256(outputPath);
    const adapter = {
      openExternal: vi.fn(async () => undefined),
      openYoutubeInChrome: vi.fn(async () => undefined),
