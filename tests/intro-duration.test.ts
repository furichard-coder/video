...
import {
  balanceIntroSegments,
  introSegmentsForOutput,
  normalizeIntroSegmentMaxDuration,
} from "../src/shared/intro-duration";
...
describe("balanced Intro segment durations", () => {
  ...
  it("only differs when a source is too short and redistributes remaining time predictably", () => {
    ...
    const balanced = balanceIntroSegments(
      [
        segment("s1", "short", 0, 5_000),
        segment("s2", "long-a", 10_000, 15_000),
        segment("s3", "long-b", 20_000, 25_000),
      ],
      assets,
      35_000,
      15_000,
