...
describe("render preflight estimate", () => {
  it("estimates output size/time and preserves a 1 GB reserve", () => {
    ...
    expect(estimate).toMatchObject({ driveRoot: "C:\\", canRender: true, minimumReserveBytes: MINIMUM_RENDER_RESERVE_BYTES });
  ...
  it("blocks when the estimated file would leave less than 1 GB", () => {
    const estimate = buildRenderEstimate("C:\\Output\\preview.mp4", MINIMUM_RENDER_RESERVE_BYTES + 5_000_000, 60_000, "720P", "H265_QSV");
