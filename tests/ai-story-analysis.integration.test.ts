...
beforeAll(async () => {
  root = await mkdtemp(path.join(os.tmpdir(), "source-app-ai-story-"));
  sourcePath = path.join(root, "河內 故事.mp4");
  ...
  await sources.ensureMetadata(store.getProject().sources[0].id);
  await store.setAiStoryContext({
    topic: "河內旅行",
    locations: ["河內"],
    people: ["主持人"],
    storySummary: "抵達後步行探索",
    audiencePromise: "看見城市體驗",
    subtitleLanguage: "zh",
  });
  await store.setSubtitleCues([
    { id: "confirmed", startMs: 0, endMs: 400, text: "人工確認開場", origin: "MANUAL", reviewStatus: "CONFIRMED" },
  ]);
  const protector = {
    isAvailable: () => true,
  ...
  aiSettings = new AiSettingsStore(dataRoot, protector);
  await aiSettings.initialize();
  await aiSettings.saveAccount({
    id: "openai-default",
    name: "測試 OpenAI",
    provider: "OPENAI",
  ...
  service = new AiStoryAnalysisService(
    path.join(dataRoot, "cache", "ai-story"),
    store,
    sources,
    aiSettings,
    provider as unknown as OpenAiProvider,
    codexProvider as unknown as CodexCliStoryProvider,
  );
...
describe("AI story and subtitle pipeline", () => {
  it("maps speech to the timeline, stores reviewable evidence, reuses visual cache, and preserves source bytes", async () => {
    ...
    const phases: string[] = [];
    const first = await service.generateSubtitles({ includeSpeechTranscription: true }, undefined, (progress) =>
      phases.push(progress.phase),
    );
    expect(first.analysisVersion).toBe(AI_STORY_ANALYZER_VERSION);
    expect(first.generatedCount).toBe(2);
    expect(first.transcribedCount).toBe(2);
    expect(first.project.subtitleCues[0]).toMatchObject({ id: "confirmed", reviewStatus: "CONFIRMED" });
    expect(first.project.subtitleCues.slice(1)).toEqual(
    ...
    const calls = analyzeStoryFrames.mock.calls.length;
    const second = await service.generateSubtitles({ includeSpeechTranscription: true });
    expect(second.generatedCount).toBe(0);
    expect(analyzeStoryFrames).toHaveBeenCalledTimes(calls);
  ...
  it("honors cancellation before reading media", async () => {
    const readinessCalls = provider.testStoryConnection.mock.calls.length;
    const controller = new AbortController();
    controller.abort();
    await expect(
      service.generateSubtitles({ includeSpeechTranscription: false }, controller.signal),
  ...
  it("fails quota preflight before probing or extracting any source frame", async () => {
    const ensureMetadata = vi.spyOn(sources, "ensureMetadata");
    ensureMetadata.mockClear();
    const unavailableProvider = {
      ...provider,
      testStoryConnection: vi.fn(async () => {
    ...
    const unavailable = new AiStoryAnalysisService(
      path.join(root, "app-data", "cache", "ai-story-unavailable"),
      store,
      sources,
      aiSettings,
      unavailableProvider as unknown as OpenAiProvider,
      unavailableCodex as unknown as CodexCliStoryProvider,
    );
    await unavailable.initialize();
    await expect(
      unavailable.generateSubtitles({ includeSpeechTranscription: false, scopes: ["INTRO"] }),
    ).rejects.toThrow(/尚未開始分析.*credit_balance_exhausted.*Codex not logged in/);
  ...
  it("falls back to the signed-in Codex ChatGPT account when the API project has no credit", async () => {
    ...
    const codex = {
      testConnection: vi.fn(async () => ({ checks: ["ChatGPT 登入可用"], providerLabel: "Codex／ChatGPT 登入" })),
      analyzeStoryFramesBatch: vi.fn(
        async (requests: Array<{ id: string }>) =>
          new Map(
            requests.map((request) => [
              request.id,
              {
                suggestedSubtitle: "森林植被有助保水與穩定土壤",
                visualSummary: "林間步道",
                eventSummary: "步行觀察森林",
                peopleSummary: [],
                locationSummary: ["大雪山"],
                topicRelevanceScore: 90,
                transcriptVisualMatchScore: 70,
                confidence: 84,
                warnings: [],
              },
            ]),
    ...
    const fallback = new AiStoryAnalysisService(
      path.join(root, "app-data", "cache", "ai-story-codex"),
      store,
      sources,
      aiSettings,
      quotaProvider as unknown as OpenAiProvider,
      codex as unknown as CodexCliStoryProvider,
    );
    await fallback.initialize();
    const openAiAnalysisCalls = analyzeStoryFrames.mock.calls.length;
    const before = await sha256(sourcePath);
    const result = await fallback.generateSubtitles({
    ...
    expect(result).toMatchObject({
      ...
      accountName: expect.stringContaining("Codex／ChatGPT"),
    });
    expect(result.project.subtitleCues[0]).toMatchObject({
      text: "森林植被有助保水與穩定土壤",
      reviewStatus: "CONFIRMED",
    ...
    expect(await sha256(sourcePath)).toBe(before);
    const cached = await fallback.generateSubtitles({
      includeSpeechTranscription: false,
      scopes: ["INTRO"],
      targetCueCount: 1,
    });
  ...
  it("builds visual knowledge drafts by default without extracting or transcribing source audio", async () => {
    ...
    const phases: string[] = [];
    const result = await service.generateSubtitles({ includeSpeechTranscription: false }, undefined, (progress) =>
      phases.push(progress.phase),
    );
  ...
  it("transcribes bounded microphone audio in Chinese and rejects unsupported input", async () => {
    ...
    expect(provider.transcribeBytes).toHaveBeenCalledWith(
    ...
    await expect(
      service.transcribeVoiceInput({ audioBytes: new Uint8Array([1]), mimeType: "video/mp4", language: "zh-TW" }),
  ...
  it("returns editable, source-traceable drafts for manually selected video frames", async () => {
    ...
    await store.setSubtitleCues([]);
    const before = await sha256(sourcePath);
    const result = await service.analyzeMaterialForSubtitles({
      assetId: asset.id,
      timelineScope: "MAIN",
      provider: "CHATGPT",
      frameTimesMs: [1_000],
    ...
    expect(result).toMatchObject({
      providerLabel: "OPENAI_API",
      analyzedFrameTimesMs: [1_000],
    ...
    expect(result.drafts).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          sourceInMs: 1_000,
          aiAnalysisVersion: "material-subtitle-v3-gemini-timeline-sync",
        }),
      ]),
    );
    expect(await sha256(sourcePath)).toBe(before);
    ...
    expect(result.overlaps).toEqual([
      {
        ...
        existingCues: [expect.objectContaining({ id: "existing-overlap", text: "既有森林字幕" })],
      },
    ]);
