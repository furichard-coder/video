...
const roots: string[] = [];
async function root() {
  const value = await mkdtemp(path.join(os.tmpdir(), "source-app-translation-settings-"));
  roots.push(value);
  return value;
}
afterEach(async () => {
...
const protector = {
  isAvailable: () => true,
  protect: (value: string) => Buffer.from(`protected:${value}`).toString("base64"),
...
describe("translation settings", () => {
  ...
  it("refuses plaintext persistence when Windows encryption is unavailable", async () => {
    const store = new TranslationSettingsStore(await root(), {
      isAvailable: () => false,
      protect: () => "",
      unprotect: () => "",
    });
