...
describe("editing validation rules", () => {
  it("accepts 0, 100, 200 and 300 percent but rejects values outside the fixed range", () => {
    ...
    expect(() => validatePercent(300.1)).toThrow();
    expect(() => validatePercent(Number.NaN)).toThrow();
    expect(
      [0, 100, 200, 300].map((volumePercent) => validateBgmTrack({ ...bgm, volumePercent }).volumePercent),
  ...
  it("sorts subtitle cues and rejects overlap or empty text", () => {
    const cues = validateSubtitleCues([
      { id: "b", startMs: 2000, endMs: 3000, text: "二" },
      { id: "a", startMs: 0, endMs: 1000, text: "一" },
    ]);
    expect(cues.map((cue) => cue.id)).toEqual(["a", "b"]);
    expect(() =>
      validateSubtitleCues([
        { id: "a", startMs: 0, endMs: 1000, text: "一" },
        { id: "b", startMs: 900, endMs: 2000, text: "二" },
      ]),
    ).toThrow(/不可重疊/);
    expect(
      validateSubtitleCues([
        { id: "a", startMs: 0, endMs: 1000, text: "既有", reviewStatus: "CONFIRMED" },
        { id: "b", startMs: 900, endMs: 2000, text: "待調整", reviewStatus: "DRAFT" },
      ]),
    ).toHaveLength(2);
  ...
  it("sorts and merges overlapping or adjacent Main exclusion ranges and subtracts them without placeholders", () => {
    ...
    expect(retainedRangesForAsset(asset)).toEqual([
      { inMs: 0, outMs: 1_000 },
      { inMs: 5_000, outMs: 6_000 },
    ]);
    expect(() =>
      normalizeMainExclusionRanges([{ id: "bad", startMs: -1, endMs: 100 }], { inMs: 0, outMs: 6_000 }),
    ).toThrow(/目前 IN／OUT/);
  });
  it("uses five seconds for photos and accepts only the three-to-seven-second range", () => {
    ...
    expect(mainRenderSelections({ sources: [photo], timelineOrder: [photo.id] } as ProjectManifest)).toEqual([
      { assetId: photo.id, inMs: 0, outMs: 5_000 },
    ]);
  });
  it("validates non-overlapping local zoom ranges, centers, and clips them to a later IN/OUT", () => {
    ...
    expect(() => validateZoomSegments([{ ...segments[0], zoomPercent: 99 }], { inMs: 0, outMs: 8_000 })).toThrow(
      /100% 到 400%/,
    );
    expect(() => validateZoomSegments([{ ...segments[0], centerXPercent: 101 }], { inMs: 0, outMs: 8_000 })).toThrow(
      /0% 到 100%/,
    );
    expect(() =>
      validateZoomSegments([segments[0], { ...segments[1], startMs: 1_900 }], { inMs: 0, outMs: 8_000 }),
    ).toThrow(/不可重疊/);
