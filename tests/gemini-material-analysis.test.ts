...
describe("Gemini material image analysis", () => {
  it("sends one derived image and normalizes species plus a simplified subtitle", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "gemini-material-"));
    roots.push(root);
    const frame = path.join(root, "frame.jpg");
    await writeFile(frame, Buffer.from([1, 2, 3, 4]));
          ...
          candidates: [
            {
              content: {
                parts: [
                  {
                    text: JSON.stringify({
                      suggestedSubtitle: "林間可見一隻臺灣山羌",
                      visualSummary: "森林中的小型鹿科動物",
                      eventSummary: "山羌站在林下",
                      peopleSummary: [],
                      locationSummary: ["森林"],
                      animalSpecies: ["臺灣山羌"],
                      speciesExplanation: "常活動於森林底層。",
                      topicRelevanceScore: 95,
                      transcriptVisualMatchScore: 80,
                      confidence: 88,
                      warnings: [],
                    }),
                  },
    ...
    const result = await provider.analyze(
      ...
      { model: "gemini-2.5-flash", apiKey: "secret-test-key-that-is-long-enough" },
    );
