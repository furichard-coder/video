...
const candidates = [1, 2, 3].map((index) => ({
  ...
  artist: "作者",
  reason: "適合城市步行",
  auditionUrl: `https://www.youtube.com/watch?v=abc123XY${index}`,
  evidenceUrl: `https://www.youtube.com/watch?v=abc123XY${index}`,
  rightsEvidence: "YouTube Audio Library／授權頁線索，需確認條款",
...
function dependencies(openAiResult: "OK" | "FAIL" = "OK") {
  ...
  const aiSettings = {
    getSnapshot: () => ({ activeAccountId: account.id, accounts: [account] }),
    getRuntimeAccount: () => account,
  } as unknown as AiSettingsStore;
  const openAi = {
    suggestMusic: vi.fn(async () => {
  ...
  const codex = {
    ...
    suggestMusic: vi.fn(async () => ({ suggestions: candidates, warnings: [] })),
  } as unknown as CodexCliStoryProvider;
...
describe("music suggestion service", () => {
  ...
  it("filters out candidates without rights evidence when royalty-free search is enabled", async () => {
    const d = dependencies();
    vi.mocked(d.openAi.suggestMusic).mockResolvedValue({
      suggestions: [...candidates, { ...candidates[0], title: "未標示授權曲", rightsEvidence: "" }],
      warnings: [],
    });
    const service = new MusicSuggestionService(d.store, d.aiSettings, d.openAi, d.codex, d.openExternal, "C:\\cache");
